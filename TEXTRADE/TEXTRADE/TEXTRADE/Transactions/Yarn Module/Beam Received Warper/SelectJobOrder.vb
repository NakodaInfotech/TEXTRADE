...
Public Class SelectJobOrder
    ...
    Sub FILLGRID(ByVal WHERE As String)
        Try
            ...
            Dim dt As DataTable = objcmn.SEARCH(" CAST(0 AS BIT) AS CHK, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ALLJOBORDER.JOB_NO AS JOBNO, ALLJOBORDER.JOB_DATE AS DATE, ISNULL(ALLJOBORDER.JOB_REED, 0) AS REED, ISNULL(ALLJOBORDER.JOB_REEDSPACE, 0) AS REEDSPACE, ISNULL(ALLJOBORDER.JOB_PICKS, 0) AS PICS, ALLJOBORDER.TYPE AS FROMTYPE, ISNULL(ALLJOBORDER.JOB_REFNO, '') AS REFNO, ISNULL(ALLJOBORDER.JOB_TOTALMTRS - ALLJOBORDER.JOB_OUTMTRS, 0) AS MTRS, YARNQUALITYMASTER.YARN_NAME AS YARNNAME  ", "", "  ALLJOBORDER INNER JOIN YARNQUALITYMASTER ON ALLJOBORDER.JOB_ITEMID = YARNQUALITYMASTER.YARN_ID LEFT OUTER JOIN LEDGERS ON ALLJOBORDER.JOB_LEDGERID = LEDGERS.Acc_id ", WHERE & " AND (ALLJOBORDER.JOB_TOTALMTRS- ALLJOBORDER.JOB_OUTMTRS >0) AND ALLJOBORDER.JOB_YEARID = " & YearId)
