...
Public Class YarnLoomEfficiencyDetails
    ...
    Sub FILLGRID()
        Try
            ...
            Dim dt As DataTable = objclsCMST.search(" CAST(0 AS BIT) AS CHK, ISNULL(YARNLOOMEFFICIENCY.YLE_no, 0) AS YLENO, YARNLOOMEFFICIENCY.YLE_date AS DATE, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(CONTRACTMASTER.CONTRACT_NAME, '') AS ROUNDER, ISNULL(YARNLOOMEFFICIENCY.YLE_TOTALRECMTRS, 0) AS TOTALRECMTRS, ISNULL(YARNLOOMEFFICIENCY.YLE_TOTALWEFT, 0) AS TOTALWEFT, ISNULL(YARNLOOMEFFICIENCY.YLE_remarks, '') AS REMARKS, ISNULL(LOOMMASTER_DESC.LOOM_NO, '') AS LOOMNO, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_ITEMNAME, '') AS BEAMNAME, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_GRIDSRNO, 0) AS GRIDSRNO, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_BEAMNO, 0) AS BEAMNO, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_RPM, 0) AS RPM, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_PICKS, 0) AS PICKS, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_RECMTRS, 0) AS RECMTRS, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_WEFT, 0) AS WEFT, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_WARP, 0) AS WARP, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_EFFPER, 0) AS EFFPER, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_AVGPICK, 0) AS AVGPICK, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_GRIDREMARKS, '') AS GRIDREMARKS, ISNULL(YARNLOOMEFFICIENCY_DESC.YLE_DONE, 0) AS DONE  ", "", " YARNLOOMEFFICIENCY INNER JOIN YARNLOOMEFFICIENCY_DESC ON YARNLOOMEFFICIENCY.YLE_no = YARNLOOMEFFICIENCY_DESC.YLE_no AND YARNLOOMEFFICIENCY.YLE_yearid = YARNLOOMEFFICIENCY_DESC.YLE_yearid  LEFT OUTER JOIN LOOMMASTER_DESC ON YARNLOOMEFFICIENCY_DESC.YLE_LOOMNO = LOOMMASTER_DESC.LOOM_NO LEFT OUTER JOIN CONTRACTMASTER ON YARNLOOMEFFICIENCY.YLE_ROUNDERID = CONTRACTMASTER.CONTRACT_ID LEFT OUTER JOIN LEDGERS ON YARNLOOMEFFICIENCY.YLE_ledgerid = LEDGERS.Acc_id  ", " AND  (YARNLOOMEFFICIENCY.YLE_YEARID  = '" & YearId & "') ORDER BY YLENO")
