...
Public Class YarnSaleOrder
    ...
    Private Sub cmbname_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbname.Validating
        Try
            If cmbname.Text.Trim <> "" Then
                ...
                Dim DT As DataTable = OBJCMN.SEARCH("ISNULL(LEDGERS.Acc_crdays, 0) AS Expr1, ISNULL(LEDGERS_1.Acc_cmpname, '') AS transport, ISNULL(LEDGERS_2.Acc_cmpname, '') AS agent, ISNULL(LEDGERS.Acc_mobile, '') AS MOBILENO, ISNULL(LEDGERS.Acc_email,'') AS EMAIL, ISNULL(LEDGERS.ACC_DISC, 0) AS DISC, ISNULL(CITYMASTER.city_name, '') AS CITY ", "", "  LEDGERS LEFT OUTER JOIN CITYMASTER ON LEDGERS.Acc_cityid = CITYMASTER.city_id LEFT OUTER JOIN LEDGERS AS LEDGERS_2 ON LEDGERS.ACC_AGENTID = LEDGERS_2.Acc_id AND LEDGERS.Acc_cmpid = LEDGERS_2.Acc_cmpid AND LEDGERS.Acc_locationid = LEDGERS_2.Acc_locationid AND LEDGERS.Acc_yearid = LEDGERS_2.Acc_yearid LEFT OUTER JOIN LEDGERS AS LEDGERS_1 ON LEDGERS.ACC_TRANSID = LEDGERS_1.Acc_id AND LEDGERS.Acc_cmpid = LEDGERS_1.Acc_cmpid AND LEDGERS.Acc_locationid = LEDGERS_1.Acc_locationid AND LEDGERS.Acc_yearid = LEDGERS_1.Acc_yearid ", " AND ledgers.ACC_CMPNAME = '" & cmbname.Text.Trim & "'  AND ledgers.ACC_CMPID = " & CmpId & " AND ledgers.ACC_LOCATIONID = " & Locationid & " AND ledgers.ACC_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
                    ...
                    If CMBTRANS.Text.Trim = "" Then CMBTRANS.Text = DT.Rows(0).Item("transport")
                    If CMBBROKER.Text.Trim = "" Then CMBBROKER.Text = DT.Rows(0).Item("agent")
                    If cmbcity.Text.Trim = "" Then cmbcity.Text = DT.Rows(0).Item("CITY")
                    If ClientName <> "MASHOK" Then
    ...
    Sub EDITROW()
        Try
            If GRIDSO.CurrentRow.Index >= 0 And GRIDSO.Item(gsrno.Index, GRIDSO.CurrentRow.Index).Value <> Nothing Then
                ...
                CMBMILL.Text = GRIDSO.Item(GMILLNAME.Index, GRIDSO.CurrentRow.Index).Value.ToString
    ...
    Sub PRINTREPORT()
        ...
        '    Dim OBJPO As New YarnSaleOrderDesign
        '    OBJPO.MdiParent = MDIMain
        '    OBJPO.PARTYNAME = cmbname.Text.Trim
        '    OBJPO.AGENTNAME = CMBBROKER.Text.Trim
        '    OBJPO.FRMSTRING = "POREPORT"
        '    OBJPO.FORMULA = "{YARNSALEORDER.YSO_NO}=" & Val(TXTSONO.Text) & " and {YARNSALEORDER.YSO_yearid}=" & YearId
        '    OBJPO.Show()
        'Catch ex As Exception
        '    Throw ex
        'End Try
...
LINE1:
    ...
    Private Sub YarnSaleOrder_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
    ...
    Private Sub CMBPACKING_Validated(sender As Object, e As EventArgs) Handles CMBPACKING.Validated
        Try
            If CMBPACKING.Text.Trim <> "" Then
                ...
                If DT.Rows.Count > 0 Then
                    If ClientName <> "MASHOK" Then CMBTRANS.Text = DT.Rows(0).Item("transport")
    ...
    Private Sub CMBPACKING_Validating(sender As Object, e As CancelEventArgs) Handles CMBPACKING.Validating
        Try
            If CMBPACKING.Text.Trim <> "" Then NAMEVALIDATE(CMBPACKING, cmbcode, e, Me, txtadd, " AND  (GROUP_SECONDARY = 'SUNDRY DEBTORS' OR GROUP_SECONDARY = 'SUNDRY CREDITORS')", "Sundry Debtors", "ACCOUNTS", CMBTRANS.Text)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
