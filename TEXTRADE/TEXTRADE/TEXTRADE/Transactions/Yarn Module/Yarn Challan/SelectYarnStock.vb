    ...
    Sub FILLGRID()
        Try
            ...
            If ALLOWYARNBARCODEPRINT = True Then
                ...
                DT = OBJCMN.SEARCH(" CAST(0 AS BIT) AS CHK, YARNQUALITY, CATEGORY, MILLNAME, DESIGNNO, COLOR, LOTNO, LRNO, SUM(ISNULL(CONES, 0)) AS CONES, SUM(BAGS) AS BAGS, SUM(WT) AS WT,  BARCODE, 0 AS FROMNO, 0 AS FROMSRNO, '' AS FROMTYPE, ISNULL(RACK, '') AS RACK  ", "", "  YARNBARCODESTOCK ", WHERECLAUSE & " GROUP BY GODOWN, YARNQUALITY, CATEGORY, MILLNAME, DESIGNNO, COLOR, LOTNO, LRNO, BARCODE, ISNULL(RACK, '') HAVING SUM(WT) > 0 ")
            Else
