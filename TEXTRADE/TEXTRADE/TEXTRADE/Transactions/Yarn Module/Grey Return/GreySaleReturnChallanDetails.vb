...
Public Class GreySaleReturnChallanDetails
    ...
    Private Sub SaleReturnChallanDetails_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            DTROW = USERRIGHTS.Select("FormName = 'GREYSALE RETURN'")
    ...
    Sub FILLGRID()
        Try
            Dim OBJPURCH As New ClsGreySaleReturnChallan
    ...
    Sub showform(ByVal editval As Boolean, ByVal SRCHNO As Integer)
        Try
            ...
            If (editval = False) Or (editval = True And gridbill.RowCount > 0) Then
                Dim OBJPURCH As New GreySaleReturnChallan
    ...
    Sub serverprop(ByVal fromno As Integer, ByVal tono As Integer, Optional ByVal NOOFCOPIES As Integer = 1, Optional ByVal FRMSTRING As String = "PRINT")
        Try
            ...
            For I As Integer = fromno To tono
                ...
                OBJ.RecordSelectionFormula = "{GREYSALERETURNCHALLAN.GSRCH_no}=" & Val(I) & " and {GREYSALERETURNCHALLAN.GSRCH_yearid}=" & YearId
            ...
            If FRMSTRING = "MAIL" Then
                ...
                OBJMAIL.subject = "Grey Sale Return"
                OBJMAIL.ShowDialog()
            End If
        ...
        Try
            ...
            For I As Integer = 0 To gridbill.RowCount - 1
                ...
                If ROW("CHK") = True Then
                    ...
                    OBJ.RecordSelectionFormula = "{GREYSALERETURNCHALLAN.GSRCH_no}=" & Val(ROW("SRCHNO")) & " and {GREYSALERETURNCHALLAN.GSRCH_yearid}=" & YearId
    ...
    Private Sub ToolStripButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton2.Click
        Try
            ...
            opti.SheetName = "Grey Sale Return Challan Details"
            gridbill.ExportToXls(PATH, opti)
            EXCELCMPHEADER(PATH, "Grey Sale Return Challan Details", gridbill.VisibleColumns.Count + gridbill.GroupCount)
        Catch ex As Exception
            MsgBox("Grey Sale Return Challan Details Excel File is Open, Please Close the File first then try to Export", MsgBoxStyle.Critical)
        End Try
    End Sub
    ...
    Private Sub TOOLWHATSAPP_Click(sender As Object, e As EventArgs) Handles TOOLWHATSAPP.Click
        Try
            If Val(TXTFROM.Text.Trim) > 0 And Val(TXTTO.Text.Trim) > 0 Then
                If Val(TXTFROM.Text.Trim) > Val(TXTTO.Text.Trim) Then
                    MsgBox("Enter Proper Grey Sale Return Challan Nos", MsgBoxStyle.Critical)
                    Exit Sub
                Else
                    If MsgBox("Wish to Whatsapp Grey Sale Return Challan from " & Val(TXTFROM.Text.Trim) & " To " & Val(TXTTO.Text.Trim) & " ?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then Exit Sub
                    serverprop(Val(TXTFROM.Text.Trim), Val(TXTTO.Text.Trim), Val(TXTCOPIES.Text.Trim), "WHATSAPP")
                End If
            Else
                If MsgBox("Wish to Whatsapp Selected Grey Sale Return Challan ?", MsgBoxStyle.YesNo) = vbYes Then
                    cmdok.Focus()
        ...
        Try
            If Val(TXTFROM.Text.Trim) > 0 And Val(TXTTO.Text.Trim) > 0 Then
                If Val(TXTFROM.Text.Trim) > Val(TXTTO.Text.Trim) Then
                    ...
                    Exit Sub
                Else
                    If MsgBox("Wish to Mail Grey Sale Return Challan from " & Val(TXTFROM.Text.Trim) & " To " & Val(TXTTO.Text.Trim) & " ?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then Exit Sub
                    serverprop(Val(TXTFROM.Text.Trim), Val(TXTTO.Text.Trim), Val(TXTCOPIES.Text.Trim), "MAIL")
                End If
            Else
                If MsgBox("Wish to Mail Selected Grey Sale Return Challan ?", MsgBoxStyle.YesNo) = vbYes Then
                    cmdok.Focus()
