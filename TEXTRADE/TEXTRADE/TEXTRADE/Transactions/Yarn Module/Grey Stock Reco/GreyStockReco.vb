...
Public Class GreyStockReco
    ...
    Sub GETUNCHECKEDDATA()
        Try
            ...
            Dim OBJCMN As New ClsCommon()
            Dim dttable As DataTable = OBJCMN.SEARCH("GREYBARCODESTOCK.*", "", " GREYBARCODESTOCK ", " AND GREYBARCODESTOCK.BARCODE NOT IN (SELECT BARCODE FROM STOCKTAKING_DESC WHERE YEARID = " & YearId & ") AND GREYBARCODESTOCK.YEARID = " & YearId & TEMPCONDITION & " AND GREYBARCODESTOCK.BARCODE IN (" & BARCODE & ")")
            If dttable.Rows.Count > 0 Then
                For Each dr As DataRow In dttable.Rows
    ...
    Private Sub StockReco_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If EDIT = True Then
                ...
                Dim objSTOCK As New ClsGreyStockAdjustment()
                ...
                If dttable.Rows.Count > 0 Then
                    ...
                    Dim OBJCMN As New ClsCommon
                    Dim DT As DataTable = OBJCMN.SEARCH("ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_GRIDSRNO, 0) AS GRIDSRNO, ISNULL(PIECETYPEMASTER.PIECETYPE_name, '') AS PIECETYPE, ISNULL(ITEMMASTER.item_name, '') AS ITEM, ISNULL(QUALITYMASTER.QUALITY_name, '') AS QUALITY, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_BALENO, '')  AS BALENO, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_GRIDDESC, '') AS GRIDDESC, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_LOTNO, '') AS LOTNO, ISNULL(DESIGNMASTER.DESIGN_NO, '') AS DESIGN, ISNULL(COLORMASTER.COLOR_name, '') AS COLOR, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_CUT, 0) AS CUT, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_QTY, 0) AS QTY, ISNULL(UNITMASTER.unit_abbr, '') AS UNIT,  ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_MTRS, 0) AS MTRS, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_BARCODE, '') AS BARCODE, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_OUTPCS, 0) AS OUTPCS, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_OUTMTRS, 0) AS OUTMTRS, GREYSTOCKADJUSTMENT_INDESC.GREYSA_GRIDDONE AS GRIDDONE, ISNULL(SHELFMASTER.SHELF_NAME, '') AS INSHELF, ISNULL(RACKMASTER.RACK_NAME, '') AS INRACK, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_RATE, 0) AS RATE, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_PER, 'Mtrs') AS PER, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_AMOUNT, 0) AS AMOUNT ", "", " GREYSTOCKADJUSTMENT_INDESC INNER JOIN PIECETYPEMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_PIECETYPEID = PIECETYPEMASTER.PIECETYPE_id LEFT OUTER JOIN RACKMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_RACKID = RACKMASTER.RACK_ID LEFT OUTER JOIN SHELFMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_SHELFID = SHELFMASTER.SHELF_ID LEFT OUTER JOIN UNITMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_QTYUNITID = UNITMASTER.unit_id LEFT OUTER JOIN DESIGNMASTER AS DESIGNMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN QUALITYMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_QUALITYID = QUALITYMASTER.QUALITY_id LEFT OUTER JOIN COLORMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN  ITEMMASTER AS ITEMMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_ITEMID = ITEMMASTER.item_id ", " AND GREYSA_NO = " & TEMPRECONO & " AND GREYSA_YEARID = " & YearId & " ORDER BY GREYSTOCKADJUSTMENT_INDESC.GREYSA_GRIDSRNO")
        ...
        Try
            ...
            If ALLOWMANUALRECNO = True Then
                If Val(TXTRECONO.Text.Trim) <> 0 And EDIT = False Then
                    ...
                    Dim dttable As DataTable = OBJCMNn.SEARCH(" ISNULL(GREYSTOCKADJUSTMENT.GREYSA_NO,0)  AS RECONO", "", " GREYSTOCKADJUSTMENT ", "  AND GREYSTOCKADJUSTMENT.GREYSA_NO=" & Val(TXTRECONO.Text.Trim) & " AND GREYSTOCKADJUSTMENT.GREYSA_YEARID = " & YearId)
                    If dttable.Rows.Count > 0 Then
    ...
    Private Function CHECKBARCODE() As Boolean
        Try
            Dim BLN As Boolean = True
            Dim OBJCMN As New ClsCommon
            Dim DT As DataTable = OBJCMN.SEARCH(" ISNULL(GREYSA_BARCODE,'') AS BARCODE ", "", " GREYSTOCKADJUSTMENT_INDESC ", " AND GREYSA_YEARID =  " & YearId)
            If DT.Rows.Count > 0 Then
    ...
    Sub getmaxno()
        ...
        DTTABLE = getmax(" isnull(max(GREYSA_no),0) + 1 ", " GREYSTOCKADJUSTMENT ", " AND GREYSA_yearid=" & YearId)
    ...
    Sub PRINTBARCODE()
        Try
            If ALLOWBARCODEPRINT Then
                ...
                Dim DT1 As DataTable = OBJCMN1.SEARCH("ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_GRIDSRNO, 0) AS GRIDSRNO, ISNULL(PIECETYPEMASTER.PIECETYPE_name, '') AS PIECETYPE, ISNULL(ITEMMASTER.item_name, '') AS ITEM, ISNULL(QUALITYMASTER.QUALITY_name, '') AS QUALITY, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_BALENO, '')  AS BALENO, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_GRIDDESC, '') AS GRIDDESC, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_LOTNO, '') AS LOTNO, ISNULL(DESIGNMASTER.DESIGN_NO, '') AS DESIGN, ISNULL(COLORMASTER.COLOR_name, '') AS COLOR, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_CUT, 0) AS CUT, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_QTY, 0) AS QTY, ISNULL(UNITMASTER.unit_abbr, '') AS UNIT,  ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_MTRS, 0) AS MTRS, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_RATE, 0) AS RATE, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_PER, 'Mtrs') AS PER, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_AMOUNT, 0) AS AMOUNT, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_BARCODE, '') AS BARCODE, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_OUTPCS, 0) AS OUTPCS, ISNULL(GREYSTOCKADJUSTMENT_INDESC.GREYSA_OUTMTRS, 0) AS OUTMTRS, GREYSTOCKADJUSTMENT_INDESC.GREYSA_GRIDDONE AS GRIDDONE, ISNULL(SHELFMASTER.SHELF_NAME, '') AS INSHELF, ISNULL(RACKMASTER.RACK_NAME, '') AS INRACK ", "", " GREYSTOCKADJUSTMENT_INDESC INNER JOIN PIECETYPEMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_PIECETYPEID = PIECETYPEMASTER.PIECETYPE_id LEFT OUTER JOIN RACKMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_RACKID = RACKMASTER.RACK_ID LEFT OUTER JOIN SHELFMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_SHELFID = SHELFMASTER.SHELF_ID LEFT OUTER JOIN UNITMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_QTYUNITID = UNITMASTER.unit_id LEFT OUTER JOIN DESIGNMASTER AS DESIGNMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN QUALITYMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_QUALITYID = QUALITYMASTER.QUALITY_id LEFT OUTER JOIN COLORMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN  ITEMMASTER AS ITEMMASTER ON GREYSTOCKADJUSTMENT_INDESC.GREYSA_ITEMID = ITEMMASTER.item_id ", " AND GREYSA_NO = " & TEMPRECONO & " AND GREYSA_YEARID = " & YearId & " ORDER BY GREYSTOCKADJUSTMENT_INDESC.GREYSA_GRIDSRNO")
    ...
    Private Sub cmddelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddelete.Click
        Try
            If EDIT = True Then
                ...
                Dim ALPARAVAL As New ArrayList
                Dim OBSTOCK As New ClsGreyStockAdjustment
...
LINE1:
    ...
    Private Sub TXTCHNO_Validated(sender As Object, e As EventArgs) Handles TXTCHNO.Validated
        Try
            If (ClientName = "SONU" Or ClientName = "KCRAYON" Or ClientName = "INDRAPUJAFABRICS" Or ClientName = "MNARESH") And CMBCMPNAME.Text.Trim <> "" And Val(TXTCHNO.Text.Trim) > 0 And EDIT = False Then
                ...
                Dim ALPARAVAL As New ArrayList
                Dim objclsGDN As New ClsGreyStockAdjustment()
    ...
    Private Sub TXTRECONO_Validating(sender As Object, e As CancelEventArgs) Handles TXTRECONO.Validating
        Try
            If Val(TXTRECONO.Text.Trim) <> 0 And EDIT = False Then
                Dim OBJCMN As New ClsCommon
                Dim dttable As DataTable = OBJCMN.SEARCH(" ISNULL(GREYSTOCKADJUSTMENT.GREYSA_NO,0)  AS RECONO", "", " GREYSTOCKADJUSTMENT ", "  AND GREYSTOCKADJUSTMENT.GREYSA_NO=" & Val(TXTRECONO.Text.Trim) & " AND GREYSTOCKADJUSTMENT.GREYSA_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
                    MsgBox("Rec No Already Exist")
                    e.Cancel = True
                End If
