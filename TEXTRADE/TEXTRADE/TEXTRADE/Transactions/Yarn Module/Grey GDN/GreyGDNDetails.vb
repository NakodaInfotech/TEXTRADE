...
Public Class GreyGDNDetails
    ...
    Private Sub SALEOrderDetails_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If ClientName <> "SAFFRON" And ClientName <> "SAFFRONOFF" Then fillgrid(" and dbo.GREYGDN.gdn_yearid=" & YearId & " ")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Sub fillgrid(ByVal TEMPCONDITION)
        Try
            ...
            Dim dt As DataTable = objclsCMST.search("CAST(0 AS BIT) AS CHK, GREYGDN.GDN_TYPENO AS TYPECHALLANNO, GREYGDN.GDN_NO AS SRNO, ISNULL(GREYGDN.GDN_TRANSREFNO, '') AS CHNO, GREYGDN.GDN_date AS DATE, LEDGERS.Acc_cmpname AS CMPNAME, ISNULL(JOBBERLEDGERS.Acc_cmpname, '') AS JOBBERNAME, ISNULL(GREYGDN.GDN_CONSIGNEE, '') AS CONSIGNEE, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENT, GREYGDN.GDN_SONO AS SONO, GREYGDN.GDN_MULTISONO AS MULTISONO, GREYGDN.GDN_SODATE AS SODATE, GREYGDN.GDN_TOTALPCS AS PCS, GREYGDN.GDN_TOTALMTRS AS MTRS, GREYGDN.GDN_TOTALBALES AS TOTALBALES, GREYGDN.GDN_BALENOFROM AS BALENOFROM , ISNULL(PACKINGLEDGERS.Acc_cmpname, '') AS PACKING, ISNULL(GREYGDN.GDN_TRANSREMARKS, '') AS PARTYPONO, ISNULL(GREYGDN.GDN_HOLDFORAPPROVAL, 0) AS HOLDFORAPPROVAL, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSPORT, ISNULL(CITYMASTER.city_name, '') AS CITYNAME, ISNULL(LEDGERS.ACC_WHATSAPPNO, '0') AS PARTYWHATSAPP, ISNULL(LEDGERS.Acc_email, '') AS PARTYEMAIL, ISNULL(AGENTLEDGERS.Acc_email, '') AS AGENTEMAIL, ISNULL(AGENTLEDGERS.ACC_WHATSAPPNO, '0') AS AGENTWHATSAPP, ISNULL(USERMASTER.User_Name, '') AS USERNAME", "", "   GREYGDN INNER JOIN LEDGERS ON GREYGDN.GDN_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN USERMASTER ON GREYGDN.GDN_USERID = USERMASTER.User_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON GREYGDN.GDN_AGENTID = AGENTLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS JOBBERLEDGERS ON GREYGDN.GDN_JOBBERID = JOBBERLEDGERS.Acc_id LEFT OUTER JOIN CHALLANTYPEMASTER ON CHALLANTYPEMASTER.CHALLANTYPE_ID = GREYGDN.GDN_TYPEID LEFT OUTER JOIN LEDGERS AS PACKINGLEDGERS ON GREYGDN.GDN_DISPATCHTOID = PACKINGLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GREYGDN.GDN_transid = TRANSLEDGERS.Acc_id LEFT OUTER JOIN CITYMASTER ON GREYGDN.GDN_CITYid = CITYMASTER.city_id", TEMPCONDITION & " ORDER BY GREYGDN.GDN_NO")
    ...
    Private Sub PrintToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PrintToolStripButton.Click
        Try
            ...
            '    MsgBox("Enter Propoer Grey Challan Nos", MsgBoxStyle.Critical)
            '    Exit Sub
            'Else
            '    If MsgBox("Wish to Print Grey Challan from " & Val(TXTFROM.Text.Trim) & " To " & Val(TXTTO.Text.Trim) & " ?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then Exit Sub
            ...
            If Val(TXTFROM.Text.Trim) > 0 And Val(TXTTO.Text.Trim) > 0 Then
                If Val(TXTFROM.Text.Trim) > Val(TXTTO.Text.Trim) Then
                    MsgBox("Enter Proper Grey Challan Nos", MsgBoxStyle.Critical)
                    Exit Sub
                End If
                If MsgBox("Wish to Print Grey Challan from " & TXTFROM.Text.Trim & " To " & TXTTO.Text.Trim & " ?", MsgBoxStyle.YesNo) = vbYes Then
                    If PRINTDIALOG.ShowDialog = DialogResult.OK Then PRINTDOC.PrinterSettings = PRINTDIALOG.PrinterSettings
                    SERVERPROP(Val(TXTFROM.Text.Trim), Val(TXTTO.Text.Trim), Val(TXTCOPIES.Text.Trim))
                End If
            Else
                If MsgBox("Wish to Print Selected Grey Challan ?", MsgBoxStyle.YesNo) = vbYes Then
                    If PRINTDIALOG.ShowDialog = DialogResult.OK Then PRINTDOC.PrinterSettings = PRINTDIALOG.PrinterSettings
                    cmdok.Focus()
    ...
    Sub SERVERPROP(ByVal fromno As Integer, ByVal tono As Integer, Optional ByVal NOOFCOPIES As Integer = 1, Optional ByVal FRMSTRING As String = "PRINT")
        Try
            ...
            If (ClientName = "MANSI" Or ClientName = "CHINTAN") AndAlso MsgBox("Print Grey Challan for Garments?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then GARMENTCHALLAN = True
            ...
            For I As Integer = fromno To tono
                ...
                OBJ.RecordSelectionFormula = "{GREYGDN.GDN_no}=" & Val(I) & " and {GREYGDN.GDN_yearid}=" & YearId
                ...
                Else
                    oDfDopt.DiskFileName = Application.StartupPath & "\GREYGDN_" & I & ".pdf"
        ...
        Try
            ...
            For I As Integer = 0 To gridbill.RowCount - 1
                ...
                If ROW("CHK") = True Then
                    ...
                    OBJ.RecordSelectionFormula = "{GREYGDN.GDN_no}=" & Val(ROW("SRNO")) & " and {GREYGDN.GDN_yearid}=" & YearId
                    ...
                    If FRMSTRING = "PRINT" Then
                        ...
                        OBJ.PrintToPrinter(Val(NOOFCOPIES), True, 0, 0)
                    Else
                        oDfDopt.DiskFileName = Application.StartupPath & "\" & ROW("CMPNAME") & "GREYGDN_" & ROW("SRNO") & ".pdf"
                        expo = OBJ.ExportOptions
                        expo.ExportDestinationType = ExportDestinationType.DiskFile
                        expo.ExportFormatType = ExportFormatType.PortableDocFormat
                        expo.DestinationOptions = oDfDopt
                        OBJ.Export()
                        ALATTACHMENT.Add(oDfDopt.DiskFileName)
                        FILENAME.Add(ROW("CMPNAME") & "GREYGDN_" & ROW("SRNO") & ".pdf")

                        'ADDINT IN DTEMAIL
                        DTMAIL.Rows.Add(ROW("SRNO"), 0, "", ROW("SRNO"), ROW("DATE"), ROW("CMPNAME"), ROW("PARTYEMAIL"), ROW("AGENT"), ROW("AGENTEMAIL"), 0, UCase(CmpName) & " - Grey Challan No. " & ROW("SRNO") & " Dated " & ROW("DATE"), oDfDopt.DiskFileName, ROW("CMPNAME") & "GREYGDN_" & ROW("SRNO") & ".pdf")

                        'ADDING IN DTWHATSAPP
                        DTWHATSAPP.Rows.Add(ROW("SRNO"), 0, "", ROW("SRNO"), ROW("DATE"), ROW("CMPNAME"), ROW("PARTYWHATSAPP"), ROW("AGENT"), ROW("AGENTWHATSAPP"), 0, UCase(CmpName) & " - Grey Challan No. " & ROW("SRNO") & " Dated " & ROW("DATE"), oDfDopt.DiskFileName, ROW("CMPNAME") & "GREYGDN_" & ROW("SRNO") & ".pdf")
            ...
            If FRMSTRING = "MAIL" Then
                ...
                OBJEMAIL.FORMTYPE = "Grey Challan"
                OBJEMAIL.DT = DTMAIL
                OBJEMAIL.ShowDialog()
                Exit Sub
    ...
    Private Sub ToolStripButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton2.Click
        Try
            Dim PATH As String = Application.StartupPath & "\Grey Challan Details.XLS"
            Dim opti As New DevExpress.XtraPrinting.XlsExportOptions
            opti.ShowGridLines = True
            opti.SheetName = "Grey Challan Details"
            gridbill.ExportToXls(PATH, opti)
            EXCELCMPHEADER(PATH, "Grey Challan Details", gridbill.VisibleColumns.Count + gridbill.GroupCount)
        Catch ex As Exception
            MsgBox("Grey Challan Details Excel File is Open, Please Close the File first then try to Export", MsgBoxStyle.Critical)
        End Try
    End Sub
    ...
    Private Sub TOOLGRIDDETAILS_Click(sender As Object, e As EventArgs) Handles TOOLGRIDDETAILS.Click
        Try
            Dim OBJDTLS As New GreyGDNGridDetails
        ...
        Try
            If Val(TXTFROM.Text.Trim) > 0 And Val(TXTTO.Text.Trim) > 0 Then
                If Val(TXTFROM.Text.Trim) > Val(TXTTO.Text.Trim) Then
                    ...
                    Exit Sub
                Else
                    If MsgBox("Wish to Whatsapp Grey Challan from " & Val(TXTFROM.Text.Trim) & " To " & Val(TXTTO.Text.Trim) & " ?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then Exit Sub
                    SERVERPROP(Val(TXTFROM.Text.Trim), Val(TXTTO.Text.Trim), Val(TXTCOPIES.Text.Trim), "WHATSAPP")
                End If
            Else
                If MsgBox("Wish to Whatsapp Selected Grey Challan ?", MsgBoxStyle.YesNo) = vbYes Then
                    cmdok.Focus()
    ...
    Private Sub TOOLREFRESH_Click(sender As Object, e As EventArgs) Handles TOOLREFRESH.Click
        Try
            fillgrid(" AND CHALLANTYPE_NAME = '" & CMBTYPE.Text.Trim & "' and dbo.GREYGDN.gdn_yearid=" & YearId & " ")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub CMBTYPE_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles CMBTYPE.Validated
        Try
            If CMBTYPE.Text <> "" And (ClientName = "SAFFRON" Or ClientName = "SAFFRONOFF") Then fillgrid(" AND CHALLANTYPE_NAME = '" & CMBTYPE.Text.Trim & "' and dbo.GREYGDN.gdn_yearid=" & YearId & " ")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
        ...
        Try
            If Val(TXTFROM.Text.Trim) > 0 And Val(TXTTO.Text.Trim) > 0 Then
                If Val(TXTFROM.Text.Trim) > Val(TXTTO.Text.Trim) Then
                    ...
                    Exit Sub
                Else
                    If MsgBox("Wish to Mail Grey Challan from " & Val(TXTFROM.Text.Trim) & " To " & Val(TXTTO.Text.Trim) & " ?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then Exit Sub
                    SERVERPROP(Val(TXTFROM.Text.Trim), Val(TXTTO.Text.Trim), Val(TXTCOPIES.Text.Trim), "MAIL")
                End If
            Else
                If MsgBox("Wish to Mail Selected Grey Challan ?", MsgBoxStyle.YesNo) = vbYes Then
                    cmdok.Focus()
