...
Public Class YarnJobOrder
    ...
    Sub getmaxno()
        ...
        DTTABLE = getmax(" isnull(max(JOB_no),0) + 1 ", " JOBORDER ", " AND JOB_cmpid=" & CmpId & " and JOB_locationid=" & Locationid & " and JOB_yearid=" & YearId)
...
LINE1:
            'temptypename = cmbtype.Text.Trim
            tempdesignno = Val(TXTJONO.Text) - 1
...
LINE1:
            ...
            clear()
            If Val(TXTJONO.Text) - 1 >= tempdesignno Then
                EDIT = True
                'DesignCardMaster_Load(sender, e)
                SHOWDATA()
            Else
