...
Public Class YarnJobOrder
    ...
    Sub SHOWDATA()
        Try
            ...
            If dttable.Rows.Count > 0 Then
                ...
                Dim dttable3 As DataTable = OBJCMN.SEARCH(" ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGESRNO, 0) AS SELVEDGEGRIDSRNO, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGESYM, '') AS SELVEDGEGRIDSYM, ISNULL(YARNQUALITYMASTER.YARN_NAME, '') AS SELVEDGEYARNQUALITY, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGEDENIER, 0) AS SELVEDGEDENIER, ISNULL(MILLMASTER.MILL_NAME, '') AS SELVEDGEMILLNAME, ISNULL(COLORMASTER.COLOR_name, '') AS SELVEDGESHADE, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGEPE, 0) AS SELVEDGEPE, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGEBE, 0) AS SELVEDGEBE, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGETE, 0) AS SELVEDGETE, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGEWT, 0) AS SELVEDGEWT, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGECONS, 0) AS SELVEDGECONS, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGERATE, 0) AS SELVEDGERATE, ISNULL(JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGECOST, 0) AS SELVEDGECOST", "", " JOBORDER_SELVEDGEMATCHING LEFT OUTER JOIN COLORMASTER ON JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGECOLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN MILLMASTER ON JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGEMILLID = MILLMASTER.MILL_ID LEFT OUTER JOIN YARNQUALITYMASTER ON JOBORDER_SELVEDGEMATCHING.JOB_SELVEDGEYARNQUALITYID = YARNQUALITYMASTER.YARN_ID   ", " AND  JOBORDER_SELVEDGEMATCHING.JOB_NO = " & TEMPJONO & " AND JOBORDER_SELVEDGEMATCHING.JOB_YEARID = " & YearId & " ORDER BY SELVEDGEGRIDSRNO")
    ...
    Private Sub cmdok_Click(sender As Object, e As EventArgs) Handles cmdok.Click
        Try
            ...
            'GRID SELVEDGE
            ' Initialize variables for pipe-separated strings
            Dim SELVEDGESrNo As String = ""
            Dim SELVEDGESym As String = ""
            Dim SELVEDGEYarnQuality As String = ""
            Dim SELVEDGEDenier As String = ""
            Dim SELVEDGEMillName As String = ""
            Dim SELVEDGEShade As String = ""
            Dim SELVEDGEPE As String = ""
            Dim SELVEDGEBE As String = ""
            Dim SELVEDGETE As String = ""
            Dim SELVEDGEWt As String = ""
            Dim SELVEDGECons As String = ""
            Dim SELVEDGERate As String = ""
            Dim SELVEDGECost As String = ""

            For Each row As Windows.Forms.DataGridViewRow In GRIDSELVEDGE.Rows
                If row.Cells(SSRNO.Index).Value IsNot Nothing Then
                    If SELVEDGESrNo = "" Then
                        SELVEDGESrNo = Val(row.Cells(SSRNO.Index).Value)
                        SELVEDGESym = row.Cells(SSYM.Index).Value.ToString
                        SELVEDGEYarnQuality = row.Cells(SQUALITY.Index).Value.ToString
                        SELVEDGEDenier = Val(row.Cells(SDENIER.Index).Value)
                        SELVEDGEMillName = row.Cells(SMILL.Index).Value.ToString
                        SELVEDGEShade = row.Cells(SSHADE.Index).Value.ToString
                        SELVEDGEPE = Val(row.Cells(SPE.Index).Value)
                        SELVEDGEBE = Val(row.Cells(SBE.Index).Value)
                        SELVEDGETE = Val(row.Cells(SENDS.Index).Value)
                        SELVEDGEWt = Val(row.Cells(SWT.Index).Value)
                        SELVEDGECons = Val(row.Cells(SCONS.Index).Value)
                        SELVEDGERate = Val(row.Cells(SRATE.Index).Value)
                        SELVEDGECost = Val(row.Cells(SCOST.Index).Value)
                    Else
                        SELVEDGESrNo = SELVEDGESrNo & "|" & row.Cells(SSRNO.Index).Value
                        SELVEDGESym = SELVEDGESym & "|" & row.Cells(SSYM.Index).Value.ToString
                        SELVEDGEYarnQuality = SELVEDGEYarnQuality & "|" & row.Cells(SQUALITY.Index).Value.ToString
                        SELVEDGEDenier = SELVEDGEDenier & "|" & Val(row.Cells(SDENIER.Index).Value)
                        SELVEDGEMillName = SELVEDGEMillName & "|" & row.Cells(SMILL.Index).Value.ToString
                        SELVEDGEShade = SELVEDGEShade & "|" & row.Cells(SSHADE.Index).Value.ToString
                        SELVEDGEPE = SELVEDGEPE & "|" & Val(row.Cells(SPE.Index).Value)
                        SELVEDGEBE = SELVEDGEBE & "|" & Val(row.Cells(SBE.Index).Value)
                        SELVEDGETE = SELVEDGETE & "|" & Val(row.Cells(SENDS.Index).Value)
                        SELVEDGEWt = SELVEDGEWt & "|" & Val(row.Cells(SWT.Index).Value)
                        SELVEDGECons = SELVEDGECons & "|" & Val(row.Cells(SCONS.Index).Value)
                        SELVEDGERate = SELVEDGERate & "|" & Val(row.Cells(SRATE.Index).Value)
                        SELVEDGECost = SELVEDGECost & "|" & Val(row.Cells(SCOST.Index).Value)
                    End If
                End If
            Next

            alParaval.Add(SELVEDGESrNo)
            alParaval.Add(SELVEDGESym)
            alParaval.Add(SELVEDGEYarnQuality)
            alParaval.Add(SELVEDGEDenier)
            alParaval.Add(SELVEDGEMillName)
            alParaval.Add(SELVEDGEShade)
            alParaval.Add(SELVEDGEPE)
            alParaval.Add(SELVEDGEBE)
            alParaval.Add(SELVEDGETE)
            alParaval.Add(SELVEDGEWt)
            alParaval.Add(SELVEDGECons)
            alParaval.Add(SELVEDGERate)
            alParaval.Add(SELVEDGECost)
