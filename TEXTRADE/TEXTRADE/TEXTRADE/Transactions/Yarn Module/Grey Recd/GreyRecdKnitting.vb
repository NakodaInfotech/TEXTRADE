...
NEXTLINE:
    ...
    Private Sub cmdok_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            ...
            Dim JOBORDEROTHERITEMNAME As String = ""
            Dim JOBORDERREFNO As String = ""
            Dim JOBORDERPONO As String = ""
            Dim JOBORDERPARTYNAME As String = ""


            For Each row As Windows.Forms.DataGridViewRow In GRIDJOBORDER.Rows
                If row.Cells(0).Value <> Nothing Then
                    ...
                    If JOBORDERGRIDSRNO = "" Then
                        ...
                        JOBORDEROTHERITEMNAME = row.Cells(POTHERITEMNAME.Index).Value.ToString
                        JOBORDERREFNO = row.Cells(PREFNO.Index).Value.ToString
                        JOBORDERPONO = row.Cells(PPONO.Index).Value.ToString
                        JOBORDERPARTYNAME = row.Cells(PPARTYNAME.Index).Value.ToString
                    ...
                    Else
                        ...
                        JOBORDEROTHERITEMNAME = JOBORDEROTHERITEMNAME & "|" & row.Cells(POTHERITEMNAME.Index).Value.ToString
                        JOBORDERREFNO = JOBORDERREFNO & "|" & row.Cells(PREFNO.Index).Value.ToString
                        JOBORDERPONO = JOBORDERPONO & "|" & row.Cells(PPONO.Index).Value.ToString
                        JOBORDERPARTYNAME = JOBORDERPARTYNAME & "|" & row.Cells(PPARTYNAME.Index).Value.ToString
            ...
            alParaval.Add(JOBORDEROTHERITEMNAME)
            alParaval.Add(JOBORDERREFNO)
            alParaval.Add(JOBORDERPONO)
            alParaval.Add(JOBORDERPARTYNAME)
        ...
        Try
            ...
            If EDIT = True Then
                ...
                If dttable.Rows.Count > 0 Then
                    ...
                    'JOBORDER GRID
                    'Dim OBJCMN As New ClsCommon
                    dttable = OBJCMN.SEARCH("  GREYRECDKNITTING_JODETAILS.GREY_GRIDSRNO AS GRIDSRNO, ITEMMASTER.item_name AS ITEMNAME, ISNULL(DESIGNMASTER.DESIGN_NO, '') AS DESIGNNO, ISNULL(COLORMASTER.COLOR_name, '') AS COLOR, ISNULL(GREYRECDKNITTING_JODETAILS.GREY_ORDERMTRS, 0) AS ORDERMTRS, GREYRECDKNITTING_JODETAILS.GREY_FROMNO AS FROMNO, GREYRECDKNITTING_JODETAILS.GREY_FROMSRNO AS FROMSRNO, GREYRECDKNITTING_JODETAILS.GREY_FROMTYPE AS FROMTYPE, ISNULL(GREYRECDKNITTING_JODETAILS.GREY_MTRS, 0) AS GRNMTRS, ISNULL(GREYRECDKNITTING_JODETAILS.GREY_OTHERITEMNAME, '') AS OTHERITEMNAME, ISNULL(GREYRECDKNITTING_JODETAILS.GREY_REFNO, '') AS REFNO, ISNULL(GREYRECDKNITTING_JODETAILS.GREY_PONO, '') AS PONO, ISNULL(LEDGERS.Acc_cmpname, '') AS PARTYNAME  ", "", " GREYRECDKNITTING_JODETAILS INNER JOIN ITEMMASTER ON GREYRECDKNITTING_JODETAILS.GREY_ITEMID = ITEMMASTER.item_id LEFT OUTER JOIN LEDGERS ON GREYRECDKNITTING_JODETAILS.GREY_PARTYNAMEID = LEDGERS.Acc_id LEFT OUTER JOIN COLORMASTER ON GREYRECDKNITTING_JODETAILS.GREY_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN DESIGNMASTER ON GREYRECDKNITTING_JODETAILS.GREY_DESIGNID = DESIGNMASTER.DESIGN_id ", " AND GREYRECDKNITTING_JODETAILS.GREY_NO = " & TEMPGREYNO & "  AND GREYRECDKNITTING_JODETAILS.GREY_YEARID = " & YearId)
                    If dttable.Rows.Count > 0 Then
                        For Each DTR As DataRow In dttable.Rows
                            GRIDJOBORDER.Rows.Add(Val(DTR("GRIDSRNO")), DTR("ITEMNAME"), DTR("DESIGNNO"), DTR("COLOR"), DTR("OTHERITEMNAME"), DTR("REFNO"), DTR("PONO"), DTR("PARTYNAME"), Val(DTR("ORDERMTRS")), Val(DTR("FROMNO")), Val(DTR("FROMSRNO")), DTR("FROMTYPE"), Val(DTR("GRNMTRS")))
                        Next
                    End If
        ...
        Try
            ...
            If DTPO.Rows.Count > 0 Then
                ...
                For Each DTROW As DataRow In DTPO.Rows
                    ...
                    GRIDJOBORDER.Rows.Add(0, DTROW("ITEMNAME"), DTROW("DESIGNNO"), DTROW("COLOR"), DTROW("OTHERITEMNAME"), DTROW("REFNO"), DTROW("PONO"), DTROW("PARTYNAME"), Val(DTROW("JOBMTRS")), DTROW("JOBNO"), DTROW("JOBSRNO"), DTROW("FROMTYPE"), 0)
...
NEXTLINE:
    ...
    Private Sub CMBDYEINGNAME_Validated(sender As Object, e As EventArgs) Handles CMBDYEINGNAME.Validated
        Try
            If ClientName = "SWPL" Then
                TXTROLLNO.Focus()
            End If
