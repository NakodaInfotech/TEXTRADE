...
Public Class GreyJobOut
    ...
    Sub getmaxno()
        ...
        DTTABLE = getmax(" isnull(max(GJO_no),0) + 1 ", " GREYJOBOUT ", " AND GJO_cmpid=" & CmpId & " and GJO_locationid=" & Locationid & " and GJO_yearid=" & YearId)
        ...
        Try
            ...
            If TXTCHALLANNO.Text.Trim <> "" And (ClientName <> "RADHA" And ClientName <> "VINTAGEINDIA") Then
                If (EDIT = False) Or (EDIT = True And LCase(PARTYCHALLANNO) <> LCase(TXTCHALLANNO.Text.Trim)) Then
                    'for search
                    DT = OBJCMN.SEARCH(" GJO_challanno, LEDGERS.ACC_cmpname", "", " GREYJOBOUT inner join LEDGERS on LEDGERS.ACC_id = GJO_ledgerid ", " and GJO_challanno = '" & TXTCHALLANNO.Text.Trim & "' and LEDGERS.ACC_cmpname = '" & CMBNAME.Text.Trim & "' AND GJO_YEARID =" & YearId)
    ...
    Private Sub cmdok_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            ...
            'WHILE ADDING COLUMN IN GREYJOBOUT DONT FORGET TO ADD SAME COLUMNS IN FORMS GIVEN BELOW
            '1) JOBIN -- DIRECTGREYJOBOUT 
            '2) GRN --- DIRECTGREYJOBOUT
            ...
            Dim objCUTTING As New ClsGreyJobOut()
    ...
    Private Sub GREYJOBOUT_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        Try
            ...
            If EDIT = True Then
                ...
                If dttable.Rows.Count > 0 Then
                    ...
                    Dim OBJCMN As New ClsCommon
                    dttable = OBJCMN.SEARCH(" GJO_GRIDSRNO AS GRIDSRNO, GJO_REMARKS AS REMARKS, GJO_NAME AS NAME, GJO_IMGPATH AS IMGPATH, GJO_NEWIMGPATH AS NEWIMGPATH", "", " GREYJOBOUT_UPLOAD", " AND GJO_NO = " & TEMPJONO & " AND GJO_CMPID = " & CmpId & " AND GJO_LOCATIONID = " & Locationid & " AND GJO_YEARID = " & YearId)
                    If dttable.Rows.Count > 0 Then
...
LINE1:
    ...
    Private Sub cmddelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddelete.Click
        Try
            If EDIT = True Then
                ...
                Dim ALPARAVAL As New ArrayList
                Dim OBJEMB As New ClsGreyJobOut
        ...
        Try
            If TXTCHALLANNO.Text.Trim.Length > 0 And ClientName <> "RADHA" Then
                If (EDIT = False) Or (EDIT = True And LCase(PARTYCHALLANNO) <> LCase(TXTCHALLANNO.Text.Trim)) Then
                    ...
                    Dim dt As DataTable = objclscommon.SEARCH(" GJO_challanno, LEDGERS.ACC_cmpname", "", " GREYJOBOUT inner join LEDGERS on LEDGERS.ACC_id = GJO_ledgerid ", " and GJO_challanno = '" & TXTCHALLANNO.Text.Trim & "' and LEDGERS.ACC_cmpname = '" & CMBNAME.Text.Trim & "' AND GJO_YEARID =" & YearId)
                    If dt.Rows.Count > 0 Then
    ...
    Sub GENERATEEWB()
        Try
            ...
            If TEMPSTATUS = "FAILED" Then
                ...
                DT = OBJCMN.Execute_Any_String("INSERT INTO EWAYENTRY VALUES (" & Val(TXTJONO.Text.Trim) & ",'GREYJOBOUT','" & TOKEN & "','','" & TEMPSTATUS & "', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                Exit Sub
            End If
            ...
            Catch ex As WebException
                ...
                'ADD DATA IN EWAYENTRY
                DT = OBJCMN.Execute_Any_String("INSERT INTO EWAYENTRY VALUES (" & Val(TXTJONO.Text.Trim) & ",'GREYJOBOUT','" & TOKEN & "','','FAILED', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                Exit Sub
            End Try
            ...
            DT = OBJCMN.Execute_Any_String("UPDATE GREYJOBOUT SET GJO_EWAYBILLNO = '" & TXTEWAYBILLNO.Text.Trim & "' FROM GREYJOBOUT WHERE JOB_NO = " & Val(TEMPJONO) & "  AND INVOICE_YEARID = " & YearId, "", "")

            'ADD DATA IN EWAYENTRY
            DT = OBJCMN.Execute_Any_String("INSERT INTO EWAYENTRY VALUES (" & Val(TXTJONO.Text.Trim) & ",'GREYJOBOUT','" & TOKEN & "','" & EWBNO & "','" & TEMPSTATUS & "', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
    ...
    Sub PRINTEWB()
        Try
            ...
            Try
                ...
                'ADD DATA IN EWAYENTRY
                DT = OBJCMN.Execute_Any_String("INSERT INTO EWAYENTRY VALUES (" & Val(TXTJONO.Text.Trim) & ",'GREYJOBOUT','" & TOKENNO & "','" & EWBNO & "','PRINT SUCCESS1', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                DT = OBJCMN.Execute_Any_String("INSERT INTO EWAYENTRY VALUES (" & Val(TXTJONO.Text.Trim) & ",'GREYJOBOUT','" & TOKENNO & "','" & EWBNO & "','PRINT SUCCESS2', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")

            Catch ex As WebException
                RESPONSE = ex.Response
                MsgBox("Error While Printing EWB, Please check the Data Properly")
                'ADD DATA IN EWAYENTRY
                DT = OBJCMN.Execute_Any_String("INSERT INTO EWAYENTRY VALUES (" & Val(TXTJONO.Text.Trim) & ",'GREYJOBOUT','" & TOKENNO & "','" & EWBNO & "','PRINT FAILED1', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                DT = OBJCMN.Execute_Any_String("INSERT INTO EWAYENTRY VALUES (" & Val(TXTJONO.Text.Trim) & ",'GREYJOBOUT','" & TOKENNO & "','" & EWBNO & "','PRINT FAILED2', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                Exit Sub
            End Try
    ...
    Private Sub TOOLWHATSAPP_Click(sender As Object, e As EventArgs) Handles TOOLWHATSAPP.Click
        Try
            ...
            DT = OBJCMN.Execute_Any_String("UPDATE GREYJOBOUT SET GJO_SENDWHATSAPP = 1 WHERE GJO_NO = " & TEMPJONO & " AND GJO_YEARID = " & YearId, "", "")
    ...
    Async Sub SENDWHATSAPP(JONO As Integer)
        Try
            ...
            OBJJO.FORMULA = "{GREYJOBOUT.GJO_NO}=" & Val(JONO) & " and {GREYJOBOUT.GJO_yearid}=" & YearId
            OBJJO.JONO = Val(JONO)
            OBJJO.NOOFCOPIES = 1
            OBJJO.Show()
            OBJJO.Close()
            ...
            OBJWHATSAPP.PATH.Add(Application.StartupPath & "\GJO_" & Val(JONO) & ".pdf")
            OBJWHATSAPP.FILENAME.Add("GJO_" & Val(JONO) & ".pdf")
    ...
    Sub SMSCODE()
        If ALLOWSMS = True Then
            ...
            If SENDMSG(MSG, TXTMOBILENO.Text.Trim) = "1701" Then
                MsgBox("Message Sent")
                DT = OBJCMN.Execute_Any_String("UPDATE GREYJOBOUT SET GJO_SMSSEND = 1 WHERE GJO_NO = " & TEMPJONO & " AND GJO_YEARID = " & YearId, "", "")
                LBLSMS.Visible = True
            Else
