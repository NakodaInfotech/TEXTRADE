...
CHECKNEXTLINEMTRS:
    ...
    Private Sub cmdok_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            ...
            Dim OBJJobIn As New ClsGreyJobIn()
        ...
        Try
            If ALLOWBARCODEPRINT Then
                ...
                For Each ROW As DataGridViewRow In GRIDJOBIN.Rows
                    ...
                    Dim OBJCMN As New ClsCommon
                    dttable = OBJCMN.Execute_Any_String("UPDATE GREYJOBIN Set GJI_BARCODEPRINTED = 1 WHERE GJI_NO = " & TEMPJOBINNO & " And GJI_YEARID = " & YearId, "", "")
...
NEXTLINE:
    ...
    Private Sub GreyJobIn_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub GreyJobIn_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If EDIT = True Then
                ...
                dttable = OBJCMN.SEARCH(" GJI_GRIDSRNO AS GRIDSRNO, GJI_REMARKS AS REMARKS, GJI_NAME AS NAME, GJI_IMGPATH AS IMGPATH, GJI_NEWIMGPATH AS NEWIMGPATH", "", " GREYJOBIN_UPLOAD", " AND GJI_NO = " & TEMPJOBINNO & " AND GJI_CMPID = " & CmpId & " AND GJI_LOCATIONID = " & Locationid & " AND GJI_YEARID = " & YearId)
    ...
    Private Sub OpenToolStripButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles OpenToolStripButton.Click
        Try
            ...
            Dim OBJJobinDetails As New GreyJobInDetails
...
LINE1:
    ...
    Private Sub cmddelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddelete.Click
        Try
            ...
            If EDIT = True Then
                ...
                If lbllocked.Visible = True Then
                    MsgBox("Grey JobIn Receipt Locked", MsgBoxStyle.Critical)
                    Exit Sub
                End If

                TEMPMSG = MsgBox("Delete Grey JobIn Receipt?", MsgBoxStyle.YesNo)
                If TEMPMSG = vbYes Then
                    Dim alParaval As New ArrayList
                    ...
                    Dim OBJMATREC As New ClsGreyJobIn()
                    OBJMATREC.alParaval = alParaval
                    IntResult = OBJMATREC.Delete()
                    MsgBox("Grey JobIn Receipt Deleted")
                    CLEAR()
                    EDIT = False
    ...
    Private Sub cmbname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbname.Validated
        Try
            If EDIT = False And cmbname.Text.Trim <> "" Then
                ...
                Else
                    DT = OBJCMN.SEARCH(" JONO ", "", " (SELECT GREYJOBOUT.GJO_no AS JONO FROM GREYJOBOUT INNER JOIN LEDGERS ON GREYJOBOUT.GJO_ledgerid = LEDGERS.Acc_id WHERE LEDGERS.Acc_CMPNAME='" & cmbname.Text.Trim & "' AND ROUND((GREYJOBOUT.GJO_TOTALMTRS - GREYJOBOUT.GJO_RECDMTRS),2) > 0 AND GREYJOBOUT.GJO_CLOSE=0 AND ISNULL(GREYJOBOUT.GJO_LOTCOMPLETED,0)=0 AND GREYJOBOUT.GJO_YEARID = " & YearId & " UNION ALL SELECT DISTINCT SM_BILLNO AS JONO FROM STOCKMASTER INNER JOIN LEDGERS ON STOCKMASTER.SM_LEDGERIDTO= LEDGERS.Acc_id WHERE LEDGERS.ACC_CMPNAME = '" & cmbname.Text.Trim & "' AND ROUND((SM_MTRS - SM_OUTMTRS),2) > 0 AND SM_BILLNO <> 0 AND ISNULL(SM_LOTCOMPLETED,0)=0 AND SM_YEARID = " & YearId & " AND (ISNULL(SM_DYEINGJOB,'')= '' OR ISNULL(SM_DYEINGJOB,'') = 'JOB')) AS T", "")
                End If
                If DT.Rows.Count > 0 Then
    ...
    Sub fillolddesign(ByRef cmbDESIGN As ComboBox)
        Try
            ...
            Dim DT As DataTable = OBJCMN.SEARCH(" (SELECT DISTINCT DESIGNMASTER.DESIGN_NO) AS DESIGN ", "", " GREYJOBOUT INNER JOIN GREYJOBOUT_DESC ON GREYJOBOUT.GJO_no = GREYJOBOUT_DESC.GJO_NO AND GREYJOBOUT.GJO_cmpid = GREYJOBOUT_DESC.GJO_CMPID AND GREYJOBOUT.GJO_locationid = GREYJOBOUT_DESC.GJO_LOCATIONID AND GREYJOBOUT.GJO_yearid = GREYJOBOUT_DESC.GJO_YEARID LEFT OUTER JOIN COLORMASTER ON GREYJOBOUT_DESC.GJO_YEARID = COLORMASTER.COLOR_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = COLORMASTER.COLOR_locationid AND GREYJOBOUT_DESC.GJO_CMPID = COLORMASTER.COLOR_cmpid AND GREYJOBOUT_DESC.GJO_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN DESIGNMASTER ON GREYJOBOUT_DESC.GJO_YEARID = DESIGNMASTER.DESIGN_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = DESIGNMASTER.DESIGN_locationid AND GREYJOBOUT_DESC.GJO_CMPID = DESIGNMASTER.DESIGN_cmpid AND GREYJOBOUT_DESC.GJO_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN QUALITYMASTER ON GREYJOBOUT_DESC.GJO_YEARID = QUALITYMASTER.QUALITY_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = QUALITYMASTER.QUALITY_locationid AND GREYJOBOUT_DESC.GJO_CMPID = QUALITYMASTER.QUALITY_cmpid AND GREYJOBOUT_DESC.GJO_QUALITYID = QUALITYMASTER.QUALITY_id LEFT OUTER JOIN ITEMMASTER ON GREYJOBOUT_DESC.GJO_YEARID = ITEMMASTER.item_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = ITEMMASTER.item_locationid AND GREYJOBOUT_DESC.GJO_CMPID = ITEMMASTER.item_cmpid AND GREYJOBOUT_DESC.GJO_ITEMID = ITEMMASTER.item_id LEFT OUTER JOIN PIECETYPEMASTER ON GREYJOBOUT_DESC.GJO_YEARID = PIECETYPEMASTER.PIECETYPE_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = PIECETYPEMASTER.PIECETYPE_locationid AND GREYJOBOUT_DESC.GJO_CMPID = PIECETYPEMASTER.PIECETYPE_cmpid AND GREYJOBOUT_DESC.GJO_PIECETYPEID = PIECETYPEMASTER.PIECETYPE_id", " AND GREYJOBOUT.GJO_no=" & Val(CMBJONO.Text.Trim) & " AND GREYJOBOUT.GJO_CMPID = " & CmpId & " AND GREYJOBOUT.GJO_LOCATIONID = " & Locationid & " AND GREYJOBOUT.GJO_YEARID = " & YearId)
            If DT.Rows.Count > 0 Then
    ...
    Private Sub GreyJobIn_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
    ...
    Private Sub CMBBARCODE_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMBBARCODE.Validated
        Try
            If CMBBARCODE.Text.Trim <> "" Then
                Dim OBJCMN As New ClsCommon
                Dim DTR As DataTable = OBJCMN.SEARCH("T.JONO, T.MTRS AS MTRS, T.FROMNO AS FROMNO, T.FROMSRNO AS FROMSRNO, T.GRIDTYPE AS GRIDTYPE", "", " (SELECT GJO_NO AS JONO, GJO_MTRS AS MTRS, GJO_NO AS FROMNO, GJO_GRIDSRNO AS FROMSRNO, 'GREYJOBOUT' AS GRIDTYPE FROM GREYJOBOUT_DESC WHERE GJO_BARCODE='" & CMBBARCODE.Text.Trim & "' AND GJO_YEARID = " & YearId & " UNION ALL SELECT SM_BILLNO AS BILLNO,  SM_MTRS AS MTRS, SM_NO AS FROMNO, SM_NO AS FROMSRNO, 'OPENING' AS GRIDTYPE FROM STOCKMASTER WHERE SM_BARCODE = '" & CMBBARCODE.Text.Trim & "' AND SM_YEARID = " & YearId & ") AS T", "")
        ...
        Try
            If CMBPCSNO.Text.Trim <> "" And TXTFROMNO.Text.Trim <> "" And TXTFROMTYPE.Text.Trim <> "" And CMBJONO.Text.Trim <> "" And cmbname.Text.Trim <> "" Then
                ...
                ElseIf ClientName = "KARAN" Then
                    DT = OBJCMN.SEARCH(" ISNULL(GJO_MTRS,0) AS MTRS, ISNULL(DESIGNMASTER.DESIGN_NO,'') AS DESIGNNO, ISNULL(ITEMMASTER.ITEM_NAME,'') AS  ITEMNAME, GJO_GRIDSRNO AS FROMSRNO, ISNULL(COLORMASTER.COLOR_NAME,'') AS COLOR, '' AS PARTYDESIGNNO ", "", " GREYJOBOUT_DESC INNER JOIN ITEMMASTER ON GJO_ITEMID = ITEM_ID LEFT OUTER JOIN DESIGNMASTER ON GJO_DESIGNID = DESIGN_ID LEFT OUTER JOIN COLORMASTER ON GJO_COLORID = COLOR_ID ", " AND GJO_GRIDSRNO = " & Val(CMBPCSNO.Text.Trim) & " AND GJO_NO = " & Val(CMBJONO.Text.Trim) & " AND GJO_YEARID = " & YearId)
                End If
                If DT.Rows.Count > 0 Then
    ...
    Private Sub CMBJONO_Validated(sender As Object, e As EventArgs) Handles CMBJONO.Validated
        Try
            If Val(CMBJONO.Text) > 0 Then
                ...
                'Dim DT As DataTable = OBJCMN.search(" GREYJOBOUT_DESC.GJO_GRIDSRNO AS GRIDSRNO, (SELECT DISTINCT PIECETYPEMASTER.PIECETYPE_name) AS PIECETYPE, GREYJOBOUT_DESC.GJO_BALENO AS BALENO, ITEMMASTER.item_name AS ITEM, (SELECT DISTINCT QUALITYMASTER.QUALITY_name) AS QUALITY, (SELECT DISTINCT DESIGNMASTER.DESIGN_NO) AS DESIGN, (SELECT DISTINCT COLORMASTER.COLOR_name) AS COLOR, GREYJOBOUT_DESC.GJO_BARCODE AS BARCODE, (GREYJOBOUT.GJO_TOTALMTRS - GREYJOBOUT.GJO_RECDMTRS) AS BALANCEMTRS, GREYJOBOUT.GJO_TOTALMTRS AS OUTMTRS , GREYJOBOUT_DESC.GJO_FROMNO AS FROMNO, GREYJOBOUT_DESC.GJO_FROMSRNO AS FROMSRNO, GREYJOBOUT_DESC.GJO_FROMTYPE AS FROMTYPE", "", " GREYJOBOUT INNER JOIN GREYJOBOUT_DESC ON GREYJOBOUT.GJO_no = GREYJOBOUT_DESC.GJO_NO AND GREYJOBOUT.GJO_cmpid = GREYJOBOUT_DESC.GJO_CMPID AND GREYJOBOUT.GJO_locationid = GREYJOBOUT_DESC.GJO_LOCATIONID AND GREYJOBOUT.GJO_yearid = GREYJOBOUT_DESC.GJO_YEARID LEFT OUTER JOIN COLORMASTER ON GREYJOBOUT_DESC.GJO_YEARID = COLORMASTER.COLOR_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = COLORMASTER.COLOR_locationid AND GREYJOBOUT_DESC.GJO_CMPID = COLORMASTER.COLOR_cmpid AND GREYJOBOUT_DESC.GJO_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN DESIGNMASTER ON GREYJOBOUT_DESC.GJO_YEARID = DESIGNMASTER.DESIGN_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = DESIGNMASTER.DESIGN_locationid AND GREYJOBOUT_DESC.GJO_CMPID = DESIGNMASTER.DESIGN_cmpid AND GREYJOBOUT_DESC.GJO_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN QUALITYMASTER ON GREYJOBOUT_DESC.GJO_YEARID = QUALITYMASTER.QUALITY_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = QUALITYMASTER.QUALITY_locationid AND GREYJOBOUT_DESC.GJO_CMPID = QUALITYMASTER.QUALITY_cmpid AND GREYJOBOUT_DESC.GJO_QUALITYID = QUALITYMASTER.QUALITY_id LEFT OUTER JOIN ITEMMASTER ON GREYJOBOUT_DESC.GJO_YEARID = ITEMMASTER.item_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = ITEMMASTER.item_locationid AND GREYJOBOUT_DESC.GJO_CMPID = ITEMMASTER.item_cmpid AND GREYJOBOUT_DESC.GJO_ITEMID = ITEMMASTER.item_id LEFT OUTER JOIN PIECETYPEMASTER ON GREYJOBOUT_DESC.GJO_YEARID = PIECETYPEMASTER.PIECETYPE_yearid AND GREYJOBOUT_DESC.GJO_LOCATIONID = PIECETYPEMASTER.PIECETYPE_locationid AND GREYJOBOUT_DESC.GJO_CMPID = PIECETYPEMASTER.PIECETYPE_cmpid AND GREYJOBOUT_DESC.GJO_PIECETYPEID = PIECETYPEMASTER.PIECETYPE_id", " AND GREYJOBOUT.GJO_no=" & Val(CMBJONO.Text.Trim) & " AND GREYJOBOUT.GJO_CMPID = " & CmpId & " AND GREYJOBOUT.GJO_LOCATIONID = " & Locationid & " AND GREYJOBOUT.GJO_YEARID = " & YearId)
                Dim DT As DataTable = OBJCMN.SEARCH(" SUM(BALANCEMTRS) AS BALANCEMTRS, SUM(OUTMTRS) AS OUTMTRS, TYPE ", "", " (SELECT SUM((GREYJOBOUT.GJO_TOTALMTRS - GREYJOBOUT.GJO_RECDMTRS)) AS BALANCEMTRS, SUM(GREYJOBOUT.GJO_RECDMTRS) AS OUTMTRS, 'GREYJOBOUT' AS TYPE FROM GREYJOBOUT INNER JOIN LEDGERS ON GREYJOBOUT.GJO_ledgerid = LEDGERS.Acc_id WHERE LEDGERS.Acc_CMPNAME='" & cmbname.Text.Trim & "' AND ROUND((GREYJOBOUT.GJO_TOTALMTRS - GREYJOBOUT.GJO_RECDMTRS),2) > 0 AND GREYJOBOUT.GJO_CLOSE=0 AND GREYJOBOUT.GJO_LOTCOMPLETED = 'FALSE' AND GREYJOBOUT.GJO_NO = " & Val(CMBJONO.Text.Trim) & " AND GREYJOBOUT.GJO_YEARID = " & YearId & " UNION ALL SELECT  (SUM(SM_MTRS) -SUM( SM_OUTMTRS)) AS BALANCEMTRS, SUM(SM_MTRS)  AS OUTMTRS, 'OPENING' AS TYPE FROM STOCKMASTER INNER JOIN LEDGERS ON STOCKMASTER.SM_LEDGERIDTO= LEDGERS.Acc_id WHERE LEDGERS.ACC_CMPNAME = '" & cmbname.Text.Trim & "' AND SM_BILLNO <> 0 AND SM_LOTCOMPLETED = 'FALSE' AND (ISNULL(SM_DYEINGJOB,'')= '' OR ISNULL(SM_DYEINGJOB,'') = 'JOB') AND SM_YEARID = " & YearId & " AND SM_BILLNO = " & Val(CMBJONO.Text.Trim) & ") AS T", " GROUP BY TYPE HAVING ISNULL(SUM(BALANCEMTRS),0) > 0")
                If DT.Rows.Count > 0 Then

                    'IF WE HAVE 2 RECORDS THEN WE HAVE TO ASK USER WHERTHER WE HAVE TO FETCH DATA FROM OPENING OR GREYJOBOUT
                    If DT.Rows.Count > 1 Then
                        Dim TEMPWHERE As String = ""
                        If MsgBox("Same Challan No is Present in Opening & Current Year, Fetch Data from Opening?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then TEMPWHERE = " AND T.TYPE = 'OPENING' " Else TEMPWHERE = " AND T.TYPE = 'GREYJOBOUT' "
                        DT = OBJCMN.SEARCH(" SUM(BALANCEMTRS) AS BALANCEMTRS, SUM(OUTMTRS) AS OUTMTRS, TYPE ", "", " (SELECT SUM((GREYJOBOUT.GJO_TOTALMTRS - GREYJOBOUT.GJO_RECDMTRS)) AS BALANCEMTRS, SUM(GREYJOBOUT.GJO_RECDMTRS) AS OUTMTRS, 'GREYJOBOUT' AS TYPE FROM GREYJOBOUT INNER JOIN LEDGERS ON GREYJOBOUT.GJO_ledgerid = LEDGERS.Acc_id WHERE LEDGERS.Acc_CMPNAME='" & cmbname.Text.Trim & "' AND ROUND((GREYJOBOUT.GJO_TOTALMTRS - GREYJOBOUT.GJO_RECDMTRS),2) > 0 AND GREYJOBOUT.GJO_CLOSE=0 AND GREYJOBOUT.GJO_NO = " & Val(CMBJONO.Text.Trim) & " AND GREYJOBOUT.GJO_YEARID = " & YearId & " UNION ALL SELECT  (SUM(SM_MTRS) -SUM( SM_OUTMTRS)) AS BALANCEMTRS, SUM(SM_MTRS)  AS OUTMTRS, 'OPENING' AS TYPE FROM STOCKMASTER INNER JOIN LEDGERS ON STOCKMASTER.SM_LEDGERIDTO= LEDGERS.Acc_id WHERE LEDGERS.ACC_CMPNAME = '" & cmbname.Text.Trim & "' AND SM_BILLNO <> 0 AND SM_YEARID = " & YearId & " AND SM_BILLNO = " & Val(CMBJONO.Text.Trim) & " AND (ISNULL(SM_DYEINGJOB,'')= '' OR ISNULL(SM_DYEINGJOB,'') = 'JOB')) AS T", TEMPWHERE & " GROUP BY TYPE HAVING ISNULL(SUM(BALANCEMTRS),0) > 0")
                    End If
                    ...
                    Else
                        If DT.Rows(0).Item("TYPE") = "GREYJOBOUT" Then
                            DTR = OBJCMN.SEARCH(" isnull(PIECETYPEMASTER.PIECETYPE_name,'FRESH') AS PIECETYPE, ISNULL(ITEMMASTER.item_name,'') AS ITEM, ISNULL(PROCESSMASTER.PROCESS_NAME, '') AS PROCESS, GJO_BARCODE AS BARCODE, ISNULL(DESIGNMASTER.DESIGN_NO,'') AS DESIGNNO, ISNULL(GREYJOBOUT.GJO_LOTNO,'') AS LOTNO, ISNULL(GREYJOBOUT.GJO_CHALLANNO,'') AS WEAVERCHNO, ISNULL(WEAVERLEDGERS.ACC_CMPNAME,'') AS WEAVERNAME, GREYJOBOUT_DESC.GJO_GRIDSRNO AS GRIDSRNO, ISNULL(GREYJOBOUT.GJO_TOTALRATE,0) AS RATE, GREYJOBOUT.GJO_NO AS FROMNO, 'GREYJOBOUT' AS FROMTYPE, '' as BALENO ", "", "  GREYJOBOUT INNER JOIN GREYJOBOUT_DESC ON GREYJOBOUT.GJO_no = GREYJOBOUT_DESC.GJO_NO AND GREYJOBOUT.GJO_yearid = GREYJOBOUT_DESC.GJO_YEARID LEFT OUTER JOIN PROCESSMASTER ON GREYJOBOUT.GJO_PROCESSID = PROCESSMASTER.PROCESS_ID LEFT OUTER JOIN COLORMASTER ON GREYJOBOUT_DESC.GJO_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN DESIGNMASTER ON GREYJOBOUT_DESC.GJO_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN QUALITYMASTER ON GREYJOBOUT_DESC.GJO_QUALITYID = QUALITYMASTER.QUALITY_id LEFT OUTER JOIN ITEMMASTER ON GREYJOBOUT_DESC.GJO_ITEMID = ITEMMASTER.item_id LEFT OUTER JOIN PIECETYPEMASTER ON GREYJOBOUT_DESC.GJO_PIECETYPEID = PIECETYPEMASTER.PIECETYPE_id LEFT OUTER JOIN LEDGERS AS WEAVERLEDGERS ON GREYJOBOUT.GJO_PARTYLEDGERID = WEAVERLEDGERS.ACC_ID", " AND GREYJOBOUT.GJO_no=" & Val(CMBJONO.Text.Trim) & " AND (GJO_MTRS - ROUND(GJO_OUTMTRS,0)) > 0 AND GREYJOBOUT.GJO_YEARID = " & YearId & " ORDER BY GJO_GRIDSRNO")
                        Else
                    ...
                    If DTR.Rows.Count > 0 Then
                        ...
                        'FOR MANSI FETCH ALL GRID DATA FROM GREYJOBOUT
                        If (ClientName = "MANSI" Or ClientName = "SNCM") And DT.Rows(0).Item("TYPE") = "GREYJOBOUT" Then
                            Dim objJO As New ClsGreyJobOut()
                            Dim ALPARAVAL As New ArrayList
                            ALPARAVAL.Add(CMBJONO.Text.Trim)
                            ALPARAVAL.Add(CmpId)
