...
Public Class GreyJobOutDetails
    ...
    Private Sub GreyJobOutDetails_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub GreyJobOutDetails_LOAD(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If ClientName <> "SAFFRON" And ClientName <> "SAFFRONOFF" Then fillgrid(" AND dbo.GREYJOBOUT.GJO_yearid=" & YearId & " order by dbo.GREYJOBOUT.GJO_no ")
    End Sub

    Sub fillgrid(ByVal TEMPCONDITION)
        Try
            Dim objclsCMST As New ClsCommonMaster
            Dim dt As DataTable = objclsCMST.search("  GREYJOBOUT.GJO_no AS SRNO, GREYJOBOUT.GJO_TYPENO AS TYPENO, GREYJOBOUT.GJO_date AS DATE, LEDGERS.Acc_cmpname AS NAME, ISNULL(PARTYNAMELEDGERS.Acc_cmpname, '') AS PARTYNAME,  ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN, ISNULL(GREYJOBOUT.GJO_LOTNO, 0) AS LOTNO, ISNULL(GREYJOBOUT.GJO_CHALLANNO, 0) AS CHALLANNO, GREYJOBOUT.GJO_TOTALPCS AS TOTALPCS, GREYJOBOUT.GJO_TOTALMTRS AS TOTALMTRS, GREYJOBOUT.GJO_RECDMTRS AS RECDMTRS, ROUND(GREYJOBOUT.GJO_TOTALMTRS - GREYJOBOUT.GJO_RECDMTRS, 2) AS BALMTRS, ISNULL(GREYJOBOUT.GJO_remarks, '') AS REMARKS, ISNULL(PROCESSMASTER.PROCESS_NAME, '') AS PROCESS, GREYJOBOUT.GJO_BALENUMBER AS BALENUMBER, ISNULL(GREYJOBOUT.GJO_LOTREADY, 0) AS LOTREADY, ISNULL(GREYJOBOUT.GJO_TOTALAMOUNT, 0) AS TOTALAMOUNT, ISNULL(GREYJOBOUT.GJO_AVGWT, 0) AS AVGWT, ISNULL(LEDGERS.ACC_WHATSAPPNO, '0') AS PARTYWHATSAAP, ISNULL(LEDGERS.Acc_email, '') AS PARTYEMAIL, ISNULL(USERMASTER.User_Name, '') AS USERNAME, ISNULL(DISPATCHLEDGERS.Acc_cmpname, '') AS DISPATCHFROM ", "", " GREYJOBOUT INNER JOIN LEDGERS ON GREYJOBOUT.GJO_ledgerid = LEDGERS.Acc_id AND GREYJOBOUT.GJO_cmpid = LEDGERS.Acc_cmpid AND GREYJOBOUT.GJO_locationid = LEDGERS.Acc_locationid AND GREYJOBOUT.GJO_yearid = LEDGERS.Acc_yearid LEFT OUTER JOIN  LEDGERS AS DISPATCHLEDGERS ON GREYJOBOUT.GJO_DISPATCHFROMID = DISPATCHLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS PARTYNAMELEDGERS ON GREYJOBOUT.GJO_partyledgerid = PARTYNAMELEDGERS.Acc_id LEFT OUTER JOIN USERMASTER ON GREYJOBOUT.GJO_userid = USERMASTER.User_id LEFT OUTER JOIN GODOWNMASTER ON GREYJOBOUT.GJO_yearid = GODOWNMASTER.GODOWN_yearid AND GREYJOBOUT.GJO_locationid = GODOWNMASTER.GODOWN_locationid AND GREYJOBOUT.GJO_cmpid = GODOWNMASTER.GODOWN_cmpid AND  GREYJOBOUT.GJO_GODOWNID = GODOWNMASTER.GODOWN_id LEFT OUTER JOIN PROCESSMASTER ON PROCESSMASTER.PROCESS_ID = GREYJOBOUT.GJO_PROCESSID LEFT OUTER JOIN GREYJOBOUTTYPEMASTER ON GREYJOBOUTTYPEMASTER.GJOTYPE_ID = GREYJOBOUT.GJO_TYPEID ", Where & TEMPCONDITION)
    ...
    Sub SERVERPROPDIRECT(Optional ByVal INVOICEMAIL As Boolean = False, Optional ByVal WHATSAPP As Boolean = False)
        Try
            ...
            For I As Integer = Val(TXTFROM.Text.Trim) To Val(TXTTO.Text.Trim)
                ...
                OBJJO.FORMULA = "{GREYJOBOUT.GJO_NO}=" & Val(I) & " and {GREYJOBOUT.GJO_yearid}=" & YearId
                OBJJO.JONO = Val(I)
                OBJJO.NOOFCOPIES = Val(TXTCOPIES.Text.Trim)
                OBJJO.Show()
                OBJJO.Close()
                ALATTACHMENT.Add(Application.StartupPath & "\GJO_" & I & ".pdf")
                FILENAME.Add("GREYJOBOUT_" & I & ".pdf")
            Next

            If INVOICEMAIL Then
                Dim OBJMAIL As New SendMail
                OBJMAIL.ALATTACHMENT = ALATTACHMENT
                OBJMAIL.subject = "GREYJOBOUT"
                OBJMAIL.ShowDialog()
            End If
    ...
    Sub SERVERPROPSELECTED(Optional ByVal INVOICEMAIL As Boolean = False, Optional ByVal WHATSAPP As Boolean = False)
        Try
            ...
            For I As Integer = 0 To Val(SELECTEDROWS.Length - 1)
                ...
                OBJJO.GSTRPT = GSTRPT
                OBJJO.PRINTSETTING = PRINTDIALOG
                OBJJO.FORMULA = "{GREYJOBOUT.GJO_NO}=" & Val(ROW("SRNO")) & " and {GREYJOBOUT.GJO_yearid}=" & YearId
                OBJJO.JONO = Val(ROW("SRNO"))
                OBJJO.PARTYNAME = ROW("NAME")
                OBJJO.NOOFCOPIES = Val(TXTCOPIES.Text.Trim)
                OBJJO.Show()
                OBJJO.Close()
                ALATTACHMENT.Add(Application.StartupPath & "\" & ROW("NAME") & "GJO_" & Val(ROW("SRNO")) & ".pdf")
                FILENAME.Add(ROW("NAME") & "GJO_" & Val(ROW("SRNO")) & ".pdf")
            Next
    ...
    Private Sub TOOLREFRESH_Click(sender As Object, e As EventArgs) Handles TOOLREFRESH.Click
        Try
            fillgrid(" AND dbo.GREYJOBOUT.GJO_yearid=" & YearId & " order by dbo.GREYJOBOUT.GJO_no ")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub GreyJobOutDetails_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Try
    ...
    Private Sub CMBTYPE_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles CMBTYPE.Validated
        Try
            If CMBTYPE.Text <> "" And (ClientName = "SAFFRON" Or ClientName = "SAFFRONOFF") Then fillgrid(" AND JOTYPE_NAME = '" & CMBTYPE.Text.Trim & "'  AND dbo.GREYJOBOUT.GJO_yearid=" & YearId & " order by dbo.GREYJOBOUT.GJO_no ")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
