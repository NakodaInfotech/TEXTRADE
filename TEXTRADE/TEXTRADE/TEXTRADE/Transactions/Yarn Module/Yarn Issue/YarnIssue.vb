...
Public Class YarnIssue
    ...
    Sub CLEAR()
        ...
        tstxtbillno.Clear()
        CHKYARNRECD.CheckState = CheckState.Unchecked
            ...
            If EDIT = True Then
                ...
                If dttable.Rows.Count > 0 Then
                    ...
                    For Each dr As DataRow In dttable.Rows
                        ...
                        CHKYARNRECD.Checked = Convert.ToBoolean(dr("YARNRECD"))
                        TXTBEAMDESC.Text = dr("BEAMDESC")
        ...
        Try
            ...
            If DTYARN.Rows.Count > 0 Then
                For Each DTROW As DataRow In DTYARN.Rows
                    GRIDYARN.Rows.Add(0, DTROW("YARNQUALITY"), DTROW("MILLNAME"), DTROW("DESIGNNO"), DTROW("COLOR"), DTROW("LOTNO"), Format(Val(DTROW("BAGS")), "0"), Format(Val(DTROW("WT")), "0.00"), Format(Val(DTROW("CONES")), "0"), DTROW("LRNO"), Format(DTLRDATE.Value.Date, "dd/MM/yyyy"), DTROW(" BARCODE"), DTROW("FROMNO"), DTROW("FROMSRNO"), DTROW("FROMTYPE"))
                Next
                getsrno(GRIDYARN)
                total()
                GRIDYARN.FirstDisplayedScrollingRowIndex = GRIDYARN.RowCount - 1
...
LINE1:
    ...
    Private Sub cmbitemname_Validated(sender As Object, e As EventArgs) Handles CMBITEMNAME.Validated
        Try
            If CMBITEMNAME.Text.Trim <> "" And ClientName = "AKASHDEEP" Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.search(" ITEMMASTER_WARPDETAILS.ITEM_WARPSRNO AS SRNO, YARNQUALITYMASTER.YARN_NAME AS YARNQUALITY, ISNULL(COLORMASTER.COLOR_name, '') AS SHADE, ITEMMASTER_WARPDETAILS.ITEM_WARPENDS AS ENDS, ITEMMASTER_WARPDETAILS.ITEM_WARPWT AS WT, ITEMMASTER_WARPDETAILS.ITEM_WARPRATE AS RATE, ITEMMASTER_WARPDETAILS.ITEM_WARPAMOUNT AS AMOUNT ", "", " ITEMMASTER_WARPDETAILS INNER JOIN YARNQUALITYMASTER ON ITEMMASTER_WARPDETAILS.ITEM_WARPQUALITYID = YARNQUALITYMASTER.YARN_ID INNER JOIN ITEMMASTER ON ITEMMASTER_WARPDETAILS.ITEM_ID = ITEMMASTER.item_id LEFT OUTER JOIN COLORMASTER ON ITEMMASTER_WARPDETAILS.ITEM_WARPSHADEID = COLORMASTER.COLOR_id  ", " AND ITEMMASTER.ITEM_NAME = '" & CMBITEMNAME.Text.Trim & "' AND ITEMMASTER.ITEM_YEARID = " & YearId)
                If dt.Rows.Count > 0 Then
                    For Each ROW As DataRow In dt.Rows
                        GRIDYARN.Rows.Add(Val(ROW("SRNO")), ROW("YARNQUALITY"), "", "", ROW("SHADE"), "", 0, Val(ROW("WT")), 0, "", Format(DTLRDATE.Value.Date, "dd/MM/yyyy"))
                    Next
                End If
            End If
        Catch ex As Exception
        ...
        Try
            If TXTBARCODE.Text.Trim.Length > 0 Then
                ...
                If DT.Rows.Count > 0 Then
                    ...
                    GRIDYARN.Rows.Add(GRIDYARN.RowCount + 1, DT.Rows(0).Item("YARNQUALITY"), DT.Rows(0).Item("MILLNAME"), DT.Rows(0).Item("DESIGNNO"), DT.Rows(0).Item("COLOR"), 0, DT.Rows(0).Item("LOTNO"), DT.Rows(0).Item("BAGS"), DT.Rows(0).Item("WT"), DT.Rows(0).Item("CONES"), DT.Rows(0).Item("LRNO"), DTLRDATE.Value = Now.Date, DTLIFTDATE.Value = Now.Date, DT.Rows(0).Item("BARCODE"), DT.Rows(0).Item("FROMNO"), DT.Rows(0).Item("FROMSRNO"), DT.Rows(0).Item("FROMTYPE"))
                    total()
                    GRIDYARN.FirstDisplayedScrollingRowIndex = GRIDYARN.RowCount - 1
