...
Public Class YarnReturnPurchaseDetails
    ...
    Sub fillgrid(ByVal TEMPCONDITION)
        Try
            ...
            dt = objclsCMST.search(" ISNULL(YARNPURCHASERETURN.YARNRET_NO, 0) AS SRNO, ISNULL(YARNPURCHASERETURN.YARNRET_date, GETDATE()) AS DATE, ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN,ISNULL(YARNPURCHASERETURN.YARNRET_CHALLANNO, '') AS CHALLANNO, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(YARNPURCHASERETURN.YARNRET_TOTALBAGS, 0) AS TOTALBAGS, ISNULL(YARNPURCHASERETURN.YARNRET_TOTALWT, 0) AS TOTALWT, ISNULL(YARNPURCHASERETURN.YARNRET_TOTALCONES, 0) AS TOTALCONES, ISNULL(YARNPURCHASERETURN.YARNRET_remarks, '') AS REMARKS, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(MILLMASTER.MILL_NAME, '') AS MILL, ISNULL(DESIGNMASTER.DESIGN_NO, '') AS DESIGN, ISNULL(COLORMASTER.COLOR_name, '') AS COLOR, ISNULL(YARNPURCHASERETURN_DESC.YARNRET_LOTNO, '') AS LOTNO, ISNULL(YARNPURCHASERETURN_DESC.YARNRET_BAGS, 0) AS BAGS, ISNULL(YARNPURCHASERETURN_DESC.YARNRET_WT, 0) AS WT, ISNULL(YARNPURCHASERETURN_DESC.YARNRET_CONES, 0) AS CONES ", "", "YARNPURCHASERETURN INNER JOIN GODOWNMASTER ON YARNPURCHASERETURN.YARNRET_GODOWNID = GODOWNMASTER.GODOWN_id INNER JOIN LEDGERS ON YARNPURCHASERETURN.YARNRET_LEDGERID = LEDGERS.Acc_id INNER JOIN YARNPURCHASERETURN_DESC ON YARNPURCHASERETURN.YARNRET_NO = YARNPURCHASERETURN_DESC.YARNRET_NO AND YARNPURCHASERETURN.YARNRET_yearid = YARNPURCHASERETURN_DESC.YARNRET_YEARID INNER JOIN YARNQUALITYMASTER ON YARNPURCHASERETURN_DESC.YARNRET_YARNQUALITYID = YARNQUALITYMASTER.YARN_ID INNER JOIN MILLMASTER ON YARNPURCHASERETURN_DESC.YARNRET_MILLID = MILLMASTER.MILL_ID LEFT OUTER JOIN COLORMASTER ON YARNPURCHASERETURN_DESC.YARNRET_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN DESIGNMASTER ON YARNPURCHASERETURN_DESC.YARNRET_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON YARNPURCHASERETURN.YARNRET_transledgerid = TRANSLEDGERS.Acc_id", TEMPCONDITION)
