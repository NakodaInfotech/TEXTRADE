...
Public Class StoreStockRecoInDetails
    ...
    Sub FILLGRID()
        Try
            ...
            dt = OBJCMN.SEARCH(" STORESTOCKADJUSTMENT.SA_NO AS SANO, STORESTOCKADJUSTMENT_INDESC.SA_INGRIDSRNO AS INGRIDSRNO, STORESTOCKADJUSTMENT_INDESC.SA_INDESC AS INDESC, STORESTOCKADJUSTMENT_INDESC.SA_INQTY AS INQTY, UNITMASTER.unit_name AS INUNIT, STORESTOCKADJUSTMENT_INDESC.SA_INRATE AS INRATE, STOREITEMMASTER.STOREITEM_NAME AS INITEMNAME,  ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(STORESTOCKADJUSTMENT.SA_CHALLANNO, '')  AS CHALLANNO, STORESTOCKADJUSTMENT.SA_CHALLANDATE AS CHALLANDATE, ISNULL(STORESTOCKADJUSTMENT.SA_REMARKS, '') AS REMARKS ", "", " STORESTOCKADJUSTMENT INNER JOIN STORESTOCKADJUSTMENT_INDESC ON STORESTOCKADJUSTMENT.SA_NO = STORESTOCKADJUSTMENT_INDESC.SA_NO AND  STORESTOCKADJUSTMENT.SA_yearid = STORESTOCKADJUSTMENT_INDESC.SA_YEARID INNER JOIN UNITMASTER ON STORESTOCKADJUSTMENT_INDESC.SA_INUNITID = UNITMASTER.unit_id INNER JOIN STOREITEMMASTER ON STORESTOCKADJUSTMENT_INDESC.SA_INITEMID = STOREITEMMASTER.STOREITEM_ID INNER JOIN GODOWNMASTER ON STORESTOCKADJUSTMENT.SA_GODOWNID = GODOWNMASTER.GODOWN_id INNER JOIN LEDGERS ON STORESTOCKADJUSTMENT.SA_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON STORESTOCKADJUSTMENT.SA_TRANSID = TRANSLEDGERS.Acc_id  ", "  AND (STORESTOCKADJUSTMENT_INDESC.SA_yearid= " & YearId & ") ORDER BY dbo.STORESTOCKADJUSTMENT_INDESC.SA_NO")
