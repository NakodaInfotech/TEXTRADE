...
Public Class SalesEnquiry
    ...
    Dim TEMPROW, TEMPFOLLOWROW, TEMPBLANKETREPNO As Integer
    ...
    Private Sub cmdOK_Click(sender As Object, e As EventArgs) Handles cmdOK.Click
        Try
            ...
            Dim objclsPurord As New ClsSalesEnquiry()
    ...
    Sub fillgrid(MATCHING As String)
        ...
        Else
            ...
            cmbcolor.Focus()
        End If
        If ClientName = "SHREEVALLABH" Or ClientName = "RAJDEEP" Or ClientName = "KRISHNA" Or ClientName = "SIDDHGIRI" Then CMBDESIGN.Focus()

        If ClientName = "INDRAPUJAFABRICS" Or ClientName = "INDRAPUJAIMPEX" Or ClientName = "SUPRIYA" Or ClientName = "SMS" Or ClientName = "SONU" Or ClientName = "MNARESH" Or ClientName = "SIDDHGIRI" Then
            TXTRATE.Clear()
            cmbitemname.Focus()
        End If
    ...
    Sub getmax_SO_no()
        ...
        DTTABLE = getmax(" isnull(max(ENQ_no),0) + 1 ", "SALESENQUIRY", " AND ENQ_cmpid=" & CmpId & " and ENQ_locationid=" & Locationid & " and ENQ_yearid=" & YearId)
    ...
    Private Function errorvalid() As Boolean
        ...
        If Val(TXTSONO.Text.Trim) > 0 And EDIT = False And TXTSONO.ReadOnly = False Then
            DTTABLE = OBJCMN.SEARCH("  ISNULL(ENQ_no, 0) AS SONO ", "", " SALESENQUIRY ", "  AND SALESENQUIRY.ENQ_no=" & TXTSONO.Text.Trim & " AND SALESENQUIRY.ENQ_CMPID = " & CmpId & " AND SALESENQUIRY.ENQ_LOCATIONID = " & Locationid & " AND SALESENQUIRY.ENQ_YEARID = " & YearId)
            If DTTABLE.Rows.Count > 0 Then
                EP.SetError(TXTSONO, "Enquiry No Already Exist")
                bln = False
            End If
        End If
...
line1:
    ...
    Private Sub SalesEnquiry_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Dim DTROW() As DataRow
            DTROW = USERRIGHTS.Select("FormName = 'ENQUIRY'")
            ...
            If EDIT = True Then
                ...
                Dim objclsSO As New ClsSalesEnquiry()
                ...
                'FOLLOWUP GRID
                Dim OBJCMN As New ClsCommon
                dt_po = OBJCMN.SEARCH("ISNULL(SALESENQUIRY_FOLLOWUP.ENQ_NO, '') AS FOLLOWUPNO, ISNULL(SALESENQUIRY_FOLLOWUP.ENQ_SRNO, 0) AS SRNO, ISNULL(SALESENQUIRY_FOLLOWUP.ENQ_DATE, '') AS FOLLOWUPDATE, ISNULL(SALESENQUIRY_FOLLOWUP.ENQ_GIVENBY, '') AS GIVENBY,  ISNULL(SALESENQUIRY_FOLLOWUP.ENQ_GIVENTO, '') AS GIVENTO,  ISNULL(SALESENQUIRY_FOLLOWUP.ENQ_NEXTDATE, '') AS NEXTDATE, ISNULL(SALESENQUIRY_FOLLOWUP.ENQ_NARRATION, '') AS NARR", "", " SALESENQUIRY_FOLLOWUP ", " AND ENQ_NO = " & TEMPSONO & " AND ENQ_YEARID = " & YearId & " ORDER BY ENQ_SRNO")
                If dt_po.Rows.Count > 0 Then
                    For Each DTFOLLOW As DataRow In dt_po.Rows
                        GRIDFOLLOW.Rows.Add(DTFOLLOW("SRNO"), Format(Convert.ToDateTime(DTFOLLOW("FOLLOWUPDATE")).Date, "dd/MM/yyyy"), DTFOLLOW("GIVENBY"), DTFOLLOW("GIVENTO"), DTFOLLOW("NEXTDATE"), DTFOLLOW("NARR"))
                    Next
                End If
    ...
    Private Sub CMBPACKING_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles CMBPACKING.Validated
        Try
            If ClientName = "YASHVI" Or ClientName = "KOTHARI" Or ClientName = "KOTHARINEW" Or ClientName = "AVIS" Or ClientName = "KRISHNA" Or ClientName = "SUPEEMA" Or ClientName = "MAHAVIRPOLYCOT" Then
                If CMBPACKING.Text.Trim <> "" Then
                    'GET REGISTER , AGENCT AND TRANS
                    Dim OBJCMN As New ClsCommon
                    Dim DT As DataTable = OBJCMN.SEARCH(" ISNULL(LEDGERSTRANS.Acc_cmpname, '') AS TRANSNAME,ISNULL(CITYMASTER.CITY_NAME,'') AS CITYNAME, ISNULL(LEDGERS.ACC_MOBILE,'') AS MOBILENO, ISNULL(BILLTOLEDGERS.ACC_CMPNAME,'') AS BILLTONAME ", "", " LEDGERS LEFT OUTER JOIN LEDGERS AS LEDGERSTRANS ON LEDGERS.ACC_TRANSID = LEDGERSTRANS.Acc_id LEFT OUTER JOIN CITYMASTER ON LEDGERS.ACC_DELIVERYATID = CITY_ID LEFT OUTER JOIN LEDGERS AS BILLTOLEDGERS ON LEDGERS.ACC_BILLTOID = BILLTOLEDGERS.ACC_ID", " and LEDGERS.acc_cmpname = '" & CMBPACKING.Text.Trim & "'  and LEDGERS.acc_YEARid = " & YearId)
                    If DT.Rows.Count > 0 Then
                        If DT.Rows(0).Item("TRANSNAME") = "" Then
                            cmbtrans.Text = cmbtrans.Text.Trim
                        Else
                            If cmbname.Text.Trim <> CMBPACKING.Text.Trim Then cmbtrans.Text = DT.Rows(0).Item("TRANSNAME")
                        End If
    ...
    Private Sub SalesEnquiry_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub GRIDSO_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GRIDSO.CellClick
        Try
            ...
            If GRIDSO.RowCount > 0 Then
                If (ClientName = "MSANCHITKUMAR") And GRIDSO.Item(gitemname.Index, GRIDSO.CurrentRow.Index).Value <> "" And cmbname.Text.Trim <> "" Then
                    Dim OBJCMN As New ClsCommon
        ...
        Try
            If EDIT = True Then
                ...
                If tempMsg = vbYes Then
                    ...
                    Dim clspo As New ClsSalesEnquiry()
            ...
            If cmbname.Text.Trim <> "" Then
                ...
                If DT.Rows.Count > 0 Then
                    ...
                    If CMBPACKINGTYPE.Text.Trim = "" Then
                        ...
                        If ClientName <> "INDRAPUJA IMPEX" Then cmbcity.Text = DT.Rows(0).Item("CITYNAME")
                    End If
        ...
        Try
            If GRIDSO.CurrentRow.Index >= 0 And GRIDSO.Item(gsrno.Index, GRIDSO.CurrentRow.Index).Value <> Nothing Then
                ...
                If Convert.ToBoolean(GRIDSO.Rows(GRIDSO.CurrentRow.Index).Cells(GDONE.Index).Value) = True Then 'If row.Cells(16).Value <> "0" Then 
                    MsgBox("Item Locked. First Delete from Sale Enquiry")
                    Exit Sub
                End If
...
LINE1:
    ...
    Private Sub OpenToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OpenToolStripButton.Click
        Try
            ...
            Dim objpodtls As New SalesEnquiryDetails
...
line1:
    ...
    Private Sub SalesEnquiry_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
            If ClientName = "CC" Or ClientName = "C3" Or ClientName = "SHREEDEV" Then
            ...
            If ClientName = "MNARESH" Or ClientName = "MSANCHITKUMAR" Then LBLRATE.Visible = True
...
LINESINGLE:
    ...
    Private Sub TXTSONO_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTSONO.Validating
        Try
            ...
            If Val(TXTSONO.Text.Trim) <> 0 And EDIT = False Then
                Dim OBJCMN As New ClsCommon
                Dim dttable As DataTable = OBJCMN.SEARCH("  ISNULL(ENQ_no, 0) AS SONO ", "", " SALESENQUIRY ", "  AND SALESENQUIRY.ENQ_no=" & TXTSONO.Text.Trim & " AND SALESENQUIRY.ENQ_CMPID = " & CmpId & " AND SALESENQUIRY.ENQ_LOCATIONID = " & Locationid & " AND SALESENQUIRY.ENQ_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
                    MsgBox("Enquiry No Already Exist")
                    e.Cancel = True
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub cmbitemname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbitemname.Validated
        ...
        '        If (ClientName = "MAHAVIR" Or ClientName = "BARKHA" Or ClientName = "MAHAJAN" Or ClientName = "SHUBHI" Or ClientName = "SUBHLAXMI" Or ClientName = "SMS" Or ClientName = "RAJKRIPA" Or ClientName = "MAHAVIRPOLYCOT" Or ClientName = "SIDDHPOLYCOT" Or ClientName = "SIDDHGIRI") Then
        ...
        '            If DT.Rows.Count > 0 Then
        '                If ClientName <> "SIDDHGIRI" Then TXTCUT.Text = DT.Rows(0).Item("CUT")
        ...
        '                    CMBPER.Text = "Mtrs"
        '                End If
        '                If ClientName = "SIDDHGIRI" Then cmbqtyunit.Text = DT.Rows(0).Item("UNIT")
    ...
    Private Sub cmbname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbname.Validated
        Try
            If cmbname.Text.Trim <> "" And CMBPACKING.Text.Trim = "" And ClientName <> "INDRAPUJA IMPEX" Then CMBPACKING.Text = cmbname.Text.Trim
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub TXTBLANKETREPNO_Validated(sender As Object, e As EventArgs) Handles TXTBLANKETREPNO.Validated
        Try
            If Val(TXTBLANKETREPNO.Text.Trim) > 0 And EDIT = False Then
                If MsgBox("Wish to Copy Blanket Rep. No " & Val(TXTBLANKETREPNO.Text.Trim) & "?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then Exit Sub
                TEMPBLANKETREPNO = TXTBLANKETREPNO.Text.Trim
                ...
                Dim dt_po As DataTable = OBJCLSBLANKET.SELECTBLANKETREPRESENT()
...
LINE1:
    ...
    Private Sub TXTCHNO_Validating(sender As Object, e As CancelEventArgs) Handles TXTCMPSONO.Validating
        Try
            If (ClientName = "KRISHNA") And CMBCMPNAME.Text.Trim <> "" And Val(TXTCMPSONO.Text.Trim) > 0 And EDIT = False Then
                ...
                Dim ALPARAVAL As New ArrayList
                Dim OBJSO As New ClsSalesEnquiry()
    ...
    Private Sub TXTCUT_GotFocus(sender As Object, e As EventArgs) Handles TXTCUT.GotFocus
        TXTCUT.SelectAll()
    End Sub
    Private Sub TXTFNARR_Validating(sender As Object, e As CancelEventArgs) Handles TXTFNARR.Validating
        Try
            If DTFOLLOWDATE.Text.Trim <> "" And TXTFOLLOWUPGIVENBY.Text.Trim <> "" And TXTFOLLOWTO.Text.Trim <> "" Then
                FILLFOLLOWUP()
            Else
    ...
    Sub FILLFOLLOWUP()
        ...
        If GRIDFOLLOWDOUBLECLICK = False Then
            GRIDFOLLOW.Rows.Add(Val(TXTFSRNO.Text.Trim), DTFOLLOWDATE.Text, TXTFOLLOWUPGIVENBY.Text.Trim, TXTFOLLOWTO.Text.Trim, DTNEXTDATE.Text, TXTFNARR.Text.Trim)
            getsrno(GRIDFOLLOW)
        ...
        ElseIf GRIDFOLLOWDOUBLECLICK = True Then
            ...
            GRIDFOLLOW.Item(GFNAME.Index, TEMPFOLLOWROW).Value = TXTFOLLOWUPGIVENBY.Text.Trim
    ...
    Private Sub GRIDFOLLOW_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles GRIDFOLLOW.CellDoubleClick
        Try
            ...
            If e.RowIndex >= 0 And GRIDFOLLOW.Item(GFSRNO.Index, e.RowIndex).Value <> Nothing Then
                ...
                TXTFOLLOWUPGIVENBY.Text = GRIDFOLLOW.Item(GFNAME.Index, e.RowIndex).Value
    ...
    Private Sub GRIDFOLLOW_KeyDown(sender As Object, e As KeyEventArgs) Handles GRIDFOLLOW.KeyDown
        Try
    ...
    Private Sub DTFOLLOWDATE_Validating(sender As Object, e As CancelEventArgs) Handles DTFOLLOWDATE.Validating
        Try
    ...
    Private Sub DTNEXTDATE_Validating(sender As Object, e As CancelEventArgs) Handles DTNEXTDATE.Validating
        Try
