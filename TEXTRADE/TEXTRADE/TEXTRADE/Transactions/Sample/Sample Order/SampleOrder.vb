...
LINE1:
    ...
    Private Sub CMBPARTYNAME_Enter(sender As Object, e As EventArgs) Handles CMBPARTYNAME.Enter
        Try
            If ClientName = "VINTAGEINDIA" Then
                If CMBPARTYNAME.Text.Trim = "" Then FILLNAME(CMBPARTYNAME, EDIT, " AND (GROUPMASTER.GROUP_SECONDARY ='SUNDRY CREDITORS' OR GROUPMASTER.GROUP_SECONDARY ='SUNDRY DEBTORS') and ACC_TYPE = 'ACCOUNTS'")
            Else
                If CMBPARTYNAME.Text.Trim = "" Then FILLNAME(CMBPARTYNAME, EDIT, " and (GROUPMASTER.GROUP_SECONDARY = 'Sundry Debtors' OR (GROUPMASTER.GROUP_SECONDARY = 'Sundry Creditors' and LEDGERS.ACC_TYPE = 'AGENT'))")
            End If
        Catch ex As Exception
    ...
    Private Sub TXTSONO_Validated(sender As Object, e As EventArgs) Handles TXTSONO.Validated
        Try
            ...
            If Val(TXTSONO.Text.Trim) > 0 And EDIT = False Then
                ...
                Dim OBJCMN As New ClsCommon
                'THIS WAS OG CODE WITH RESPECT TO SHADE, NOW WE HAVE REMOVD SHADE
                ...
                Dim DT As DataTable = OBJCMN.SEARCH("LEDGERS.ACC_CMPNAME AS NAME, ISNULL(ITEMMASTER.item_name, '') AS ITEM, ISNULL(DESIGNMASTER.DESIGN_NO, '') AS DESIGN ", "", " SALEORDER INNER JOIN SALEORDER_DESC ON SALEORDER.so_no = SALEORDER_DESC.SO_NO AND SALEORDER.SO_YEARID = SALEORDER_DESC.SO_YEARID INNER JOIN LEDGERS ON SALEORDER.SO_LEDGERID = LEDGERS.ACC_ID LEFT OUTER JOIN DESIGNMASTER ON DESIGNMASTER.DESIGN_id = SALEORDER_DESC.SO_DESIGNID LEFT OUTER JOIN ITEMMASTER AS ITEMMASTER ON SALEORDER_DESC.SO_ITEMID = ITEMMASTER.item_id ", " AND SALEORDER.SO_NO = " & Val(TXTSONO.Text.Trim) & " AND SALEORDER.SO_YEARID = " & YearId & " GROUP BY LEDGERS.ACC_CMPNAME, ISNULL(ITEMMASTER.item_name, ''), ISNULL(DESIGNMASTER.DESIGN_NO, '')")
                If DT.Rows.Count > 0 Then
                    CMBPARTYNAME.Text = DT.Rows(0).Item("NAME")
                    For Each DR As DataRow In DT.Rows
                        'GRIDSO.Rows.Add(0, CMBSAMPLETYPE.Text.Trim, DR("ITEM"), "", DR("DESIGN"), DR("COLOR"), 1, "", 0, 0)
                        GRIDSO.Rows.Add(0, CMBSAMPLETYPE.Text.Trim, DR("ITEM"), "", DR("DESIGN"), "", 1, "", 0, 0)
                    Next
                    GETSRNO(GRIDSO)
                    TOTAL()
                Else
