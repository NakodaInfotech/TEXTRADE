...
Public Class InHouseChecking
    ...
    Function errorvalid() As Boolean
        Try
            ...
            For Each row As DataGridViewRow In GRIDCHECKING.Rows
                If Val(row.Cells(GMTRS.Index).Value) = 0 And ClientName <> "PARAS" And ClientName <> "RAJKRIPA" And ClientName <> "SNCM" And ClientName <> "MILUXE" And ClientName <> "ROVIRO" Then
        ...
        Try
            ...
            If DT.Rows.Count > 0 Then
                ...
                ElseIf TXTTYPE.Text = "JOBIN" Then
                    ...
                    DTTABLE = OBJCMN.SEARCH(" JOBIN_DESC.JI_MTRS AS GREYMTRS, JOBIN_DESC.JI_MTRS AS RECDMTRS, ITEMMASTER.item_name AS ITEMNAME, ISNULL(QUALITYMASTER.QUALITY_name, '') AS QUALITY, ISNULL(DESIGNMASTER.DESIGN_NO, '') AS DESIGN, ISNULL(COLORMASTER.COLOR_name, '') AS COLOR, JOBIN_DESC.JI_GRIDSRNO AS FROMSRNO, ISNULL(UNITMASTER.unit_abbr, '') AS UNIT, ISNULL(JOBIN.JI_LOTNO, '') AS LOTNO, ISNULL(JOBIN_DESC.JI_RATE, 0) AS RATE, (CASE WHEN VERSION.VERSION_CLIENTNAME = 'VINTAGEINDIA' THEN ISNULL(JOBIN_DESC.JI_PCSNO,0) ELSE  ISNULL(JOBIN_DESC.JI_BALENO, '') END) AS BALENO, ISNULL(JOBIN_DESC.JI_WT, '') AS RECDWT, ISNULL(ITEMMASTER.ITEM_PERCENT, 0) AS WTPER, ISNULL(JOBIN.JI_CHALLANNO, '') AS CHALLANNO, JOBIN.JI_CHALLANDATE AS CHALLANDATE ", "", " JOBIN_DESC INNER JOIN ITEMMASTER ON JOBIN_DESC.JI_ITEMID = ITEMMASTER.item_id INNER JOIN JOBIN ON JOBIN_DESC.JI_NO = JOBIN.JI_no AND JOBIN_DESC.JI_YEARID = JOBIN.JI_yearid LEFT OUTER JOIN COLORMASTER ON JOBIN_DESC.JI_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN DESIGNMASTER ON JOBIN_DESC.JI_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN QUALITYMASTER ON JOBIN_DESC.JI_QUALITYID = QUALITYMASTER.QUALITY_id LEFT OUTER JOIN UNITMASTER ON JOBIN_DESC.JI_QTYUNITID = UNITMASTER.unit_id CROSS JOIN VERSION ", " AND JOBIN.JI_NO = " & Val(DT.Rows(0).Item("MATRECNO")) & " AND JOBIN.JI_YEARID = " & YearId)
