        ...
        Try
            ...
            ElseIf FETCHITEMWISESHADE = True Then
                ...
                If FRMSTRING = "PO" Then
                    DT = OBJCMN.SEARCH(" ISNULL(C.COLOR_name,'') AS COLOR,  SUM(ISNULL(B.PCS, 0)) AS TOTALPCS, SUM(ISNULL(B.MTRS, 0)) AS TOTALMTRS, 0.00 AS CUT, 0 AS ORDERPCS, 0.00 AS ORDERMTRS , ISNULL(( SELECT SUM(PO_MTRS-PO_RECDMTRS) FROM ALLPURCHASEORDER_DESC LEFT OUTER JOIN ITEMMASTER ON ITEM_id = PO_ITEMID LEFT OUTER JOIN COLORMASTER ON COLOR_id = PO_COLORID WHERE PO_CLOSED = 'FALSE' AND PO_MTRS-PO_RECDMTRS > 0 AND (ITEMMASTER.ITEM_NAME = I.ITEM_NAME And COLOR_name = C.COLOR_name And PO_YEARID = I.ITEM_YEARID)),0) AS PENDINGQTY", "", " ITEMMASTER AS I INNER JOIN  ITEMMASTER_COLOR ON I.ITEM_id = ITEMMASTER_COLOR.ITEM_ID INNER JOIN  COLORMASTER AS C ON ITEMMASTER_COLOR.ITEM_COLORID = C.COLOR_id LEFT OUTER JOIN BARCODESTOCK AS B ON C.COLOR_name = B.COLOR AND I.ITEM_NAME = B.ITEMNAME AND I.ITEM_YEARID = B.YEARID AND B.UNIT IN (SELECT UNIT_ABBR FROM DEFAULTSTOCKUNIT) ", WHERECLAUSE & GROUPBY)
                Else
