    ...
    Private Sub cmbname_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbname.Validating
        Try
            If cmbname.Text.Trim <> "" Then
                ...
                Dim DT As DataTable = OBJCMN.SEARCH("ISNULL(LEDGERS.ACC_CDPER, 0) AS CDPER, ISNULL(LEDGERS.Acc_crdays, 0) AS CRDAYS, ISNULL(LEDGERS.ACC_GSTIN, '') AS GSTIN, ISNULL(LEDGERS.Acc_mobile, '') AS MOBILENO,  ISNULL(PACKINGTYPEMASTER.PACKINGTYPE_name, '') AS PACKINGTYPE, ISNULL(SALESMANMASTER.SALESMAN_NAME, '') AS SALESMAN, ISNULL(CITYMASTER.city_name, '') AS CITYNAME,   ISNULL(LEDGERS.ACC_WARNING, '') AS WARNINGTEXT, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANS1", "", "  LEDGERS LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON LEDGERS.ACC_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON LEDGERS.ACC_AGENTID = AGENTLEDGERS.Acc_id LEFT OUTER JOIN PACKINGTYPEMASTER ON LEDGERS.ACC_PACKINGTYPEID = PACKINGTYPEMASTER.PACKINGTYPE_id LEFT OUTER JOIN SALESMANMASTER ON LEDGERS.ACC_SALESMANID = SALESMANMASTER.SALESMAN_ID LEFT OUTER JOIN CITYMASTER ON LEDGERS.ACC_DELIVERYATID = CITYMASTER.city_id ", " AND LEDGERS.ACC_CMPNAME = '" & cmbname.Text.Trim & "'  AND LEDGERS.ACC_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
...
LINE1:
    ...
    Private Sub ToolStripButton3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PrintToolStripButton.Click
        Try
            If EDIT = True Then
                PRINTREPORT()
                If ClientName = "SOFTAS" Or ClientName = "MYCOT" Then PRINTBARCODE()
            End If
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
            ...
            For Each ROW As DataGridViewRow In GRIDSO.Rows
                ...
                For J As Integer = 1 To Val(TXTCOPIES.Text.Trim)
                    ...
                    ElseIf ClientName = "MYCOT" Then
                        ...
                        Dim TEMPHEADERNAME As String = ""
                        DT = OBJCMN.SEARCH(" ISNULL(PACKINGTYPEMASTER.PACKINGTYPE_name,'') AS PACKINGTYPE ", "", " LEDGERS INNER JOIN PACKINGTYPEMASTER ON LEDGERS.ACC_PACKINGTYPEID = PACKINGTYPEMASTER.PACKINGTYPE_id AND LEDGERS.Acc_yearid = PACKINGTYPEMASTER.PACKINGTYPE_yearid", " AND  LEDGERS.Acc_cmpname = '" & cmbname.Text.Trim & "' AND LEDGERS.Acc_yearid = " & YearId)
                        If DT.Rows.Count > 0 Then TEMPHEADERNAME = DT.Rows(0).Item("PACKINGTYPE")
...
CLS
CODEPAGE 1252
TEXT 342,373,""ROMAN.TTF"",180,1,20,""" & TEMPHEADERNAME & """
BAR 11,292, 356, 3
TEXT 368,266,""ROMAN.TTF"",180,1,16,""" & ROW.Cells(GPARTYPONO.Index).Value & """
QRCODE 132,197,L,5,A,180,M2,S7,""TEXTRADE""
TEXT 368,125,""ROMAN.TTF"",180,1,12,""WIDTH""
TEXT 268,125,""ROMAN.TTF"",180,1,12,"":""
TEXT 248,125,""ROMAN.TTF"",180,1,12,""" & TEMPWIDTH & """
TEXT 368,195,""ROMAN.TTF"",180,1,16,""" & ROW.Cells(gdesc.Index).Value & """
TEXT 349,50,""ROMAN.TTF"",180,1,8,""EXCLUSIVE PREMIUM FABRICS""
BAR 11,71, 356, 3
PRINT 1,1")
                        oWrite.Dispose()
        ...
        Try
            If TXTBARCODE.Text.Trim.Length > 0 Then
                ...
                If DT.Rows.Count > 0 Then
                    ...
                    If ClientName <> "KENCOT" And ClientName <> "SUPRIYA" Then TXTRATE.Text = Val(DT.Rows(0).Item("RATE"))
...
INSERTDATA:
                    ...
                    If ClientName = "MAHAVIRPOLYCOT" Or ClientName = "YASHVI" Or ClientName = "SUPRIYA" Or ClientName = "AVIS" Or ClientName = "KENCOT" Or ClientName = "MYCOT" Then
