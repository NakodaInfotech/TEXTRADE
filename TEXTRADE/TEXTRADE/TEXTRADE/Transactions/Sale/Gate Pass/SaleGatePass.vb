        ...
        Try
            ...
            If DTTABLE.Rows.Count > 0 Then
                ...
                ElseIf DTTABLE.Rows(0).Item("FROMTYPE") = "GREYGDN" Then
                    If ClientName = "SNCM" Then
                        DT1 = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(DELIVERYLEDGERS.Acc_cmpname, '')  AS DELIVERYNAME, ISNULL(TOCITYMASTER.city_name, '') AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, GREYGDN.GDN_NO AS GDNNO, GREYGDN.GDN_TOTALPCS AS PCS, SUM(GREYGDN.GDN_TOTALMTRS) AS MTRS, ISNULL(GREYGDN.GDN_TOTALBALES, 0) AS NOOFBALES, '' AS PARTYPONO, 'GREYGDN' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(GREYGDN.GDN_remarks AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO , ISNULL(GREYGDN_DESC.GDN_BARCODE,'') AS BARCODE , ISNULL(UNITMASTER.unit_abbr,'') AS UNIT  ", "", "  GREYGDN INNER JOIN LEDGERS ON GREYGDN.GDN_ledgerid = LEDGERS.Acc_id INNER JOIN GREYGDN_DESC ON GREYGDN.GDN_NO = GREYGDN_DESC.GDN_NO AND GREYGDN.GDN_YEARID = GREYGDN_DESC.GDN_YEARID LEFT OUTER JOIN UNITMASTER ON GREYGDN_DESC.GDN_QUALITYID = UNITMASTER.unit_id LEFT OUTER JOIN CITYMASTER AS TOCITYMASTER ON GREYGDN.GDN_CITYid= TOCITYMASTER.city_id LEFT OUTER JOIN LEDGERS AS DELIVERYLEDGERS ON GREYGDN.GDN_DISPATCHTOID = DELIVERYLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GREYGDN.GDN_transid = TRANSLEDGERS.Acc_id    ", "  and GREYGDN.GDN_NO IN(" & TEMPCHALLANNO & ") and ISNULL(GREYGDN.GDN_GPDONE,0) = 0 AND GREYGDN.GDN_YEARID = " & YearId & "  GROUP BY GREYGDN.GDN_NO, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(DELIVERYLEDGERS.Acc_cmpname, ''), ISNULL(TOCITYMASTER.city_name, ''), GREYGDN.GDN_TOTALBALES, GREYGDN.GDN_TOTALPCS, ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(GREYGDN.GDN_remarks AS VARCHAR(1000)), ISNULL(GREYGDN_DESC.GDN_BARCODE,'') , ISNULL(UNITMASTER.unit_abbr,'') ")
                    Else
                        DT1 = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(DELIVERYLEDGERS.Acc_cmpname, '')  AS DELIVERYNAME, ISNULL(TOCITYMASTER.city_name, '') AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, GREYGDN.GDN_NO AS GDNNO, GREYGDN.GDN_TOTALPCS AS PCS, SUM(GREYGDN.GDN_TOTALMTRS) AS MTRS, ISNULL(GREYGDN.GDN_TOTALBALES, 0) AS NOOFBALES, '' AS PARTYPONO, 'GREYGDN' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(GREYGDN.GDN_remarks AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO    ", "", " GREYGDN INNER JOIN LEDGERS ON GREYGDN.GDN_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN CITYMASTER AS TOCITYMASTER ON GREYGDN.GDN_CITYid = TOCITYMASTER.city_id LEFT OUTER JOIN LEDGERS AS DELIVERYLEDGERS ON GREYGDN.GDN_DISPATCHTOID = DELIVERYLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GREYGDN.GDN_transid = TRANSLEDGERS.Acc_id    ", "  and GREYGDN.GDN_NO IN(" & TEMPCHALLANNO & ") and ISNULL(GREYGDN.GDN_GPDONE,0) = 0 AND GREYGDN.GDN_YEARID = " & YearId & "  GROUP BY GREYGDN.GDN_NO, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(DELIVERYLEDGERS.Acc_cmpname, ''), ISNULL(TOCITYMASTER.city_name, ''), GREYGDN.GDN_TOTALBALES, GREYGDN.GDN_TOTALPCS, ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(GREYGDN.GDN_remarks AS VARCHAR(1000)) ")
                    End If

                ElseIf DTTABLE.Rows(0).Item("FROMTYPE") = "GREYISS" Then
                    If ClientName = "SNCM" Then
                        DT1 = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ''  AS DELIVERYNAME, '' AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, GREYISSUEPROCESS.GREYISS_NO AS GDNNO, '' AS PCS, SUM(GREYISSUEPROCESS.GREYISS_TOTALMTRS) AS MTRS, ISNULL(GREYISSUEPROCESS.GREYISS_TOTALBALES, 0) AS NOOFBALES, '' AS PARTYPONO, 'GREYISS' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(GREYISSUEPROCESS.GREYISS_REMARKS AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO , ISNULL(GREYISSUEPROCESS_DESC.GREYISS_BARCODE,'') AS BARCODE , ISNULL(UNITMASTER.unit_abbr,'') AS UNIT ", "", "  GREYISSUEPROCESS INNER JOIN LEDGERS ON GREYISSUEPROCESS.GREYISS_LEDGERID = LEDGERS.Acc_id INNER JOIN GREYISSUEPROCESS_DESC ON GREYISSUEPROCESS.GREYISS_NO = GREYISSUEPROCESS_DESC.GREYISS_NO AND GREYISSUEPROCESS.GREYISS_YEARID = GREYISSUEPROCESS_DESC.GREYISS_YEARID LEFT OUTER JOIN UNITMASTER ON GREYISSUEPROCESS_DESC.GREYISS_QUALITYID = UNITMASTER.unit_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GREYISSUEPROCESS.GREYISS_TRANSID = TRANSLEDGERS.Acc_id ", "  and GREYISSUEPROCESS.GREYISS_NO IN(" & TEMPCHALLANNO & ") and ISNULL(GREYISSUEPROCESS.GREYISS_GPDONE,0) = 0 AND GREYISSUEPROCESS.GREYISS_YEARID = " & YearId & " GROUP BY GREYISSUEPROCESS.GREYISS_NO, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), GREYISSUEPROCESS.GREYISS_TOTALBALES, ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(GREYISSUEPROCESS.GREYISS_REMARKS AS VARCHAR(1000)), ISNULL(GREYISSUEPROCESS_DESC.GREYISS_BARCODE,'') , ISNULL(UNITMASTER.unit_abbr,'') ")
                    Else
                        DT1 = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME,  ''  AS DELIVERYNAME,  '' AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, GREYISSUEPROCESS.GREYISS_NO AS GDNNO, '' AS PCS, SUM(GREYISSUEPROCESS.GREYISS_TOTALMTRS) AS MTRS, ISNULL(GREYISSUEPROCESS.GREYISS_TOTALBALES, 0) AS NOOFBALES, '' AS PARTYPONO, 'GREYISS' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(GREYISSUEPROCESS.GREYISS_REMARKS AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO ", "", " GREYISSUEPROCESS INNER JOIN LEDGERS ON GREYISSUEPROCESS.GREYISS_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GREYISSUEPROCESS.GREYISS_TRANSID = TRANSLEDGERS.Acc_id ", "  and GREYISSUEPROCESS.GREYISS_NO IN(" & TEMPCHALLANNO & ") and ISNULL(GREYISSUEPROCESS.GREYISS_GPDONE,0) = 0 AND GREYISSUEPROCESS.GREYISS_YEARID = " & YearId & " GROUP BY GREYISSUEPROCESS.GREYISS_NO, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), GREYISSUEPROCESS.GREYISS_TOTALBALES, ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(GREYISSUEPROCESS.GREYISS_REMARKS AS VARCHAR(1000)) ")
                    End If

                ElseIf DTTABLE.Rows(0).Item("FROMTYPE") = "OPENINGGREYGDN" Then
                    If ClientName = "SNCM" Then
                        DT1 = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(DELIVERYLEDGERS.Acc_cmpname, '')  AS DELIVERYNAME, ISNULL(TOCITYMASTER.city_name, '') AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, OPENINGGREYGDN.OPENINGGDN_NO AS GDNNO, OPENINGGREYGDN.OPENINGGDN_TOTALPCS AS PCS, SUM(OPENINGGREYGDN.OPENINGGDN_TOTALMTRS) AS MTRS, ISNULL(OPENINGGREYGDN.OPENINGGDN_TOTALBALES, 0) AS NOOFBALES, '' AS PARTYPONO, 'OPENINGGREYGDN' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(OPENINGGREYGDN.OPENINGGDN_remarks AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO , ISNULL(OPENINGGREYGDN_DESC.OPENINGGDN_BARCODE,'') AS BARCODE , ISNULL(UNITMASTER.unit_abbr,'') AS UNIT ", "", "  OPENINGGREYGDN INNER JOIN LEDGERS ON OPENINGGREYGDN.OPENINGGDN_ledgerid = LEDGERS.Acc_id INNER JOIN OPENINGGREYGDN_DESC ON OPENINGGREYGDN.OPENINGGDN_NO = OPENINGGREYGDN_DESC.OPENINGGDN_NO AND OPENINGGREYGDN.OPENINGGDN_YEARID = OPENINGGREYGDN_DESC.OPENINGGDN_YEARID LEFT OUTER JOIN UNITMASTER ON OPENINGGREYGDN_DESC.OPENINGGDN_QUALITYID = UNITMASTER.unit_id LEFT OUTER JOIN CITYMASTER AS TOCITYMASTER ON OPENINGGREYGDN.OPENINGGDN_CITYid= TOCITYMASTER.city_id LEFT OUTER JOIN LEDGERS AS DELIVERYLEDGERS ON OPENINGGREYGDN.OPENINGGDN_DISPATCHTOID = DELIVERYLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON OPENINGGREYGDN.OPENINGGDN_transid = TRANSLEDGERS.Acc_id ", "  and OPENINGGREYGDN.OPENINGGDN_NO IN(" & TEMPCHALLANNO & ") and ISNULL(OPENINGGREYGDN.OPENINGGDN_GPDONE,0) = 0 AND OPENINGGREYGDN.OPENINGGDN_YEARID = " & YearId & "  GROUP BY OPENINGGREYGDN.OPENINGGDN_NO, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(DELIVERYLEDGERS.Acc_cmpname, ''), ISNULL(TOCITYMASTER.city_name, ''), OPENINGGREYGDN.OPENINGGDN_TOTALBALES, OPENINGGREYGDN.OPENINGGDN_TOTALPCS, ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(OPENINGGREYGDN.OPENINGGDN_remarks AS VARCHAR(1000)), ISNULL(OPENINGGREYGDN_DESC.OPENINGGDN_BARCODE,'') , ISNULL(UNITMASTER.unit_abbr,'') ")
                    Else
                        DT1 = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(DELIVERYLEDGERS.Acc_cmpname, '')  AS DELIVERYNAME, ISNULL(TOCITYMASTER.city_name, '') AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, OPENINGGREYGDN.OPENINGGDN_NO AS GDNNO, OPENINGGREYGDN.OPENINGGDN_TOTALPCS AS PCS, SUM(OPENINGGREYGDN.OPENINGGDN_TOTALMTRS) AS MTRS, ISNULL(OPENINGGREYGDN.OPENINGGDN_TOTALBALES, 0) AS NOOFBALES, '' AS PARTYPONO, 'OPENINGGREYGDN' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(OPENINGGREYGDN.OPENINGGDN_remarks AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO  ", "", " OPENINGGREYGDN INNER JOIN LEDGERS ON OPENINGGREYGDN.OPENINGGDN_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN CITYMASTER AS TOCITYMASTER ON OPENINGGREYGDN.OPENINGGDN_CITYid = TOCITYMASTER.city_id LEFT OUTER JOIN LEDGERS AS DELIVERYLEDGERS ON OPENINGGREYGDN.OPENINGGDN_DISPATCHTOID = DELIVERYLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON OPENINGGREYGDN.OPENINGGDN_transid = TRANSLEDGERS.Acc_id ", "  and OPENINGGREYGDN.OPENINGGDN_NO IN(" & TEMPCHALLANNO & ") and ISNULL(OPENINGGREYGDN.OPENINGGDN_GPDONE,0) = 0 AND OPENINGGREYGDN.OPENINGGDN_YEARID = " & YearId & "  GROUP BY OPENINGGREYGDN.OPENINGGDN_NO, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(DELIVERYLEDGERS.Acc_cmpname, ''), ISNULL(TOCITYMASTER.city_name, ''), OPENINGGREYGDN.OPENINGGDN_TOTALBALES, OPENINGGREYGDN.OPENINGGDN_TOTALPCS, ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(OPENINGGREYGDN.OPENINGGDN_remarks AS VARCHAR(1000)) ")
                    End If

                ElseIf DTTABLE.Rows(0).Item("FROMTYPE") = "GREYJOBOUT" Then
                    If ClientName = "SNCM" Then
                        DT1 = OBJCMN.SEARCH("  LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(DELIVERYLEDGERS.Acc_cmpname, '')  AS DELIVERYNAME, ISNULL(TOCITYMASTER.city_name, '') AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, GREYJOBOUT.GJO_no AS GDNNO, GREYJOBOUT.GJO_TOTALPCS AS PCS, SUM(GREYJOBOUT.GJO_TOTALMTRS) AS MTRS, ISNULL(GREYJOBOUT.GJO_BALENUMBER, 0) AS NOOFBALES, '' AS PARTYPONO, 'GREYJOBOUT' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(GREYJOBOUT.GJO_remarks AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO , ISNULL(GREYJOBOUT_DESC.GJO_BARCODE,'') AS BARCODE , ISNULL(UNITMASTER.unit_abbr,'') AS UNIT ", "", "   GREYJOBOUT INNER JOIN LEDGERS ON GREYJOBOUT.GJO_ledgerid = LEDGERS.Acc_id INNER JOIN GREYJOBOUT_DESC ON GREYJOBOUT.GJO_no = GREYJOBOUT_DESC.GJO_NO AND GREYJOBOUT.GJO_yearid = GREYJOBOUT_DESC.GJO_YEARID LEFT OUTER JOIN  UNITMASTER ON GREYJOBOUT_DESC.GJO_UNITID = UNITMASTER.unit_id LEFT OUTER JOIN CITYMASTER AS TOCITYMASTER ON GREYJOBOUT.GJO_TOCITYID = TOCITYMASTER.city_id LEFT OUTER JOIN LEDGERS AS DELIVERYLEDGERS ON GREYJOBOUT.GJO_PACKINGID = DELIVERYLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GREYJOBOUT.GJO_transledgerid = TRANSLEDGERS.Acc_id  ", "  and GREYJOBOUT.GJO_no IN(" & TEMPCHALLANNO & ") and ISNULL(GREYJOBOUT.GJO_GPDONE,0) = 0 AND GREYJOBOUT.GJO_yearid = " & YearId & " GROUP BY GREYJOBOUT.GJO_no, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(DELIVERYLEDGERS.Acc_cmpname, ''), ISNULL(TOCITYMASTER.city_name, ''), GREYJOBOUT.GJO_BALENUMBER, GREYJOBOUT.GJO_TOTALPCS, ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(GREYJOBOUT.GJO_remarks AS VARCHAR(1000)), ISNULL(GREYJOBOUT_DESC.GJO_BARCODE,''), ISNULL(UNITMASTER.unit_abbr,'') ")
                    Else
                        DT1 = OBJCMN.SEARCH("  LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(DELIVERYLEDGERS.Acc_cmpname, '')  AS DELIVERYNAME, ISNULL(TOCITYMASTER.city_name, '') AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, GREYJOBOUT.GJO_no AS GDNNO, GREYJOBOUT.GJO_TOTALPCS AS PCS, SUM(GREYJOBOUT.GJO_TOTALMTRS) AS MTRS, ISNULL(GREYJOBOUT.GJO_BALENUMBER, 0) AS NOOFBALES, '' AS PARTYPONO, 'GREYJOBOUT' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(GREYJOBOUT.GJO_remarks AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO  ", "", " GREYJOBOUT INNER JOIN LEDGERS ON GREYJOBOUT.GJO_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN CITYMASTER AS TOCITYMASTER ON GREYJOBOUT.GJO_TOCITYID = TOCITYMASTER.city_id LEFT OUTER JOIN LEDGERS AS DELIVERYLEDGERS ON GREYJOBOUT.GJO_PACKINGID = DELIVERYLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GREYJOBOUT.GJO_transledgerid = TRANSLEDGERS.Acc_id  ", "  and GREYJOBOUT.GJO_no IN(" & TEMPCHALLANNO & ") and ISNULL(GREYJOBOUT.GJO_GPDONE,0) = 0 AND GREYJOBOUT.GJO_yearid = " & YearId & " GROUP BY GREYJOBOUT.GJO_no, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(DELIVERYLEDGERS.Acc_cmpname, ''), ISNULL(TOCITYMASTER.city_name, ''), GREYJOBOUT.GJO_BALENUMBER, GREYJOBOUT.GJO_TOTALPCS, ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(GREYJOBOUT.GJO_remarks AS VARCHAR(1000))")
                    End If

                ElseIf DTTABLE.Rows(0).Item("FROMTYPE") = "YARNCHALLAN" Then
                    If ClientName = "SNCM" Then
                        DT1 = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ''  AS DELIVERYNAME, '' AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, YARNCHALLAN.YARN_NO AS GDNNO, '' AS PCS, '' AS MTRS,  0 AS NOOFBALES, '' AS PARTYPONO, 'YARNCHALLAN' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(YARNCHALLAN.YARN_REMARKS AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO , '' AS BARCODE , ISNULL(UNITMASTER.unit_abbr,'') AS UNIT ", "", "  YARNCHALLAN INNER JOIN LEDGERS ON YARNCHALLAN.YARN_LEDGERID = LEDGERS.Acc_id INNER JOIN YARNCHALLAN_DESC ON YARNCHALLAN.YARN_NO = YARNCHALLAN_DESC.YARN_NO AND YARNCHALLAN.YARN_YEARID = YARNCHALLAN_DESC.YARN_YEARID LEFT OUTER JOIN UNITMASTER ON YARNCHALLAN_DESC.YARN_YARNQUALITYID = UNITMASTER.unit_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON YARNCHALLAN.YARN_TRANSLEDGERID = TRANSLEDGERS.Acc_id ", "  and YARNCHALLAN.YARN_NO IN(" & TEMPCHALLANNO & ") and ISNULL(YARNCHALLAN.YARN_GPDONE,0) = 0 AND YARNCHALLAN.YARN_YEARID = " & YearId & "  GROUP BY YARNCHALLAN.YARN_NO, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(YARNCHALLAN.YARN_REMARKS AS VARCHAR(1000)), ISNULL(UNITMASTER.unit_abbr,'') ")
                    Else
                        DT1 = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, '' AS AGENT, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME,  ''  AS DELIVERYNAME,  '' AS TOCITY, '' AS ITEMNAME, '' AS QUALITY, '' AS DESIGNNO, '' AS COLOR, YARNCHALLAN.YARN_NO AS GDNNO, ISNULL(YARNCHALLAN.YARN_TOTALQTY,0) AS PCS, SUM(ISNULL(YARNCHALLAN.YARN_TOTALWT,0)) AS MTRS, ISNULL(YARNCHALLAN.YARN_NOOFBALES, 0) AS NOOFBALES, '' AS PARTYPONO, 'YARNCHALLAN' AS FROMTYPE, ISNULL(TRANSLEDGERS.ACC_RD, 0) AS TRANSRATE, CAST(YARNCHALLAN.YARN_REMARKS AS VARCHAR(1000)) AS REMARKS, '' as TRANSREFNO ", "", " YARNCHALLAN INNER JOIN LEDGERS ON YARNCHALLAN.YARN_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON YARNCHALLAN.YARN_TRANSLEDGERID = TRANSLEDGERS.Acc_id ", "  and YARNCHALLAN.YARN_NO IN(" & TEMPCHALLANNO & ") and ISNULL(YARNCHALLAN.YARN_GPDONE,0) = 0 AND YARNCHALLAN.YARN_YEARID = " & YearId & "  GROUP BY YARNCHALLAN.YARN_NO, LEDGERS.Acc_cmpname, ISNULL(TRANSLEDGERS.Acc_cmpname, ''),ISNULL(TRANSLEDGERS.ACC_RD, 0), CAST(YARNCHALLAN.YARN_REMARKS AS VARCHAR(1000)),ISNULL(YARNCHALLAN.YARN_TOTALQTY, 0), ISNULL(YARNCHALLAN.YARN_NOOFBALES, 0) ")
                    End If
                End If
