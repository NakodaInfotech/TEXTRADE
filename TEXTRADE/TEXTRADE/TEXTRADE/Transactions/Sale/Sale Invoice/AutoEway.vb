...
Public Class AutoEway
    ...
    '            If View.GetRowCellDisplayText(e.RowHandle, View.Columns("CHECKED")) = 1 Then
    '                e.Appearance.Font = New System.Drawing.Font("CALIBRI", 9.0F, System.Drawing.FontStyle.Bold)
    '                e.Appearance.BackColor = Color.LightGreen
    '            ElseIf View.GetRowCellDisplayText(e.RowHandle, View.Columns("DISPUTED")) = 1 Then
    '                e.Appearance.Font = New System.Drawing.Font("CALIBRI", 9.0F, System.Drawing.FontStyle.Bold)
    ...
    Private Sub CMDGENERERATEEWB_Click(sender As Object, e As EventArgs) Handles CMDGENERERATEEWB.Click
        Try
            ...
            'Dim GeneratedInvoices As New List(Of Integer)
        ...
        CType(gridbilldetails.MainView, DevExpress.XtraGrid.Views.Grid.GridView)
            ...
            For I As Integer = 0 To Val(gridbill.RowCount - 1)
                ...
                If ROW("CHK") = True Then
                    Dim BillNo As Integer = Val(ROW("SRNO"))
                    Dim REGISTER As String = cmbregister.Text.Trim

                    'Load invoice in master form
                    InvoiceMaster.LoadInvoice(BillNo, REGISTER)

                    'Generate EWB
                    InvoiceMaster.AutoEway = True

                    InvoiceMaster.TOOLEWB_Click(sender, e)


                    InvoiceMaster.cmdOK_Click(sender, e)
            ...
            SendWhatsAppForBatch(SelectedRows, True)
    ...
    Private Sub SendWhatsAppForBatch(rows As List(Of DataRow), Optional ByVal WHATSAPP As Boolean = False)

        Dim OBJCMN As New ClsCommon
        Dim ALATTACHMENT As New ArrayList
        Dim FILENAME As New ArrayList
        For Each ROW As DataRow In rows
            Dim invNo As Integer = Val(ROW("SRNO"))
            ...
            Dim DT As DataTable = OBJCMN.SEARCH(" ISNULL(TOKENNO, '') AS TOKENNO, ISNULL(EWBNO, '') AS EWBNO ", "", " EWAYENTRY ", " AND EWBNO <> '' AND ENTRYNO = " & Val(ROW("SRNO")) & " And YearId = " & YearId)
            Dim OBJCMN1 As New ClsCommon
            Dim DT1 As DataTable = OBJCMN1.SEARCH("ISNULL(STATE_REMARK,'') AS STATECODE, ISNULL(REGISTERMASTER.REGISTER_ID,0) AS REGID", "", " INVOICEMASTER INNER JOIN LEDGERS ON INVOICE_LEDGERID = LEDGERS.ACC_ID LEFT OUTER JOIN STATEMASTER ON LEDGERS.ACC_STATEID = STATE_ID INNER JOIN REGISTERMASTER ON REGISTER_ID = INVOICEMASTER.INVOICE_REGISTERID ", " AND INVOICEMASTER.INVOICE_NO = " & Val(ROW("SRNO")) & " AND REGISTER_NAME = '" & cmbregister.Text.Trim & "' AND INVOICEMASTER.INVOICE_YEARID = " & YearId)
            'OBJINVOICE.Show()
            'OBJINVOICE.Close()
            If DT.Rows.Count = 0 Then Continue For

            Dim EWBNO As String = DT.Rows(0)("EWBNO").ToString.Trim
            If EWBNO = "" Then Continue For

            ALATTACHMENT.Add(Application.StartupPath & "\" & "EWB_" & DT.Rows(0)("EWBNO") & ".pdf")
            FILENAME.Add("EWB_" & DT.Rows(0)("EWBNO") & ".pdf")

            DTWHATSAPP.Rows.Add(ROW("SRNO"), DT1.Rows(0).Item("REGID"), cmbregister.Text.Trim, ROW("PRINTINITIALS"), ROW("DATE"), ROW("NAME"), ROW("PARTYWHATSAPP"), ROW("AGENTNAME"), ROW("AGENTWHATSAPP"), Val(ROW("GRANDTOTAL")), UCase(CmpName) & " - Invoice No. " & ROW("PRINTINITIALS") & " Dated " & ROW("DATE"), Application.StartupPath & "\" & "EWB_" & DT.Rows(0)("EWBNO") & ".pdf", "EWB_" & DT.Rows(0)("EWBNO") & ".pdf")

            '' 🔹 Fetch party WhatsApp + PDF path
            'Dim dt As DataTable = OBJCMN.SEARCH(
            '"ISNULL(LEDGER.ACC_WHATSAPPNO,'') AS PHONE, ISNULL(INVOICE_PRINTINITIALS,'') AS PRINTNO",
            '"",
            '"INVOICEMASTER INNER JOIN LEDGERS AS LEDGER ON INVOICE_LEDGERID = LEDGER.ACC_ID",
            '" AND INVOICE_NO = " & invNo & " AND INVOICE_YEARID = " & YearId)

            'If dt.Rows.Count > 0 Then
            '    Dim phone As String = dt.Rows(0)("PHONE").ToString
            '    Dim filepath As String = Application.StartupPath & "\INVOICE_" & invNo & ".pdf"

            'If phone <> "" And File.Exists(filepath) Then
            '    SendWhatsAppDirect(phone, filepath, "EWB_Invoice_" & invNo & ".pdf")
            'End If
        ...
        End If
        MsgBox("All EWB generated and WhatsApp sent successfully.", MsgBoxStyle.Information)
