    ...
    Sub fillgrid(Optional ByVal where As String = "")
        Try
            ...
            Else
                ...
                DT1 = objclspreq.SEARCH("*", "", "(SELECT DISTINCT CAST (0 AS BIT) AS CHK, SALERETURNCHALLAN.SRCH_NO AS SRCHNO, SALERETURNCHALLAN.SRCH_date AS DATE, ISNULL(SRCH_CHALLANNO,'') AS PARTYCHALLANNO, LEDGERS.Acc_cmpname AS NAME, ISNULL(ITEM_NAME, '') AS ITEMNAME, ISNULL(DESIGN_NO, '') AS DESIGNNO, ISNULL(COLOR_NAME, '') AS SHADE, SUM(ISNULL(SALERETURNCHALLAN_DESC.SRCH_QTY, 0)) AS PCS, SUM(ISNULL(SALERETURNCHALLAN_DESC.SRCH_MTRS, 0)) AS MTRS, ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN, ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), '') AS STATECODE, ISNULL(LEDGERS.ACC_GSTIN,'') AS GSTIN, ISNULL(SALERETURNCHALLAN.SRCH_NOOFBALES, 0) AS NOOFBALES, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(SALERETURNCHALLAN.SRCH_LRNO, '') AS LRNO, SALERETURNCHALLAN.SRCH_LRDATE AS LRDATE , 'SALERETURN' AS FROMTYPE FROM SALERETURNCHALLAN INNER JOIN SALERETURNCHALLAN_DESC ON SALERETURNCHALLAN.SRCH_NO = SALERETURNCHALLAN_DESC.SRCH_NO AND SALERETURNCHALLAN.SRCH_YEARID = SALERETURNCHALLAN_DESC.SRCH_YEARID LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON SALERETURNCHALLAN.SRCH_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN GODOWNMASTER ON SALERETURNCHALLAN.SRCH_GODOWNID = GODOWNMASTER.GODOWN_id LEFT OUTER JOIN LEDGERS ON SALERETURNCHALLAN.SRCH_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN ITEMMASTER ON SRCH_ITEMID = ITEM_ID LEFT OUTER JOIN DESIGNMASTER ON SRCH_DESIGNID = DESIGN_ID LEFT OUTER JOIN COLORMASTER ON SRCH_COLORID = COLOR_ID LEFT OUTER JOIN STATEMASTER ON LEDGERS.ACC_STATEID = STATEMASTER.STATE_ID WHERE SALERETURNCHALLAN.SRCH_DONE = 0 AND SALERETURNCHALLAN.SRCH_YEARID = " & YearId & where & " GROUP BY SALERETURNCHALLAN.SRCH_NO, SALERETURNCHALLAN.SRCH_date, ISNULL(SRCH_CHALLANNO,''), LEDGERS.Acc_cmpname, ISNULL(ITEM_NAME, '') , ISNULL(DESIGN_NO, ''), ISNULL(COLOR_NAME, '') , ISNULL(GODOWNMASTER.GODOWN_name, ''), ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), ''), ISNULL(LEDGERS.ACC_GSTIN,''),  ISNULL(SALERETURNCHALLAN.SRCH_NOOFBALES, 0), ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(SALERETURNCHALLAN.SRCH_LRNO, ''), SALERETURNCHALLAN.SRCH_LRDATE UNION ALL SELECT DISTINCT CAST (0 AS BIT) AS CHK, GREYSALERETURNCHALLAN.GSRCH_NO AS SRCHNO, GREYSALERETURNCHALLAN.GSRCH_date AS DATE, ISNULL(GSRCH_CHALLANNO,'') AS PARTYCHALLANNO, LEDGERS.Acc_cmpname AS NAME, ISNULL(ITEM_NAME, '') AS ITEMNAME, ISNULL(DESIGN_NO, '') AS DESIGNNO, ISNULL(COLOR_NAME, '') AS SHADE, SUM(ISNULL(GREYSALERETURNCHALLAN_DESC.GSRCH_QTY, 0)) AS PCS, SUM(ISNULL(GREYSALERETURNCHALLAN_DESC.GSRCH_MTRS, 0)) AS MTRS, ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN, ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), '') AS STATECODE, ISNULL(LEDGERS.ACC_GSTIN,'') AS GSTIN, ISNULL(GREYSALERETURNCHALLAN.GSRCH_NOOFBALES, 0) AS NOOFBALES, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(GREYSALERETURNCHALLAN.GSRCH_LRNO, '') AS LRNO, GREYSALERETURNCHALLAN.GSRCH_LRDATE AS LRDATE , 'GREYSALERETURN' AS FROMTYPE FROM GREYSALERETURNCHALLAN INNER JOIN GREYSALERETURNCHALLAN_DESC ON GREYSALERETURNCHALLAN.GSRCH_NO = GREYSALERETURNCHALLAN_DESC.GSRCH_NO AND GREYSALERETURNCHALLAN.GSRCH_YEARID = GREYSALERETURNCHALLAN_DESC.GSRCH_YEARID LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GREYSALERETURNCHALLAN.GSRCH_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN GODOWNMASTER ON GREYSALERETURNCHALLAN.GSRCH_GODOWNID = GODOWNMASTER.GODOWN_id LEFT OUTER JOIN LEDGERS ON GREYSALERETURNCHALLAN.GSRCH_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN ITEMMASTER ON GSRCH_ITEMID = ITEM_ID LEFT OUTER JOIN DESIGNMASTER ON GSRCH_DESIGNID = DESIGN_ID LEFT OUTER JOIN COLORMASTER ON GSRCH_COLORID = COLOR_ID LEFT OUTER JOIN STATEMASTER ON LEDGERS.ACC_STATEID = STATEMASTER.STATE_ID WHERE GREYSALERETURNCHALLAN.GSRCH_DONE = 0 AND GREYSALERETURNCHALLAN.GSRCH_YEARID = " & YearId & where & " GROUP BY GREYSALERETURNCHALLAN.GSRCH_NO, GREYSALERETURNCHALLAN.GSRCH_date, ISNULL(GSRCH_CHALLANNO,''), LEDGERS.Acc_cmpname, ISNULL(ITEM_NAME, '') , ISNULL(DESIGN_NO, ''), ISNULL(COLOR_NAME, '') , ISNULL(GODOWNMASTER.GODOWN_name, ''), ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), ''), ISNULL(LEDGERS.ACC_GSTIN,''),  ISNULL(GREYSALERETURNCHALLAN.GSRCH_NOOFBALES, 0), ISNULL(TRANSLEDGERS.Acc_cmpname, ''), ISNULL(GREYSALERETURNCHALLAN.GSRCH_LRNO, ''), GREYSALERETURNCHALLAN.GSRCH_LRDATE) AS T", " ORDER BY T.SRCHNO")
            End If
        ...
        Try
            ...
            For i As Integer = 0 To gridbill.RowCount - 1
                ...
                If Convert.ToBoolean(dtrow("CHK")) = True Then
                    DT1.Rows.Add(dtrow("SRCHNO"), dtrow("DATE"), dtrow("PARTYCHALLANNO"), dtrow("NAME"), Val(dtrow("PCS")), Val(dtrow("MTRS")), dtrow("GODOWN"), dtrow("GSTIN"), dtrow("STATECODE"), dtrow("NOOFBALES"), dtrow("TRANSNAME"), dtrow("LRNO"), dtrow("LRDATE"), dtrow("FROMTYPE"))
                End If
            Next
            Me.Close()
        Catch ex As Exception
