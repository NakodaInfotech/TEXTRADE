    ...
    Sub FILLGRID(ByVal WHERE As String)
        Try
            ...
            If ClientName = "SVS" Or ClientName = "CC" Or ClientName = "C3" Or ClientName = "SHREEDEV" Then
                DT = OBJCMN.Execute_Any_String("SELECT ITEMNAME, QUALITY, DESIGNNO, COLOR, GODOWN, JOBBERNAME, UNIT, 1 AS PCS, CUT, MTRS, BARCODE, LOTNO, FROMNO, FROMSRNO, TYPE, PIECETYPE, BALENO, GRIDREMARKS, CATEGORY, CAST(DATE AS DATE) AS [DATE], PURNAME, RACK, SHELF, CHALLANNO, PURRATE, WT FROM BARCODESTOCK WHERE ROUND(MTRS,0) > 0 " & WHERE & SELECTIONFORMULA & " AND YEARID = " & YearId, "", "")
            ElseIf ClientName = "MOHAN" Then
                DT = OBJCMN.Execute_Any_String("SELECT ITEMNAME, QUALITY, DESIGNNO, COLOR, GODOWN, JOBBERNAME, UNIT, 1 AS PCS, CUT, MTRS, BARCODE, LOTNO, FROMNO, FROMSRNO, TYPE, PIECETYPE, BALENO, GRIDREMARKS, CATEGORY, CAST(DATE AS DATE) AS [DATE], PURNAME, RACK, SHELF, CHALLANNO, PURRATE, WT FROM BARCODESTOCK WHERE 1 = 1 " & WHERE & SELECTIONFORMULA & " AND YEARID = " & YearId, "", "")
            ...
            ElseIf FRMSTRING = "GREY" Then
                DT = OBJCMN.Execute_Any_String("Select ITEMNAME, QUALITY, DESIGNNO, Color, GODOWN, JOBBERNAME, UNIT, PCS, CUT, MTRS, BARCODE, LOTNO, FROMNO, FROMSRNO, Type, PIECETYPE, BALENO, GRIDREMARKS, CATEGORY, CAST(DATE AS DATE) AS [DATE], PURNAME, RACK, SHELF, CHALLANNO, PURRATE, WT FROM GREYBARCODESTOCK WHERE 1=1 " & WHERE & SELECTIONFORMULA & " And YEARID = " & YearId & " ORDER BY TYPE, FROMNO, FROMSRNO", "", "")
            Else
                DT = OBJCMN.Execute_Any_String("Select ITEMNAME, QUALITY, DESIGNNO, Color, GODOWN, JOBBERNAME, UNIT, PCS, CUT, MTRS, BARCODE, LOTNO, FROMNO, FROMSRNO, Type, PIECETYPE, BALENO, GRIDREMARKS, CATEGORY, CAST(DATE AS DATE) AS [DATE], PURNAME, RACK, SHELF, CHALLANNO, PURRATE, WT FROM BARCODESTOCK WHERE 1=1 " & WHERE & SELECTIONFORMULA & " And YEARID = " & YearId & " ORDER BY TYPE, FROMNO, FROMSRNO", "", "")
            End If
            ...
            For Each ROW As DataGridViewRow In gridwo.Rows
                If ROW.Cells(0).Value = True Then
                    ...
                    If DTROW.Length = 0 Then
                        DT.Rows.Add(ROW.Cells(16).Value, ROW.Cells(1).Value, ROW.Cells(2).Value, ROW.Cells(3).Value, ROW.Cells(4).Value, ROW.Cells(5).Value, ROW.Cells(6).Value, ROW.Cells(7).Value, ROW.Cells(8).Value, ROW.Cells(9).Value, ROW.Cells(10).Value, ROW.Cells(11).Value, ROW.Cells(12).Value, ROW.Cells(13).Value, ROW.Cells(14).Value, ROW.Cells(15).Value, ROW.Cells(17).Value, ROW.Cells(18).Value, ROW.Cells(21).Value, ROW.Cells(22).Value, ROW.Cells(23).Value, ROW.Cells(24).Value, Val(ROW.Cells(25).Value), Val(ROW.Cells(26).Value))
                    End If
                End If
            Next
