...
Public Class SelectStockGDNGrid
    ...
    Sub FILLGRID()
        Try
            ...
            Dim DT As DataTable = OBJCMN.Execute_Any_String(" SELECT CAST(0 AS BIT) AS CHK, SUM(PCS) AS TOTALPCS, SUM(MTRS) AS TOTALMTRS, DESIGNNO, ITEMNAME, QUALITY, COLOR ,GODOWN, LOTNO, BALENO, CHALLANNO, PIECETYPE, BARCODE, UNIT, ITEMCODE, CATEGORY,PURRATE, SALERATE, DESIGNRATE,RACK,SHELF, MILLNAME, DATE, JOBBERNAME, TYPE, GRIDREMARKS, PURNAME, ROUND(PURRATE*SUM(MTRS),2) AS AMOUNT, DISPLAYNAME, SUM(WT) AS WT FROM  BARCODESTOCK WHERE YEARID = " & YearId & WHERECLAUSE & " GROUP BY DESIGNNO, ITEMNAME, QUALITY, LOTNO, BALENO, CHALLANNO, COLOR ,GODOWN, PIECETYPE, BARCODE, UNIT, ITEMCODE, CATEGORY, PURRATE,RACK,SHELF, SALERATE, DESIGNRATE, MILLNAME, DATE, JOBBERNAME, TYPE, GRIDREMARKS, PURNAME, DISPLAYNAME ORDER BY DESIGNNO, QUALITY, COLOR", "", "")
