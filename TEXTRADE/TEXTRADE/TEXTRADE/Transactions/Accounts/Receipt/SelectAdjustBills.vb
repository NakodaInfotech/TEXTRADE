...
Public Class SelectAdjustBills
    ...
    Public TEMPCMPID As Integer
    Public TEMPYEARID As Integer
    ...
    Sub fillgrid(ByVal TEMPCONDITION)
        Try
            ...
            If FRMSTRING = "BILLINTEREST" Then
                dt = OBJCMN.SEARCH(" *, CAST (0 AS BIT) AS CHK ", "", " (SELECT OPENINGBILL.BILL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, OPENINGBILL.BILL_AMT AS BILLAMT, OPENINGBILL.BILL_BALANCE AS BALAMT, 'OPENING' AS TYPE, OPENINGBILL.BILL_NARRATION AS REFNO, OPENINGBILL.BILL_NO AS BILLNO, OPENINGBILL.BILL_DATE AS DATE, OPENINGBILL.BILL_CMPID AS CMPID, OPENINGBILL.BILL_LOCATIONID AS LOCATIONID, OPENINGBILL.BILL_YEARID AS YEARID, ISNULL(OPENINGBILL.BILL_DISPUTE,0) AS DISPUTE ,0.00 AS ADJUSTAMT FROM OPENINGBILL INNER JOIN LEDGERS ON OPENINGBILL.BILL_LEDGERID = LEDGERS.Acc_id  UNION ALL  SELECT INVOICEMASTER.INVOICE_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, INVOICEMASTER.INVOICE_GRANDTOTAL AS BILLAMT, INVOICEMASTER.INVOICE_BALANCE AS BALAMT, 'INVOICE' AS TYPE, INVOICEMASTER.INVOICE_REFNO AS REFNO, INVOICEMASTER.INVOICE_NO AS BILLNO, INVOICEMASTER.INVOICE_DATE AS DATE, INVOICEMASTER.INVOICE_CMPID AS CMPID, INVOICEMASTER.INVOICE_LOCATIONID AS LOCATIONID, INVOICEMASTER.INVOICE_YEARID AS YEARID, ISNULL(INVOICEMASTER.INVOICE_DISPUTE,0) AS DISPUTE,0.00 AS ADJUSTAMT FROM INVOICEMASTER INNER JOIN LEDGERS ON INVOICEMASTER.INVOICE_LEDGERID = LEDGERS.Acc_id  UNION ALL SELECT PURCHASEMASTER.BILL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(BILL_RCM,0) = 'FALSE' THEN PURCHASEMASTER.BILL_GRANDTOTAL ELSE (CASE WHEN ISNULL(BILL_SCREENTYPE,'LINE GST') = 'LINE GST' THEN BILL_TOTALTAXABLEAMT ELSE BILL_SUBTOTAL END) END)  AS BILLAMT, PURCHASEMASTER.BILL_BALANCE AS BALAMT, 'PURCHASE' AS TYPE, PURCHASEMASTER.BILL_PARTYBILLNO AS REFNO, PURCHASEMASTER.BILL_NO AS BILLNO, PURCHASEMASTER.BILL_PARTYBILLDATE AS DATE, PURCHASEMASTER.BILL_CMPID AS CMPID, PURCHASEMASTER.BILL_LOCATIONID AS LOCATIONID, PURCHASEMASTER.BILL_YEARID AS YEARID, ISNULL(PURCHASEMASTER.BILL_DISPUTE,0) AS DISPUTE ,0.00 AS ADJUSTAMT FROM PURCHASEMASTER INNER JOIN LEDGERS ON PURCHASEMASTER.BILL_LEDGERID = LEDGERS.Acc_id UNION ALL SELECT NONPURCHASE.NP_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(NP_RCM,0) = 'TRUE' THEN NP_TOTALTAXABLEAMT ELSE NONPURCHASE.NP_GRANDTOTAL END) AS BILLAMT, NONPURCHASE.NP_BALANCE AS BALAMT, 'NONPURCHASE' AS TYPE, NONPURCHASE.NP_REFNO AS REFNO, NONPURCHASE.NP_NO AS BILLNO, NONPURCHASE.NP_PARTYBILLDATE AS DATE, NONPURCHASE.NP_CMPID AS CMPID, NONPURCHASE.NP_LOCATIONID AS LOCATIONID, NONPURCHASE.NP_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM NONPURCHASE INNER JOIN LEDGERS ON NONPURCHASE.NP_LEDGERID = LEDGERS.Acc_id  UNION ALL SELECT CREDITNOTEMASTER.CN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(CN_RCM,0) = 'TRUE' THEN CN_SUBTOTAL ELSE CREDITNOTEMASTER.CN_GTOTAL END) AS BILLAMT, CREDITNOTEMASTER.CN_BALANCE AS BALAMT, 'CREDITNOTE' AS TYPE, CREDITNOTEMASTER.CN_PARTYREFNO AS REFNO, CREDITNOTEMASTER.CN_NO AS BILLNO, CREDITNOTEMASTER.CN_date AS DATE, CREDITNOTEMASTER.CN_CMPID AS CMPID, CREDITNOTEMASTER.CN_LOCATIONID AS LOCATIONID, CREDITNOTEMASTER.CN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM CREDITNOTEMASTER INNER JOIN LEDGERS ON CREDITNOTEMASTER.CN_LEDGERID = LEDGERS.Acc_id WHERE CN_date >= '07/01/2017' UNION ALL SELECT DEBITNOTEMASTER.DN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, DEBITNOTEMASTER.DN_GTOTAL AS BILLAMT, DEBITNOTEMASTER.DN_BALANCE AS BALAMT, 'DEBITNOTE' AS TYPE, DEBITNOTEMASTER.DN_initials AS REFNO, DEBITNOTEMASTER.DN_NO AS BILLNO, DEBITNOTEMASTER.DN_date AS DATE, DEBITNOTEMASTER.DN_CMPID AS CMPID, DEBITNOTEMASTER.DN_LOCATIONID AS LOCATIONID, DEBITNOTEMASTER.DN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM DEBITNOTEMASTER INNER JOIN LEDGERS ON DEBITNOTEMASTER.DN_LEDGERID = LEDGERS.Acc_id WHERE DN_date >= '07/01/2017') AS T ", TEMPCONDITION & " AND T.YEARID =" & YearId & " ORDER BY  T.DATE, T.BILLNO ")
            ElseIf CMPNAME = "ABHEE FABRICS LLP" Then
                dt = OBJCMN.SEARCH(" *, CAST (0 AS BIT) AS CHK ", "", " (SELECT OPENINGBILL.BILL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, OPENINGBILL.BILL_AMT AS BILLAMT, OPENINGBILL.BILL_BALANCE AS BALAMT, 'OPENING' AS TYPE, OPENINGBILL.BILL_NARRATION AS REFNO, OPENINGBILL.BILL_NO AS BILLNO, OPENINGBILL.BILL_DATE AS DATE, OPENINGBILL.BILL_CMPID AS CMPID, OPENINGBILL.BILL_LOCATIONID AS LOCATIONID, OPENINGBILL.BILL_YEARID AS YEARID, ISNULL(OPENINGBILL.BILL_DISPUTE,0) AS DISPUTE ,0.00 AS ADJUSTAMT FROM OPENINGBILL INNER JOIN LEDGERS ON OPENINGBILL.BILL_LEDGERID = LEDGERS.Acc_id  WHERE(OPENINGBILL.BILL_BALANCE > 0) UNION  ALL  SELECT JOURNALMASTER.JOURNAL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, JOURNALMASTER.JOURNAL_CREDIT AS BILLAMT, (JOURNALMASTER.journal_credit-journal_amt) AS BALAMT, 'JOURNAL' AS TYPE, JOURNALMASTER.journal_refno AS REFNO, JOURNALMASTER.JOURNAL_NO AS BILLNO, JOURNALMASTER.JOURNAL_DATE AS DATE, JOURNALMASTER.JOURNAL_CMPID AS CMPID, JOURNALMASTER.JOURNAL_LOCATIONID AS LOCATIONID, JOURNALMASTER.JOURNAL_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM JOURNALMASTER INNER JOIN LEDGERS ON JOURNALMASTER.JOURNAL_LEDGERID = LEDGERS.Acc_id  WHERE(JOURNALMASTER.journal_credit - journal_amt) > 0 AND journal_credit > 0 AND journal_paytype = 'New Ref.' UNION ALL SELECT JOURNALMASTER.JOURNAL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, JOURNALMASTER.JOURNAL_DEBIT AS BILLAMT, (JOURNALMASTER.journal_DEBIT-journal_amt) AS BALAMT, 'JOURNAL' AS TYPE, JOURNALMASTER.journal_refno AS REFNO, JOURNALMASTER.JOURNAL_NO AS BILLNO, JOURNALMASTER.JOURNAL_DATE AS DATE, JOURNALMASTER.JOURNAL_CMPID AS CMPID, JOURNALMASTER.JOURNAL_LOCATIONID AS LOCATIONID, JOURNALMASTER.JOURNAL_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM JOURNALMASTER INNER JOIN LEDGERS ON JOURNALMASTER.JOURNAL_LEDGERID = LEDGERS.Acc_id  WHERE(JOURNALMASTER.journal_debit - journal_amt) > 0 AND journal_debit > 0 AND journal_paytype = 'New Ref.'  UNION ALL  SELECT INVOICEMASTER.INVOICE_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, INVOICEMASTER.INVOICE_GRANDTOTAL AS BILLAMT, INVOICEMASTER.INVOICE_BALANCE AS BALAMT, 'INVOICE' AS TYPE, INVOICEMASTER.INVOICE_REFNO AS REFNO, INVOICEMASTER.INVOICE_NO AS BILLNO, INVOICEMASTER.INVOICE_DATE AS DATE, INVOICEMASTER.INVOICE_CMPID AS CMPID, INVOICEMASTER.INVOICE_LOCATIONID AS LOCATIONID, INVOICEMASTER.INVOICE_YEARID AS YEARID, ISNULL(INVOICEMASTER.INVOICE_DISPUTE,0) AS DISPUTE,0.00 AS ADJUSTAMT FROM INVOICEMASTER INNER JOIN LEDGERS ON INVOICEMASTER.INVOICE_LEDGERID = LEDGERS.Acc_id WHERE INVOICEMASTER.INVOICE_BALANCE > 0 UNION ALL SELECT PURCHASEMASTER.BILL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(BILL_RCM,0) = 'FALSE' THEN PURCHASEMASTER.BILL_GRANDTOTAL ELSE (CASE WHEN ISNULL(BILL_SCREENTYPE,'LINE GST') = 'LINE GST' THEN BILL_TOTALTAXABLEAMT ELSE BILL_SUBTOTAL END) END)  AS BILLAMT, PURCHASEMASTER.BILL_BALANCE AS BALAMT, 'PURCHASE' AS TYPE, PURCHASEMASTER.BILL_PARTYBILLNO AS REFNO, PURCHASEMASTER.BILL_NO AS BILLNO, PURCHASEMASTER.BILL_PARTYBILLDATE AS DATE, PURCHASEMASTER.BILL_CMPID AS CMPID, PURCHASEMASTER.BILL_LOCATIONID AS LOCATIONID, PURCHASEMASTER.BILL_YEARID AS YEARID, ISNULL(PURCHASEMASTER.BILL_DISPUTE,0) AS DISPUTE,0.00 AS ADJUSTAMT FROM PURCHASEMASTER INNER JOIN LEDGERS ON PURCHASEMASTER.BILL_LEDGERID = LEDGERS.Acc_id WHERE PURCHASEMASTER.BILL_BALANCE > 0 UNION ALL SELECT NONPURCHASE.NP_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(NP_RCM,0) = 'TRUE' THEN NP_TOTALTAXABLEAMT ELSE NONPURCHASE.NP_GRANDTOTAL END) AS BILLAMT, NONPURCHASE.NP_BALANCE AS BALAMT, 'NONPURCHASE' AS TYPE, NONPURCHASE.NP_REFNO AS REFNO, NONPURCHASE.NP_NO AS BILLNO, NONPURCHASE.NP_PARTYBILLDATE AS DATE, NONPURCHASE.NP_CMPID AS CMPID, NONPURCHASE.NP_LOCATIONID AS LOCATIONID, NONPURCHASE.NP_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM NONPURCHASE INNER JOIN LEDGERS ON NONPURCHASE.NP_LEDGERID = LEDGERS.Acc_id  WHERE NONPURCHASE.NP_BALANCE > 0 UNION ALL SELECT CREDITNOTEMASTER.CN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(CN_RCM,0) = 'TRUE' THEN CN_SUBTOTAL ELSE CREDITNOTEMASTER.CN_GTOTAL END) AS BILLAMT, CREDITNOTEMASTER.CN_BALANCE AS BALAMT, 'CREDITNOTE' AS TYPE, CREDITNOTEMASTER.CN_PARTYREFNO AS REFNO, CREDITNOTEMASTER.CN_NO AS BILLNO, CREDITNOTEMASTER.CN_date AS DATE, CREDITNOTEMASTER.CN_CMPID AS CMPID, CREDITNOTEMASTER.CN_LOCATIONID AS LOCATIONID, CREDITNOTEMASTER.CN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM CREDITNOTEMASTER INNER JOIN LEDGERS ON CREDITNOTEMASTER.CN_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN CREDITNOTEMASTER_BILLDESC ON CREDITNOTEMASTER.CN_NO = CREDITNOTEMASTER_BILLDESC.CN_NO AND CREDITNOTEMASTER.CN_registerid = CREDITNOTEMASTER_BILLDESC.CN_REGISTERID WHERE CREDITNOTEMASTER.CN_BALANCE > 0 AND CN_date >= '07/01/2017'AND CN_BILLNO = '' AND CREDITNOTEMASTER_BILLDESC.CN_NO IS NULL UNION ALL SELECT CREDITNOTEMASTER.CN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, CREDITNOTEMASTER_BILLDESC.CN_AMT AS BILLAMT, CREDITNOTEMASTER_BILLDESC.CN_BALANCE AS BALAMT, 'CREDITNOTE' AS TYPE, CREDITNOTEMASTER.CN_PARTYREFNO AS REFNO, CREDITNOTEMASTER.CN_NO AS BILLNO, CREDITNOTEMASTER.CN_date AS DATE, CREDITNOTEMASTER.CN_CMPID AS CMPID, CREDITNOTEMASTER.CN_LOCATIONID AS LOCATIONID, CREDITNOTEMASTER.CN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM CREDITNOTEMASTER INNER JOIN LEDGERS ON CREDITNOTEMASTER.CN_LEDGERID = LEDGERS.Acc_id INNER JOIN CREDITNOTEMASTER_BILLDESC ON CREDITNOTEMASTER.CN_NO = CREDITNOTEMASTER_BILLDESC.CN_NO AND CREDITNOTEMASTER.CN_registerid = CREDITNOTEMASTER_BILLDESC.CN_REGISTERID WHERE CREDITNOTEMASTER_BILLDESC.CN_BALANCE > 0 AND CREDITNOTEMASTER_BILLDESC.CN_PAYTYPE = 'New Ref' UNION ALL SELECT 'SR-' + CAST(SALERETURN.SALRET_NO AS VARCHAR(50)) AS SRNO, LEDGERS.Acc_cmpname AS NAME, SALERETURN_BILLDESC.SALRET_AMT AS BILLAMT, SALERETURN_BILLDESC.SALRET_BALANCE AS BALAMT, 'SALERETURN' AS TYPE, SALERETURN.SALRET_PARTYREFNO AS REFNO, SALERETURN.SALRET_NO AS BILLNO, SALERETURN.SALRET_date AS DATE, SALERETURN.SALRET_CMPID AS CMPID, SALERETURN.SALRET_LOCATIONID AS LOCATIONID, SALERETURN.SALRET_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM SALERETURN INNER JOIN LEDGERS ON SALERETURN.SALRET_LEDGERID = LEDGERS.Acc_id INNER JOIN SALERETURN_BILLDESC ON SALERETURN.SALRET_NO = SALERETURN_BILLDESC.SALRET_NO AND SALERETURN.SALRET_YEARID = SALERETURN_BILLDESC.SALRET_YEARID WHERE SALERETURN_BILLDESC.SALRET_BALANCE > 0 AND SALERETURN_BILLDESC.SALRET_PAYTYPE = 'New Ref' UNION ALL SELECT DEBITNOTEMASTER.DN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, DEBITNOTEMASTER.DN_GTOTAL AS BILLAMT, DEBITNOTEMASTER.DN_BALANCE AS BALAMT, 'DEBITNOTE' AS TYPE, DEBITNOTEMASTER.DN_initials AS REFNO, DEBITNOTEMASTER.DN_NO AS BILLNO, DEBITNOTEMASTER.DN_date AS DATE, DEBITNOTEMASTER.DN_CMPID AS CMPID, DEBITNOTEMASTER.DN_LOCATIONID AS LOCATIONID, DEBITNOTEMASTER.DN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM DEBITNOTEMASTER INNER JOIN LEDGERS ON DEBITNOTEMASTER.DN_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN DEBITNOTEMASTER_BILLDESC ON DEBITNOTEMASTER.DN_NO = DEBITNOTEMASTER_BILLDESC.DN_NO AND DEBITNOTEMASTER.DN_registerid = DEBITNOTEMASTER_BILLDESC.DN_REGISTERID WHERE DEBITNOTEMASTER.DN_BALANCE > 0 AND DN_date >= '07/01/2017' AND DN_BILLNO = '' AND DEBITNOTEMASTER_BILLDESC.DN_NO IS NULL UNION ALL SELECT DEBITNOTEMASTER.DN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, DEBITNOTEMASTER_BILLDESC.DN_AMT AS BILLAMT, DEBITNOTEMASTER_BILLDESC.DN_BALANCE AS BALAMT, 'DEBITNOTE' AS TYPE, DEBITNOTEMASTER.DN_initials AS REFNO, DEBITNOTEMASTER.DN_NO AS BILLNO, DEBITNOTEMASTER.DN_date AS DATE, DEBITNOTEMASTER.DN_CMPID AS CMPID, DEBITNOTEMASTER.DN_LOCATIONID AS LOCATIONID, DEBITNOTEMASTER.DN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM DEBITNOTEMASTER INNER JOIN LEDGERS ON DEBITNOTEMASTER.DN_LEDGERID = LEDGERS.Acc_id INNER JOIN DEBITNOTEMASTER_BILLDESC ON DEBITNOTEMASTER.DN_NO = DEBITNOTEMASTER_BILLDESC.DN_NO AND DEBITNOTEMASTER.DN_registerid = DEBITNOTEMASTER_BILLDESC.DN_REGISTERID WHERE DEBITNOTEMASTER_BILLDESC.DN_BALANCE > 0 and DEBITNOTEMASTER_BILLDESC.DN_PAYTYPE = 'New Ref' UNION ALL SELECT PURCHASERETURN.PR_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, PURCHASERETURN_BILLDESC.PR_AMT AS BILLAMT, PURCHASERETURN_BILLDESC.PR_BALANCE AS BALAMT, 'PURCHASERETURN' AS TYPE, PURCHASERETURN.PR_initials AS REFNO, PURCHASERETURN.PR_NO AS BILLNO, PURCHASERETURN.PR_date AS DATE, PURCHASERETURN.PR_CMPID AS CMPID, PURCHASERETURN.PR_LOCATIONID AS LOCATIONID, PURCHASERETURN.PR_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM PURCHASERETURN INNER JOIN LEDGERS ON PURCHASERETURN.PR_LEDGERID = LEDGERS.Acc_id INNER JOIN PURCHASERETURN_BILLDESC ON PURCHASERETURN.PR_NO = PURCHASERETURN_BILLDESC.PR_NO AND PURCHASERETURN.PR_YEARID = PURCHASERETURN_BILLDESC.PR_YEARID WHERE PURCHASERETURN_BILLDESC.PR_BALANCE > 0 and PURCHASERETURN_BILLDESC.PR_PAYTYPE = 'New Ref') AS T ", TEMPCONDITION & " AND T.BALAMT > 0 AND T.YEARID =" & TEMPYEARID & " ORDER BY T.TYPE, T.BILLNO ")
            Else
                dt = OBJCMN.SEARCH(" *, CAST (0 AS BIT) AS CHK ", "", " (SELECT OPENINGBILL.BILL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, OPENINGBILL.BILL_AMT AS BILLAMT, OPENINGBILL.BILL_BALANCE AS BALAMT, 'OPENING' AS TYPE, OPENINGBILL.BILL_NARRATION AS REFNO, OPENINGBILL.BILL_NO AS BILLNO, OPENINGBILL.BILL_DATE AS DATE, OPENINGBILL.BILL_CMPID AS CMPID, OPENINGBILL.BILL_LOCATIONID AS LOCATIONID, OPENINGBILL.BILL_YEARID AS YEARID, ISNULL(OPENINGBILL.BILL_DISPUTE,0) AS DISPUTE ,0.00 AS ADJUSTAMT FROM OPENINGBILL INNER JOIN LEDGERS ON OPENINGBILL.BILL_LEDGERID = LEDGERS.Acc_id  WHERE(OPENINGBILL.BILL_BALANCE > 0) UNION  ALL  SELECT JOURNALMASTER.JOURNAL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, JOURNALMASTER.JOURNAL_CREDIT AS BILLAMT, (JOURNALMASTER.journal_credit-journal_amt) AS BALAMT, 'JOURNAL' AS TYPE, JOURNALMASTER.journal_refno AS REFNO, JOURNALMASTER.JOURNAL_NO AS BILLNO, JOURNALMASTER.JOURNAL_DATE AS DATE, JOURNALMASTER.JOURNAL_CMPID AS CMPID, JOURNALMASTER.JOURNAL_LOCATIONID AS LOCATIONID, JOURNALMASTER.JOURNAL_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM JOURNALMASTER INNER JOIN LEDGERS ON JOURNALMASTER.JOURNAL_LEDGERID = LEDGERS.Acc_id  WHERE(JOURNALMASTER.journal_credit - journal_amt) > 0 AND journal_credit > 0 AND journal_paytype = 'New Ref.' UNION ALL SELECT JOURNALMASTER.JOURNAL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, JOURNALMASTER.JOURNAL_DEBIT AS BILLAMT, (JOURNALMASTER.journal_DEBIT-journal_amt) AS BALAMT, 'JOURNAL' AS TYPE, JOURNALMASTER.journal_refno AS REFNO, JOURNALMASTER.JOURNAL_NO AS BILLNO, JOURNALMASTER.JOURNAL_DATE AS DATE, JOURNALMASTER.JOURNAL_CMPID AS CMPID, JOURNALMASTER.JOURNAL_LOCATIONID AS LOCATIONID, JOURNALMASTER.JOURNAL_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM JOURNALMASTER INNER JOIN LEDGERS ON JOURNALMASTER.JOURNAL_LEDGERID = LEDGERS.Acc_id  WHERE(JOURNALMASTER.journal_debit - journal_amt) > 0 AND journal_debit > 0 AND journal_paytype = 'New Ref.'  UNION ALL  SELECT INVOICEMASTER.INVOICE_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, INVOICEMASTER.INVOICE_GRANDTOTAL AS BILLAMT, INVOICEMASTER.INVOICE_BALANCE AS BALAMT, 'INVOICE' AS TYPE, INVOICEMASTER.INVOICE_REFNO AS REFNO, INVOICEMASTER.INVOICE_NO AS BILLNO, INVOICEMASTER.INVOICE_DATE AS DATE, INVOICEMASTER.INVOICE_CMPID AS CMPID, INVOICEMASTER.INVOICE_LOCATIONID AS LOCATIONID, INVOICEMASTER.INVOICE_YEARID AS YEARID, ISNULL(INVOICEMASTER.INVOICE_DISPUTE,0) AS DISPUTE,0.00 AS ADJUSTAMT FROM INVOICEMASTER INNER JOIN LEDGERS ON INVOICEMASTER.INVOICE_LEDGERID = LEDGERS.Acc_id WHERE INVOICEMASTER.INVOICE_BALANCE > 0 UNION ALL SELECT PURCHASEMASTER.BILL_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(BILL_RCM,0) = 'FALSE' THEN PURCHASEMASTER.BILL_GRANDTOTAL ELSE (CASE WHEN ISNULL(BILL_SCREENTYPE,'LINE GST') = 'LINE GST' THEN BILL_TOTALTAXABLEAMT ELSE BILL_SUBTOTAL END) END)  AS BILLAMT, PURCHASEMASTER.BILL_BALANCE AS BALAMT, 'PURCHASE' AS TYPE, PURCHASEMASTER.BILL_PARTYBILLNO AS REFNO, PURCHASEMASTER.BILL_NO AS BILLNO, PURCHASEMASTER.BILL_PARTYBILLDATE AS DATE, PURCHASEMASTER.BILL_CMPID AS CMPID, PURCHASEMASTER.BILL_LOCATIONID AS LOCATIONID, PURCHASEMASTER.BILL_YEARID AS YEARID, ISNULL(PURCHASEMASTER.BILL_DISPUTE,0) AS DISPUTE,0.00 AS ADJUSTAMT FROM PURCHASEMASTER INNER JOIN LEDGERS ON PURCHASEMASTER.BILL_LEDGERID = LEDGERS.Acc_id WHERE PURCHASEMASTER.BILL_BALANCE > 0 UNION ALL SELECT NONPURCHASE.NP_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(NP_RCM,0) = 'TRUE' THEN NP_TOTALTAXABLEAMT ELSE NONPURCHASE.NP_GRANDTOTAL END) AS BILLAMT, NONPURCHASE.NP_BALANCE AS BALAMT, 'NONPURCHASE' AS TYPE, NONPURCHASE.NP_REFNO AS REFNO, NONPURCHASE.NP_NO AS BILLNO, NONPURCHASE.NP_PARTYBILLDATE AS DATE, NONPURCHASE.NP_CMPID AS CMPID, NONPURCHASE.NP_LOCATIONID AS LOCATIONID, NONPURCHASE.NP_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM NONPURCHASE INNER JOIN LEDGERS ON NONPURCHASE.NP_LEDGERID = LEDGERS.Acc_id  WHERE NONPURCHASE.NP_BALANCE > 0 UNION ALL SELECT CREDITNOTEMASTER.CN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, (CASE WHEN ISNULL(CN_RCM,0) = 'TRUE' THEN CN_SUBTOTAL ELSE CREDITNOTEMASTER.CN_GTOTAL END) AS BILLAMT, CREDITNOTEMASTER.CN_BALANCE AS BALAMT, 'CREDITNOTE' AS TYPE, CREDITNOTEMASTER.CN_PARTYREFNO AS REFNO, CREDITNOTEMASTER.CN_NO AS BILLNO, CREDITNOTEMASTER.CN_date AS DATE, CREDITNOTEMASTER.CN_CMPID AS CMPID, CREDITNOTEMASTER.CN_LOCATIONID AS LOCATIONID, CREDITNOTEMASTER.CN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM CREDITNOTEMASTER INNER JOIN LEDGERS ON CREDITNOTEMASTER.CN_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN CREDITNOTEMASTER_BILLDESC ON CREDITNOTEMASTER.CN_NO = CREDITNOTEMASTER_BILLDESC.CN_NO AND CREDITNOTEMASTER.CN_registerid = CREDITNOTEMASTER_BILLDESC.CN_REGISTERID WHERE CREDITNOTEMASTER.CN_BALANCE > 0 AND CN_date >= '07/01/2017'AND CN_BILLNO = '' AND CREDITNOTEMASTER_BILLDESC.CN_NO IS NULL UNION ALL SELECT CREDITNOTEMASTER.CN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, CREDITNOTEMASTER_BILLDESC.CN_AMT AS BILLAMT, CREDITNOTEMASTER_BILLDESC.CN_BALANCE AS BALAMT, 'CREDITNOTE' AS TYPE, CREDITNOTEMASTER.CN_PARTYREFNO AS REFNO, CREDITNOTEMASTER.CN_NO AS BILLNO, CREDITNOTEMASTER.CN_date AS DATE, CREDITNOTEMASTER.CN_CMPID AS CMPID, CREDITNOTEMASTER.CN_LOCATIONID AS LOCATIONID, CREDITNOTEMASTER.CN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE ,0.00 AS ADJUSTAMT FROM CREDITNOTEMASTER INNER JOIN LEDGERS ON CREDITNOTEMASTER.CN_LEDGERID = LEDGERS.Acc_id INNER JOIN CREDITNOTEMASTER_BILLDESC ON CREDITNOTEMASTER.CN_NO = CREDITNOTEMASTER_BILLDESC.CN_NO AND CREDITNOTEMASTER.CN_registerid = CREDITNOTEMASTER_BILLDESC.CN_REGISTERID WHERE CREDITNOTEMASTER_BILLDESC.CN_BALANCE > 0 AND CREDITNOTEMASTER_BILLDESC.CN_PAYTYPE = 'New Ref' UNION ALL SELECT 'SR-' + CAST(SALERETURN.SALRET_NO AS VARCHAR(50)) AS SRNO, LEDGERS.Acc_cmpname AS NAME, SALERETURN_BILLDESC.SALRET_AMT AS BILLAMT, SALERETURN_BILLDESC.SALRET_BALANCE AS BALAMT, 'SALERETURN' AS TYPE, SALERETURN.SALRET_PARTYREFNO AS REFNO, SALERETURN.SALRET_NO AS BILLNO, SALERETURN.SALRET_date AS DATE, SALERETURN.SALRET_CMPID AS CMPID, SALERETURN.SALRET_LOCATIONID AS LOCATIONID, SALERETURN.SALRET_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM SALERETURN INNER JOIN LEDGERS ON SALERETURN.SALRET_LEDGERID = LEDGERS.Acc_id INNER JOIN SALERETURN_BILLDESC ON SALERETURN.SALRET_NO = SALERETURN_BILLDESC.SALRET_NO AND SALERETURN.SALRET_YEARID = SALERETURN_BILLDESC.SALRET_YEARID WHERE SALERETURN_BILLDESC.SALRET_BALANCE > 0 AND SALERETURN_BILLDESC.SALRET_PAYTYPE = 'New Ref' UNION ALL SELECT DEBITNOTEMASTER.DN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, DEBITNOTEMASTER.DN_GTOTAL AS BILLAMT, DEBITNOTEMASTER.DN_BALANCE AS BALAMT, 'DEBITNOTE' AS TYPE, DEBITNOTEMASTER.DN_initials AS REFNO, DEBITNOTEMASTER.DN_NO AS BILLNO, DEBITNOTEMASTER.DN_date AS DATE, DEBITNOTEMASTER.DN_CMPID AS CMPID, DEBITNOTEMASTER.DN_LOCATIONID AS LOCATIONID, DEBITNOTEMASTER.DN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM DEBITNOTEMASTER INNER JOIN LEDGERS ON DEBITNOTEMASTER.DN_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN DEBITNOTEMASTER_BILLDESC ON DEBITNOTEMASTER.DN_NO = DEBITNOTEMASTER_BILLDESC.DN_NO AND DEBITNOTEMASTER.DN_registerid = DEBITNOTEMASTER_BILLDESC.DN_REGISTERID WHERE DEBITNOTEMASTER.DN_BALANCE > 0 AND DN_date >= '07/01/2017' AND DN_BILLNO = '' AND DEBITNOTEMASTER_BILLDESC.DN_NO IS NULL UNION ALL SELECT DEBITNOTEMASTER.DN_initials AS SRNO, LEDGERS.Acc_cmpname AS NAME, DEBITNOTEMASTER_BILLDESC.DN_AMT AS BILLAMT, DEBITNOTEMASTER_BILLDESC.DN_BALANCE AS BALAMT, 'DEBITNOTE' AS TYPE, DEBITNOTEMASTER.DN_initials AS REFNO, DEBITNOTEMASTER.DN_NO AS BILLNO, DEBITNOTEMASTER.DN_date AS DATE, DEBITNOTEMASTER.DN_CMPID AS CMPID, DEBITNOTEMASTER.DN_LOCATIONID AS LOCATIONID, DEBITNOTEMASTER.DN_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM DEBITNOTEMASTER INNER JOIN LEDGERS ON DEBITNOTEMASTER.DN_LEDGERID = LEDGERS.Acc_id INNER JOIN DEBITNOTEMASTER_BILLDESC ON DEBITNOTEMASTER.DN_NO = DEBITNOTEMASTER_BILLDESC.DN_NO AND DEBITNOTEMASTER.DN_registerid = DEBITNOTEMASTER_BILLDESC.DN_REGISTERID WHERE DEBITNOTEMASTER_BILLDESC.DN_BALANCE > 0 and DEBITNOTEMASTER_BILLDESC.DN_PAYTYPE = 'New Ref' UNION ALL SELECT PURCHASERETURN.PR_INITIALS AS SRNO, LEDGERS.Acc_cmpname AS NAME, PURCHASERETURN_BILLDESC.PR_AMT AS BILLAMT, PURCHASERETURN_BILLDESC.PR_BALANCE AS BALAMT, 'PURCHASERETURN' AS TYPE, PURCHASERETURN.PR_initials AS REFNO, PURCHASERETURN.PR_NO AS BILLNO, PURCHASERETURN.PR_date AS DATE, PURCHASERETURN.PR_CMPID AS CMPID, PURCHASERETURN.PR_LOCATIONID AS LOCATIONID, PURCHASERETURN.PR_YEARID AS YEARID, CAST (0 AS BIT) AS DISPUTE,0.00 AS ADJUSTAMT FROM PURCHASERETURN INNER JOIN LEDGERS ON PURCHASERETURN.PR_LEDGERID = LEDGERS.Acc_id INNER JOIN PURCHASERETURN_BILLDESC ON PURCHASERETURN.PR_NO = PURCHASERETURN_BILLDESC.PR_NO AND PURCHASERETURN.PR_YEARID = PURCHASERETURN_BILLDESC.PR_YEARID WHERE PURCHASERETURN_BILLDESC.PR_BALANCE > 0 and PURCHASERETURN_BILLDESC.PR_PAYTYPE = 'New Ref') AS T ", TEMPCONDITION & " AND T.BALAMT > 0 AND T.YEARID =" & YearId & " ORDER BY T.TYPE, T.BILLNO ")
            End If
    ...
    Private Sub gridrec_CellValueChanging(sender As Object, e As CellValueChangedEventArgs) Handles gridrec.CellValueChanging
        Try
            If e.Column.FieldName = "CHK" Then
                gridrec.PostEditor() ' Commits edit
                Dim balAmtObj = gridrec.GetRowCellValue(e.RowHandle, "BALAMT")
                If e.Value = True Then
                    Dim balAmt As Decimal = 0
                    If balAmtObj IsNot DBNull.Value AndAlso Decimal.TryParse(balAmtObj.ToString(), balAmt) Then
                        gridrec.SetRowCellValue(e.RowHandle, "ADJUSTAMT", balAmt)
                    Else
                        gridrec.SetRowCellValue(e.RowHandle, "ADJUSTAMT", 0)
                    End If
                Else
                    gridrec.SetRowCellValue(e.RowHandle, "ADJUSTAMT", 0)
                End If
                gridrec.RefreshRow(e.RowHandle)
            End If
        Catch ex As Exception
            Throw ex
        End Try
