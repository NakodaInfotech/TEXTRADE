...
Public Class OpeningStockGrey
    ...
    Private Sub OpeningStock_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Dim dttable As DataTable = OBJCMN.Execute_Any_String(" SELECT  STOCKMASTER_GREY.SMGREY_TYPE AS TYPE, STOCKMASTER_GREY.SMGREY_DATE AS DATE, ISNULL(STOCKMASTER_GREY.SMGREY_GRIDSRNO, 0) AS GRIDSRNO, STOCKMASTER_GREY.SMGREY_NO AS SMNO, ISNULL(STOCKMASTER_GREY.SMGREY_LOTNO, '') AS LOTNO, ISNULL(PIECETYPEMASTER.PIECETYPE_name, '') AS PIECETYPE, ISNULL(ITEMMASTER.item_name, '') AS MERCHANT, ISNULL(QUALITYMASTER.QUALITY_name, '') AS QUALITY, ISNULL(DESIGNMASTER.DESIGN_NO, '') AS DESIGN, ISNULL(COLORMASTER.COLOR_name, '') AS COLOR, ISNULL(PROCESSMASTER.PROCESS_NAME, '') AS PROCESS, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME,  ISNULL(TONAME.Acc_cmpname, '') AS TONAME, ISNULL(STOCKMASTER_GREY.SMGREY_BILLNO, '') AS BILLNO, ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN, STOCKMASTER_GREY.SMGREY_CUT AS CUT, ISNULL(STOCKMASTER_GREY.SMGREY_WT, 0) AS WT, ISNULL(UNITMASTER.unit_abbr, '') AS UNIT, ISNULL(STOCKMASTER_GREY.SMGREY_PCS, 0) AS PCS, ISNULL(STOCKMASTER_GREY.SMGREY_MTRS, 0) AS MTRS, ISNULL(STOCKMASTER_GREY.SMGREY_PER, '') AS PER,ISNULL(STOCKMASTER_GREY.SMGREY_RATE, 0) AS RATE, ISNULL(STOCKMASTER_GREY.SMGREY_AMOUNT, 0) AS AMOUNT, ISNULL(STOCKMASTER_GREY.SMGREY_REMARKS, '') AS REMARKS, ISNULL(STOCKMASTER_GREY.SMGREY_LRNO, '') AS LRNO, ISNULL(STOCKMASTER_GREY.SMGREY_BARCODE, '') AS BARCODE, ISNULL(STOCKMASTER_GREY.SMGREY_DONE, 0) AS DONE, ISNULL(STOCKMASTER_GREY.SMGREY_LOTCOMPLETED, 0) AS LOTCOMPLETED, ISNULL(STOCKMASTER_GREY.SMGREY_OUTPCS, 0) AS OUTPCS, ISNULL(STOCKMASTER_GREY.SMGREY_OUTMTRS, 0) AS OUTMTRS, STOCKMASTER_GREY.SMGREY_CMPID AS CMPID, STOCKMASTER_GREY.SMGREY_LOCATIONID AS LOCATIONID, STOCKMASTER_GREY.SMGREY_YEARID AS YEARID, ISNULL(SHELFMASTER.SHELF_NAME, '') AS SHELF, ISNULL(RACKMASTER.RACK_NAME, '') AS RACK, ISNULL(STOCKMASTER_GREY.SMGREY_PARTYCHALLANNO,'') AS PARTYCHNO, ISNULL(STOCKMASTER_GREY.SMGREY_BALENO,'') AS BALENO, ISNULL(STOCKMASTER_GREY.SMGREY_GRIDREMARKS,'') AS GRIDREMARKS, ISNULL(STOCKMASTER_GREY.SMGREY_DYEINGJOB,'DYEING') AS DYEINGJOB, ISNULL(STOCKMASTER_GREY.SMGREY_ADDLESS,0) AS ADDLESS, ISNULL(STOCKMASTER_GREY.SMGREY_NETTRATE,0) AS NETTRATE FROM STOCKMASTER_GREY LEFT OUTER JOIN SHELFMASTER ON STOCKMASTER_GREY.SMGREY_SHELFID = SHELFMASTER.SHELF_ID LEFT OUTER JOIN RACKMASTER ON STOCKMASTER_GREY.SMGREY_RACKID = RACKMASTER.RACK_ID LEFT OUTER JOIN LEDGERS AS TONAME ON STOCKMASTER_GREY.SMGREY_LEDGERIDTO = TONAME.Acc_id LEFT OUTER JOIN DESIGNMASTER ON STOCKMASTER_GREY.SMGREY_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN GODOWNMASTER ON STOCKMASTER_GREY.SMGREY_GODOWNID = GODOWNMASTER.GODOWN_id LEFT OUTER JOIN PIECETYPEMASTER ON STOCKMASTER_GREY.SMGREY_PIECETYPEID = PIECETYPEMASTER.PIECETYPE_id LEFT OUTER JOIN QUALITYMASTER ON STOCKMASTER_GREY.SMGREY_QUALITYID = QUALITYMASTER.QUALITY_id LEFT OUTER JOIN PROCESSMASTER ON STOCKMASTER_GREY.SMGREY_PROCESSID = PROCESSMASTER.PROCESS_ID LEFT OUTER JOIN LEDGERS ON STOCKMASTER_GREY.SMGREY_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN UNITMASTER ON STOCKMASTER_GREY.SMGREY_UNITID = UNITMASTER.unit_id LEFT OUTER JOIN COLORMASTER ON STOCKMASTER_GREY.SMGREY_COLORID = COLORMASTER.COLOR_id LEFT OUTER JOIN ITEMMASTER ON STOCKMASTER_GREY.SMGREY_ITEMID = ITEMMASTER.item_id WHERE STOCKMASTER_GREY.SMGREY_TYPE = '" & FRMSTRING & "' AND STOCKMASTER_GREY.SMGREY_YEARID = " & YearId & " ORDER BY SMGREY_NO", "", "")
    ...
    Sub SAVE()
        Try
            ...
            Dim OBJSM As New ClsOpeningStockGrey
        ...
        If cmbpiecetype.Text.Trim <> "" And cmbmerchant.Text.Trim <> "" And Val(txtpcs.Text.Trim) > 0 Then
            ...
            ElseIf cmbtype.Text = "JOBBERSTOCK" Then
                If cmbtoname.Text.Trim <> "" Then
                    ...
                    Dim OBJCMN As New ClsCommon
                    Dim DT As DataTable
                    'DT = OBJCMN.search("SMGREY_BILLNO AS BILLNO", "", " STOCKMASTER ", " AND SMGREY_BILLNO = " & Val(TXTBILLNO.Text.Trim) & " AND SMGREY_NO <> " & Val(TXTNO.Text.Trim) & " AND SMGREY_YEARID = " & YearId)
                    'If DT.Rows.Count > 0 Then
