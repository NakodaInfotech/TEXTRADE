...
Public Class PaymentMaster
    ...
    Sub GETBALANCE()
        Try
            ...
            Dim DT As DataTable = OBJCMN.SEARCH("(CASE WHEN DR > 0 THEN 'Dr'  ELSE 'Cr' END) AS SALEBAL, isnull(ACC_CRLIMIT,0) AS CRLIMIT, (CASE WHEN DR > 0 THEN DR ELSE CR END) AS BALANCE ", "", "  TRIALBALANCE INNER JOIN LEDGERS ON TRIALBALANCE.LEDGERID = LEDGERS.Acc_ID ", " AND TRIALBALANCE.NAME = '" & cmbaccname.Text.Trim & "' AND TRIALBALANCE.YEARID = " & YearId)
            ...
            DT = OBJCMN.SEARCH("(CASE WHEN DR > 0 THEN 'Dr'  ELSE 'Cr' END) AS SALEBAL, isnull(ACC_CRLIMIT,0) AS CRLIMIT, (CASE WHEN DR > 0 THEN DR ELSE CR END) AS BALANCE ", "", "  TRIALBALANCE INNER JOIN LEDGERS ON TRIALBALANCE.LEDGERID = LEDGERS.Acc_ID ", " AND TRIALBALANCE.NAME = '" & cmbname.Text.Trim & "' AND TRIALBALANCE.YEARID = " & YearId)
            If DT.Rows.Count > 0 Then
        ...
        Try
            ...
            If EDIT = False Then
                ...
                If ClientName = "SIDDHGIRI" Then
                    If MsgBox("Wish to Print Chq?", MsgBoxStyle.YesNo) = vbYes Then
...
LINE1:
    ...
    Private Sub cmbaccname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbaccname.Validated
        ...
        If EDIT = False And ClientName <> "VALIANT" Then
            ...
            If ClientName = "SAKARIA" Or ClientName = "BARKHA" Or ClientName = "MAHAJAN" Or ClientName = "SHUBHI" Or ClientName = "SUBHLAXMI" Or ClientName = "KREEVE" Or ClientName = "MAHAVIRPOLYCOT" Or ClientName = "NAYRA" Or ClientName = "VINAYAK" Then
            ...
            ElseIf ClientName = "MAHAVIR" Or ClientName = "KCRAYON" Or ClientName = "SONAL" Or ClientName = "ARIHANTGARMENT" Then
    ...
    Private Sub txtamt_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtamt.Validating
        ...
        Try
            ...
            If txtsrno.Text.Trim.Length > 0 Then
                ...
                If cmbpaytype.Text = "New Ref" Then txtnarr.Text = payreginitial & "-" & Val(txtaccno.Text.Trim)
                fillgrid()
        ...
        Try
            If cmbregister.Text.Trim.Length > 0 And edit = False Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    If dt.Rows(0).Item("NAME") <> "" Then cmbaccname.Text = dt.Rows(0).Item("NAME")
    ...
    Sub PRINTREPORT()
        Try
            If MsgBox("Wish to Print Advice?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                ...
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.Execute_Any_String("UPDATE PAYMENTMASTER SET PAYMENT_PRINT = 1 FROM PAYMENTMASTER INNER JOIN REGISTERMASTER ON PAYMENT_REGISTERID = REGISTER_ID WHERE PAYMENT_NO = " & Val(txtaccno.Text.Trim) & " and REGISTERMASTER.REGISTER_NAME = '" & cmbregister.Text.Trim & "' AND PAYMENT_YEARID = " & YearId, "", "")
            ...
            If ClientName = "SOFTAS" Or ClientName = "SUPRIYA" Or ClientName = "SIMPLEX" Or ClientName = "SIDDHGIRI" Then
                If MsgBox("Wish to Print Bill Nos Behind Chq?", MsgBoxStyle.YesNo) = vbYes Then
                    Dim OBJCHQPRINT As New payment_advice
                    OBJCHQPRINT.payno = Val(txtaccno.Text.Trim)
                    OBJCHQPRINT.REGNAME = cmbregister.Text.Trim
                    OBJCHQPRINT.FRMSTRING = "CHQPRINTBACK"
                    OBJCHQPRINT.MdiParent = MDIMain
                    OBJCHQPRINT.Show()
                End If
            End If
...
1:
    ...
    Private Sub TXTCHQNO_Validated(sender As Object, e As EventArgs) Handles TXTCHQNO.Validated
        If ClientName = "SURYODAYA" Then
            ...
            Try
                ...
                If TXTCHQNO.Text.Trim <> "" Then
                    GETBALANCE()
                    Dim OBJCMN As New ClsCommon
                    Dim DT As DataTable = OBJCMN.SEARCH("ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(ACCNAME.Acc_cmpname, '') AS BANKNAME, ISNULL(CHQENTERIES_DESC.CHQ_AMT, 0) AS AMOUNT, CHQENTERIES_DESC.CHQ_CHQNO, CHQENTERIES_DESC.CHQ_YEARID", "", "LEDGERS INNER JOIN LEDGERS AS ACCNAME ON LEDGERS.Acc_yearid = ACCNAME.Acc_yearid RIGHT OUTER JOIN CHQENTERIES_DESC ON ACCNAME.Acc_id = CHQENTERIES_DESC.CHQ_BANKID AND LEDGERS.Acc_id = CHQENTERIES_DESC.CHQ_LEDGERID", " AND CHQ_CHQNO = '" & TXTCHQNO.Text.Trim & "'  AND CHQ_YEARID = " & YearId)
                    If DT.Rows.Count > 0 Then
                        If cmbaccname.Text.Trim = "" Then cmbaccname.Text = DT.Rows(0).Item("BANKNAME")
                        If cmbname.Text.Trim = "" Then cmbname.Text = DT.Rows(0).Item("NAME")
                        If txtchqamt.Text.Trim = "" Then txtchqamt.Text = DT.Rows(0).Item("AMOUNT")
                    End If
                End If
                If txtbillno.Text.Trim = "" And cmbname.Text.Trim <> "" Then
                    fillgridPURCHASE()
    ...
    Private Sub cmbregister_Validated(sender As Object, e As EventArgs) Handles cmbregister.Validated
        Try
            'GET DEFAULT SCREENTYPE
            If EDIT = False And cmbregister.Text.Trim <> "" Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.SEARCH("ISNULL(LEDGERS.Acc_cmpname,'') AS BANKNAME", "", " DEFAULTBANK LEFT OUTER JOIN REGISTERMASTER ON DEFAULTBANK.DTYPE_REGID = REGISTERMASTER.register_id LEFT OUTER JOIN LEDGERS ON DEFAULTBANK.DTYPE_BANKID = LEDGERS.Acc_id", " AND REGISTER_NAME = '" & cmbregister.Text & "' AND DTYPE_YEARID = " & YearId)
