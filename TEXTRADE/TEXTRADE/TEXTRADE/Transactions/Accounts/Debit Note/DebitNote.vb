...
Public Class DebitNote
    ...
    Sub CREATEAGENCYCN()
        Try
            ...
            Dim OBJCMN As New ClsCommon
            Dim TEMPYEARID, TEMPCMPID, TEMPLEDGERID As Integer
            ...
            'CHECK WHETHER DEBITLEDGER NAME IS PRESENT OR NOT, IF NOT PRESENT THEN ADD NEW 
            TEMPDT = OBJCMN.SEARCH("ACC_ID AS LEDGERID ", "", " LEDGERS ", " AND ACC_CMPNAME = '" & CMBCREDITLEDGER.Text.Trim & "' AND ACC_YEARID = " & TEMPYEARID)
            If TEMPDT.Rows.Count > 0 Then TEMPLEDGERID = TEMPDT.Rows(0).Item("LEDGERID") Else CREATELEDGER(CMBCREDITLEDGER.Text.Trim, TEMPCMPID, TEMPYEARID)
            ...
            ALPARAVAL.Add(TXTBILLNO.Text.Trim)
            ALPARAVAL.Add(TXTPARTYBILLNO.Text.Trim)
            ALPARAVAL.Add("ABHEE FABRICS LLP [ BUYER ]")    'BUYERNAME 
            ALPARAVAL.Add(CMBAGENT.Text.Trim)
            ALPARAVAL.Add(TXTSACCODE.Text.Trim)
            ALPARAVAL.Add(CMBNAME.Text.Trim)                'SELLERNAME
            ALPARAVAL.Add(CMBCREDITLEDGER.Text.Trim)        'CMBPACKING (DEBIT TO)
            ...
            ALPARAVAL.Add(Val(TXTTOTALPURAMT.Text.Trim))    'TXTTOTALSALEAMT
            ALPARAVAL.Add(Val(TXTTOTALTAXAMT.Text.Trim))
            ALPARAVAL.Add(Val(TXTTOTALOTHERCHGSAMT.Text.Trim))
            ALPARAVAL.Add(Val(TXTCHARGES.Text.Trim))
            ...
            Else
                ALPARAVAL.Add(DBNull.Value)
            End If
            ALPARAVAL.Add("LLP DEBIT NOTE NO - " & Val(TXTDNNO.Text.Trim))  'TXTSPECIALREMARKS
            If CHKCD.Checked = True Then ALPARAVAL.Add(1) Else ALPARAVAL.Add(0)
            ALPARAVAL.Add(CMBCOSTCENTERNAME.Text.Trim)
            ALPARAVAL.Add(TXTCOMPLAINT.Text.Trim)
            ALPARAVAL.Add(TXTCOMPLAINTBY.Text.Trim)
    ...
    Private Sub CMBNAME_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles CMBNAME.Validated
        Try
            If CMBNAME.Text.Trim <> "" Then
                ...
                'GET INVOICE DETAILS 
                DT = OBJCMN.SEARCH("*", "", " (SELECT CAST (0 AS BIT) AS CHK, CAST(PURCHASEMASTER.BILL_INITIALS AS VARCHAR(100)) AS BILLINITIALS, PURCHASEMASTER.BILL_DATE AS BILLDATE, PURCHASEMASTER.BILL_GRANDTOTAL AS BILLAMT, PURCHASEMASTER.BILL_NO AS BILLNO, LEDGERS.Acc_cmpname AS NAME, ISNULL(PURCHASEMASTER.BILL_TOTALMTRS,0) AS TOTALMTRS, ISNULL(PURCHASEMASTER.BILL_TOTALQTY,0) AS TOTALPCS, ISNULL(PURCHASEMASTER.BILL_SUBTOTAL,0) AS TAXABLEAMT  FROM PURCHASEMASTER INNER JOIN REGISTERMASTER ON PURCHASEMASTER.BILL_REGISTERID = REGISTERMASTER.register_id AND PURCHASEMASTER.BILL_CMPID = REGISTERMASTER.register_cmpid AND  PURCHASEMASTER.BILL_LOCATIONID = REGISTERMASTER.register_locationid AND PURCHASEMASTER.BILL_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON PURCHASEMASTER.BILL_CMPID = LEDGERS.Acc_cmpid AND PURCHASEMASTER.BILL_LOCATIONID = LEDGERS.Acc_locationid AND PURCHASEMASTER.BILL_YEARID = LEDGERS.Acc_yearid AND  PURCHASEMASTER.BILL_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN JOURNALMASTER ON PURCHASEMASTER.BILL_INITIALS = JOURNALMASTER.journal_refno AND PURCHASEMASTER.BILL_YEARID = JOURNALMASTER.journal_yearid WHERE (PURCHASEMASTER.BILL_YEARID = " & YearId & ") AND LEDGERS.Acc_cmpname = '" & CMBNAME.Text.Trim & "'  UNION ALL SELECT CAST (0 AS BIT) AS CHK, CAST(OPENINGBILL.BILL_INITIALS AS VARCHAR(100)) AS BILLINITIALS, OPENINGBILL.BILL_DATE AS BILLDATE, OPENINGBILL.BILL_AMT AS BILLAMT, OPENINGBILL.BILL_NO AS BILLNO, LEDGERS.Acc_cmpname AS NAME,  ISNULL(OPENINGBILL.BILL_PCS,0) AS TOTALPCS, ISNULL(OPENINGBILL.BILL_MTRS,0) AS TOTALMTRS,  ISNULL(OPENINGBILL.BILL_TAXABLEAMT,0) AS TAXABLEAMT FROM OPENINGBILL INNER JOIN REGISTERMASTER ON OPENINGBILL.BILL_REGISTERID = REGISTERMASTER.register_id AND OPENINGBILL.BILL_CMPID = REGISTERMASTER.register_cmpid AND OPENINGBILL.BILL_LOCATIONID = REGISTERMASTER.register_locationid AND OPENINGBILL.BILL_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON OPENINGBILL.BILL_CMPID = LEDGERS.Acc_cmpid AND OPENINGBILL.BILL_LOCATIONID = LEDGERS.Acc_locationid AND OPENINGBILL.BILL_YEARID = LEDGERS.Acc_yearid AND  OPENINGBILL.BILL_LEDGERID = LEDGERS.Acc_id WHERE (OPENINGBILL.bill_YEARID = " & YearId & ") AND LEDGERS.Acc_cmpname = '" & CMBNAME.Text.Trim & "' ) AS T  ", " ORDER BY BILLDATE ,BILLNO  ")
                If DT.Rows.Count > 0 Then
...
LINE1:                      'GET INVPRINTTINITIALS | PCS | MTRS | BILLAMT
                        ...
                        If DT.Rows.Count > 0 Then
                            ...
                            If DT1.Rows.Count > 0 Then
                                CMBSACDESC.Text = DT1.Rows(0).Item("HSNDESC")
                                TXTSACCODE.Text = DT1.Rows(0).Item("HSNCODE")
                                Call CMBSACDESC_Validated(sender, e)
                            End If
