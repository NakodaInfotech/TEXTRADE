...
Public Class CreditNoteDetails
    ...
    Sub fillgrid(ByVal tempcondition)
        Try
            ...
            Dim dt As DataTable = objclsCMST.search(" CREDITNOTEMASTER.CN_no AS SRNO, CREDITNOTEMASTER.CN_date AS DATE, LEDGERS.Acc_cmpname AS NAME, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENT,  (CASE WHEN ISNULL(CREDITNOTEMASTER.CN_BILLNO,'') <> '' THEN ISNULL(CREDITNOTEMASTER.CN_BILLNO,'') ELSE ISNULL(STUFF((SELECT ', ' + CN_BILLINITIALS  [text()] FROM CREDITNOTEMASTER_BILLDESC AS A WHERE A.CN_NO = CREDITNOTEMASTER.CN_NO AND A.CN_YEARID = CREDITNOTEMASTER.CN_YEARID FOR XML PATH(''), TYPE) .value('.','NVARCHAR(MAX)'),1,2,' '),'') END) AS BILLNO, DEBITLEDGERS.Acc_cmpname AS DEBITNAME, CREDITNOTEMASTER.CN_GTOTAL AS AMT, CREDITNOTEMASTER.CN_remarks AS REMARKS, ISNULL(STATEMASTER.state_name, '') AS STATENAME, ISNULL(STATEMASTER.state_remark, '') AS STATECODE, ISNULL(LEDGERS.ACC_GSTIN, '') AS GSTIN, ISNULL(HSNMASTER.HSN_ITEMDESC, '') AS HSNITEMDESC, ISNULL(HSNMASTER.HSN_CODE, '') AS HSNCODE, ISNULL(CREDITNOTEMASTER.CN_TOTALAMT, 0) AS BILLAMT, ISNULL(CREDITNOTEMASTER.CN_CHARGES, 0) AS CHGS, ISNULL(CREDITNOTEMASTER.CN_SUBTOTAL, 0) AS SUBTOTAL, ISNULL(CREDITNOTEMASTER.CN_CGSTAMT, 0) AS TOTALCGSTAMT, ISNULL(CREDITNOTEMASTER.CN_SGSTAMT, 0) AS TOTALSGSTAMT,  ISNULL(CREDITNOTEMASTER.CN_IGSTAMT, 0) AS TOTALIGSTAMT, ISNULL(CREDITNOTEMASTER.CN_ROUNDOFF, 0) AS ROUNDOFF, ISNULL(CREDITNOTEMASTER.CN_PARTYREFNO, '') AS PARTYREFNO, ISNULL(CREDITNOTEMASTER.CN_RCM, 0) AS RCM, ISNULL(CREDITNOTEMASTER.CN_NOGSTR1, 0) AS NOGSTR1, ISNULL(DELIVERYLEDGERS.Acc_cmpname, '') AS DELIVERYAT, ISNULL(GROUPMASTER.group_name, '') AS GROUPNAME, ISNULL(CREDITNOTEMASTER.CN_IRNNO, '') AS IRNNO, ISNULL(CREDITNOTEMASTER.CN_ACKNO, '') AS CNACKNO, CREDITNOTEMASTER.CN_ACKDATE AS CNACKDATE, ISNULL(LEDGERS.ACC_WHATSAPPNO, '0') AS PARTYWHATSAAP, ISNULL(LEDGERS.Acc_email, '') AS PARTYEMAIL, ISNULL(AGENTLEDGERS.Acc_email, '') AS AGENTEMAIL, ISNULL(AGENTLEDGERS.ACC_WHATSAPPNO, '0') AS AGENTWHATSAAP, CREDITNOTEMASTER.CN_SPECIALREMARKS AS SPECIALREMARK, ISNULL(COSTCENTERMASTER.COSTCENTER_name, '') AS COSTCENTERNAME , ISNULL(USERMASTER.User_Name,'') AS CREATEDBY, ISNULL(REGISTERMASTER.register_name, '') AS REGNAME, ISNULL(CREDITNOTEMASTER.CN_COMPLAINT,'') AS COMPLAINT, ISNULL(CREDITNOTEMASTER.CN_COMPLAINTBY,'') AS COMPLAINTBY, ISNULL(CREDITNOTEMASTER.CN_COMPLAINTDATE,'') AS COMPLAINTDATE ", "", "  CREDITNOTEMASTER INNER JOIN REGISTERMASTER ON CREDITNOTEMASTER.CN_registerid = REGISTERMASTER.register_id INNER JOIN LEDGERS ON CREDITNOTEMASTER.CN_LEDGERID = LEDGERS.Acc_id INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON CREDITNOTEMASTER.CN_AGENTID = AGENTLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS DELIVERYLEDGERS ON CREDITNOTEMASTER.CN_DELIVERYATID = DELIVERYLEDGERS.Acc_id INNER JOIN LEDGERS AS DEBITLEDGERS ON CREDITNOTEMASTER.CN_DEBITLEDGERID = DEBITLEDGERS.Acc_id LEFT OUTER JOIN COSTCENTERMASTER ON CREDITNOTEMASTER.CN_COSTCENTERNAMEID = COSTCENTERMASTER.COSTCENTER_id LEFT OUTER JOIN HSNMASTER ON CREDITNOTEMASTER.CN_HSNCODEID = HSNMASTER.HSN_ID LEFT OUTER JOIN STATEMASTER ON LEDGERS.Acc_stateid = STATEMASTER.state_id LEFT OUTER JOIN USERMASTER ON CREDITNOTEMASTER.CN_userid = USERMASTER.User_id", tempcondition & " AND CN_YEARID = " & YearId & " order by dbo.CREDITNOTEMASTER.CN_NO")
