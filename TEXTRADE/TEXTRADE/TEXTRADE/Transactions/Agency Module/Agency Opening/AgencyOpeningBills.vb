...
Public Class AgencyOpeningBills
    ...
    Sub FILLGRIDOPENING()
        Try
            Dim OBJCMN As New ClsCommon
            Dim DT As DataTable = OBJCMN.SEARCH(" AGENCYOPENINGBILL.ABILL_GRIDSRNO AS GRIDSRNO, AGENCYOPENINGBILL.ABILL_TYPE AS BILLTYPE, AGENCYOPENINGBILL.ABILL_NO AS BILLNO, AGENCYOPENINGBILL.ABILL_YEAR AS YEAR, AGENCYOPENINGBILL.ABILL_DATE AS BILLDATE, AGENCYOPENINGBILL.ABILL_CRDAYS AS CRDAYS, AGENCYOPENINGBILL.ABILL_DUEDATE AS DUEDATE, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENT, AGENCYOPENINGBILL.ABILL_NARRATION AS NARRATION, AGENCYOPENINGBILL.ABILL_DISPUTE AS DISPUTE, AGENCYOPENINGBILL.ABILL_AMT AS AMT, AGENCYOPENINGBILL.ABILL_AMTPAIDREC AS AMTPAIDREC, AGENCYOPENINGBILL.ABILL_EXTRAAMT AS EXTRAAMT, AGENCYOPENINGBILL.ABILL_RETURN AS [RETURN], AGENCYOPENINGBILL.ABILL_BALANCE AS BALANCE, ISNULL(AGENCYOPENINGBILL.ABILL_PRINTINITIALS, '') AS PRINTINITIALS, ISNULL(DELIVERYLEDGERS.Acc_cmpname, '') AS DELIVERYAT, ISNULL(AGENCYOPENINGBILL.ABILL_PCS, 0) AS PCS, ISNULL(AGENCYOPENINGBILL.ABILL_MTRS, 0) AS MTRS, ISNULL(AGENCYOPENINGBILL.ABILL_TOTALAMT, 0) AS TOTALAMT, ISNULL(AGENCYOPENINGBILL.ABILL_CHARGES, 0) AS CHARGES, ISNULL(AGENCYOPENINGBILL.ABILL_TAXABLEAMT, 0) AS TAXABLEAMT, ISNULL(AGENCYOPENINGBILL.ABILL_CGSTPER, 0) AS CGSTPER, ISNULL(AGENCYOPENINGBILL.ABILL_CGSTAMT, 0) AS CGSTAMT, ISNULL(AGENCYOPENINGBILL.ABILL_SGSTPER, 0) AS SGSTPER, ISNULL(AGENCYOPENINGBILL.ABILL_SGSTAMT, 0) AS SGSTAMT, ISNULL(AGENCYOPENINGBILL.ABILL_IGSTPER, 0) AS IGSTPER, ISNULL(AGENCYOPENINGBILL.ABILL_IGSTAMT, 0) AS IGSTAMT, ISNULL(AGENCYOPENINGBILL.ABILL_GRANDTOTAL, 0) AS GRANDTOTAL, ISNULL(AGENCYOPENINGBILL.ABILL_CD, 0) AS CD, ISNULL(ITEMMASTER.item_name, '') AS ITEMNAME, ISNULL(AGENCYOPENINGBILL.ABILL_RATE, 0) AS RATE, ISNULL(AGENCYOPENINGBILL.ABILL_LRNO, '') AS LRNO, ISNULL(AGENCYOPENINGBILL.ABILL_PARTYBILLNO, '') AS PARTYBILLNO, ISNULL(AGENCYOPENINGBILL.ABILL_HOLDINTCALC, 0) AS HOLDINTCALC ", "", " AGENCYOPENINGBILL INNER JOIN LEDGERS ON AGENCYOPENINGBILL.ABILL_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN ITEMMASTER ON AGENCYOPENINGBILL.ABILL_ITEMID = ITEMMASTER.item_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON AGENCYOPENINGBILL.ABILL_AGENTID = AGENTLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS DELIVERYLEDGERS ON AGENCYOPENINGBILL.ABILL_DELIVERYATID = DELIVERYLEDGERS.Acc_id ", " AND LEDGERS.ACC_CMPNAME = '" & CMBNAME.Text.Trim & "' AND ABILL_YEARID = " & YearId & "  ORDER BY AGENCYOPENINGBILL.ABILL_GRIDSRNO ")
            If DT.Rows.Count > 0 Then
                For Each ROW As DataRow In DT.Rows
                    GRIDOPENING.Rows.Add(ROW("GRIDSRNO"), ROW("BILLTYPE"), ROW("BILLNO"), ROW("YEAR"), Format(Convert.ToDateTime(ROW("BILLDATE")).Date, "dd/MM/yyyy"), ROW("CRDAYS"), Format(Convert.ToDateTime(ROW("DUEDATE")).Date, "dd/MM/yyyy"), ROW("AGENT"), ROW("NARRATION"), ROW("DISPUTE"), ROW("DELIVERYAT"), Val(ROW("PCS")), Val(ROW("MTRS")), Val(ROW("TOTALAMT")), Val(ROW("CHARGES")), Val(ROW("TAXABLEAMT")), Val(ROW("CGSTPER")), Val(ROW("CGSTAMT")), Val(ROW("SGSTPER")), Val(ROW("SGSTAMT")), Val(ROW("IGSTPER")), Val(ROW("IGSTAMT")), Val(ROW("GRANDTOTAL")), Format(Val(ROW("AMT")), "0.00"), Format(Val(ROW("AMTPAIDREC")), "0.00"), Format(Val(ROW("EXTRAAMT")), "0.00"), Format(Val(ROW("RETURN")), "0.00"), Format(Val(ROW("BALANCE")), "0.00"), ROW("PRINTINITIALS"), ROW("CD"), ROW("ITEMNAME"), Val(ROW("RATE")), ROW("LRNO"), ROW("PARTYBILLNO"), ROW("HOLDINTCALC"))
    ...
    Sub FILLGRID()
        Try
            ...
            Else
                ...
                If CHKINTCALC.Checked = True Then
                    GRIDOPENING.Item(GINTCALC.Index, TEMPROW).Value = 1
                Else
    ...
    Private Sub GRIDOPENING_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GRIDOPENING.CellDoubleClick
        Try
            ...
            If e.RowIndex >= 0 And GRIDOPENING.Item(GSRNO.Index, e.RowIndex).Value <> Nothing Then
                ...
                CHKINTCALC.Checked = Convert.ToBoolean(GRIDOPENING.Item(GINTCALC.Index, e.RowIndex).Value)
        ...
        If e.KeyCode = Keys.Delete Then
            ...
            If EDIT = True Then
                ...
                If TEMPMSG = vbYes Then
                    Dim OBJOP As New ClsAgencyOpeningBills
                    Dim ALPARAVAL As New ArrayList
    ...
    Function CHECKBILLNO() As Boolean
        Try
            ...
            If TXTBILLNO.Text.Trim <> "" And TXTYEAR.Text.Trim <> "" And CMBTYPE.Text.Trim <> "" And CMBNAME.Text.Trim <> "" Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.SEARCH(" LEDGERS.ACC_CMPNAME as NAME", "", " AGENCYOPENINGBILL INNER JOIN LEDGERS ON AGENCYOPENINGBILL.ABILL_LEDGERID = LEDGERS.Acc_id ", " AND AGENCYOPENINGBILL.ABILL_NO='" & TXTBILLNO.Text.Trim & "' and AGENCYOPENINGBILL.ABILL_YEAR = '" & TXTYEAR.Text.Trim & "' AND ABILL_TYPE = '" & CMBTYPE.Text.Trim & "' AND LEDGERS.ACC_CMPNAME <> '" & CMBNAME.Text.Trim & "' AND ABILL_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
                    EP.SetError(CMBNAME, "Agency Bill No already Exists in " & DT.Rows(0).Item("NAME"))
                    BLN = False
                End If
            End If
            Return BLN
    ...
    Private Sub TXTBILLNO_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTBILLNO.Validating
        Try
            If CMBTYPE.Text.Trim = "SALE" And GRIDDOUBLECLICK = False Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.SEARCH(" ABILL_NO as BILLNO", "", " AGENCYOPENINGBILL INNER JOIN LEDGERS ON AGENCYOPENINGBILL.ABILL_LEDGERID = LEDGERS.Acc_id ", " AND AGENCYOPENINGBILL.ABILL_NO='" & TXTBILLNO.Text.Trim & "' and AGENCYOPENINGBILL.ABILL_YEAR = '" & TXTYEAR.Text.Trim & "' AND ABILL_TYPE = 'SALE' AND ABILL_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
                    MsgBox("Agency Bill No already Exists", MsgBoxStyle.Critical)
                    e.Cancel = True
                End If
            End If
        Catch ex As Exception
