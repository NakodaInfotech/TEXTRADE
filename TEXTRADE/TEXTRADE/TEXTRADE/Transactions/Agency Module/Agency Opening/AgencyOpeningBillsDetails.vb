...
Public Class AgencyOpeningBillsDetails
    ...
    Sub getdetails(ByRef name As String)
        ...
        Dim dttable As DataTable
        'If FRMSTRING = "AGENCYOPENINGBILLS" Then
        dttable = OBJCMN.SEARCH(" LEDGERS.Acc_cmpname AS NAME, AGENCYOPENINGBILL.ABILL_GRIDSRNO AS SRNO, AGENCYOPENINGBILL.ABILL_TYPE AS TYPE, AGENCYOPENINGBILL.ABILL_NO AS BILLNO, AGENCYOPENINGBILL.ABILL_YEAR AS YEAR, AGENCYOPENINGBILL.ABILL_DATE AS DATE, AGENCYOPENINGBILL.ABILL_DUEDATE AS DUEDATE, AGENCYOPENINGBILL.ABILL_AMT AS AMT, ISNULL(ITEMMASTER.item_name, '') AS ITEMNAME, ISNULL(AGENCYOPENINGBILL.ABILL_RATE, 0) AS RATE, ISNULL(AGENCYOPENINGBILL.ABILL_LRNO, '') AS LRNO, ISNULL(AGENCYOPENINGBILL.ABILL_PARTYBILLNO, '') AS PARTYBILLNO ", "", " AGENCYOPENINGBILL INNER JOIN LEDGERS ON AGENCYOPENINGBILL.ABILL_LEDGERID = LEDGERS.Acc_id AND AGENCYOPENINGBILL.ABILL_CMPID = LEDGERS.Acc_cmpid AND AGENCYOPENINGBILL.ABILL_LOCATIONID = LEDGERS.Acc_locationid AND AGENCYOPENINGBILL.ABILL_YEARID = LEDGERS.Acc_yearid LEFT OUTER JOIN ITEMMASTER ON AGENCYOPENINGBILL.ABILL_ITEMID = ITEMMASTER.item_id ", " AND LEDGERS.ACC_CMPNAME = '" & name & "' AND ABILL_CMPID = " & CmpId & " AND ABILL_LOCATIONID = " & Locationid & " AND ABILL_YEARID = " & YearId & " ORDER BY ABILL_GRIDSRNO")
        'Else
