...
Public Class AgencySaleOrder
    ...
    Sub fillgrid(MATCHING As String)
        ...
        If ClientName = "INDRAPUJAFABRICS" Or ClientName = "INDRAPUJAIMPEX" Or ClientName = "SUPRIYA" Or ClientName = "SMS" Or ClientName = "SONU" Or ClientName = "MNARESH" Or ClientName = "SIDDHGIRI" Or ClientName = "MASHOK" Then
            TXTRATE.Clear()
    ...
    Private Function ERRORVALID() As Boolean
        ...
        If ClientName = "AVIS" Then
            ...
            For Each ROW As DataGridViewRow In GRIDSO.Rows
                If ROW.Cells(GDESIGN.Index).Value <> "" Then
                    DTTABLE = OBJCMN.SEARCH("AGENCYSALEORDER.ASO_NO AS SONO", "", "AgencySaleOrder_DESC INNER JOIN ITEMMASTER ON ITEM_ID = ASO_ITEMID LEFT OUTER JOIN DESIGNMASTER ON ASO_DESIGNID = DESIGN_ID LEFT OUTER JOIN COLORMASTER ON ASO_COLORID = COLOR_ID INNER JOIN AGENCYSALEORDER ON AGENCYSALEORDER.ASO_NO = AgencySaleOrder_DESC.ASO_NO AND AGENCYSALEORDER.ASO_YEARID = AgencySaleOrder_DESC.ASO_YEARID INNER JOIN LEDGERS ON AGENCYSALEORDER.ASO_LEDGERID = LEDGERS.ACC_ID", " AND AGENCYSALEORDER.ASO_NO <> " & Val(TXTSONO.Text.Trim) & " AND ITEM_NAME = '" & ROW.Cells(gitemname.Index).Value & "' AND ISNULL(DESIGN_NO,'') = '" & ROW.Cells(GDESIGN.Index).Value & "' AND ISNULL(COLOR_NAME,'') = '" & ROW.Cells(gcolor.Index).Value & "' AND LEDGERS.ACC_CMPNAME = '" & cmbname.Text.Trim & "' AND AGENCYSALEORDER.ASO_YEARID = " & YearId)
                    If DTTABLE.Rows.Count > 0 Then
        ...
        If Val(TXTSONO.Text.Trim) > 0 And EDIT = False And TXTSONO.ReadOnly = False Then
            DTTABLE = OBJCMN.SEARCH("  ISNULL(ASO_no, 0) AS SONO ", "", " AGENCYSALEORDER ", "  AND AGENCYSALEORDER.ASO_no=" & TXTSONO.Text.Trim & " AND AGENCYSALEORDER.ASO_CMPID = " & CmpId & " AND AGENCYSALEORDER.ASO_LOCATIONID = " & Locationid & " AND AGENCYSALEORDER.ASO_YEARID = " & YearId)
            If DTTABLE.Rows.Count > 0 Then
...
line1:
    ...
    Private Sub CMBPACKING_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles CMBPACKING.Enter
        Try
            ...
            Else
                If CMBPACKING.Text.Trim = "" Then FILLNAME(CMBPACKING, EDIT, " and GROUPMASTER.GROUP_SECONDARY = 'SUNDRY CREDITORS' AND ACC_TYPE = 'ACCOUNTS'")
            End If
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Sub CMBPACKING_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBPACKING.Validating
        Try
            If CMBPACKING.Text.Trim <> "" Then NAMEVALIDATE(CMBPACKING, CMBCODE, e, Me, txtadd, " and GROUPMASTER.GROUP_SECONDARY = 'SUNDRY CREDITORS'", "SUNDRY CREDITORS", "ACCOUNTS")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub

    Private Sub AgencySaleOrder_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Sub SHOWDATA()
        Try
            ...
            Dim objclsSO As New ClsAgencySaleOrder()
    ...
    Private Sub AgencySaleOrder_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Try
    ...
    Private Sub txtrate_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTRATE.Validating
        Try
            CALC()
            If ClientName = "YASHVI" Or ClientName = "SAFFRON" Or ClientName = "SAFFRONOFF" Or ClientName = "MASHOK" Then TXTAMOUNT_Validating(sender, e)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
        ...
        Try
            If EDIT = True Then
                ...
                If tempMsg = vbYes Then
                    ...
                    Dim clspo As New ClsAgencySaleOrder()
        ...
        Try
            If cmbname.Text.Trim <> "" Then
                ...
                If DT.Rows.Count > 0 Then
                    ...
                    If ClientName = "MAHAVIRPOLYCOT" Or ClientName = "SNCM" And Val(DT.Rows(0).Item("CDPER")) > 0 And ClientName <> "MASHOK" Then txtcd.Text = Val(DT.Rows(0).Item("CDPER"))
        ...
        Try
            If GRIDSO.CurrentRow.Index >= 0 And GRIDSO.Item(gsrno.Index, GRIDSO.CurrentRow.Index).Value <> Nothing Then
                ...
                If Convert.ToBoolean(GRIDSO.Rows(GRIDSO.CurrentRow.Index).Cells(GDONE.Index).Value) = True Then 'If row.Cells(16).Value <> "0" Then 
                    MsgBox("Item Locked. First Delete from AGENCYSALEORDER")
                    Exit Sub
                End If
...
LINE1:
    ...
    Sub PRINTREPORT()
        '        Try
        '            Dim OBJAGENCYSALEORDER As New AgencySaleOrderDesign
        '            OBJAGENCYSALEORDER.MdiParent = MDIMain
        '            OBJAGENCYSALEORDER.FRMSTRING = "AGENCYSOREPORT"
        '            OBJAGENCYSALEORDER.PARTYNAME = cmbname.Text.Trim
        '            OBJAGENCYSALEORDER.AGENTNAME = CMBAGENT.Text.Trim
        '            OBJAGENCYSALEORDER.SHIPTONAME = CMBPACKING.Text.Trim
        '            OBJAGENCYSALEORDER.SONO = Val(TXTSONO.Text.Trim)
        '            OBJAGENCYSALEORDER.FORMULA = "{AGENCYSALEORDER.ASO_no}=" & Val(TXTSONO.Text) & " and {AGENCYSALEORDER.ASO_yearid}=" & YearId
        '            If ClientName = "MAHAVIRPOLYCOT" Then
        '                If MsgBox("Wish to Print Party SO Report?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
        '                    OBJAGENCYSALEORDER.PARTYSOREPORT = True
        '                    GoTo LINE1
        '                End If
        '            End If
        '            If ClientName = "KRISHNA" Or ClientName = "KRFABRICS" Or ClientName = "AVIS" Or ClientName = "SHREENAKODA" Or ClientName = "SOFTAS" Then
        '                If MsgBox("Wish to Print SO Report With Rate?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then OBJAGENCYSALEORDER.RATERACK = "RATE" Else OBJAGENCYSALEORDER.RATERACK = "RACK"
        '            End If

        '            If ClientName = "GELATO" Or ClientName = "ANOX" Then
        '                If MsgBox("Wish to Print SO Report With Image?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then OBJAGENCYSALEORDER.WITHPHOTO = True
        '            End If

        'LINE1:
        '            OBJAGENCYSALEORDER.Show()
        ...
        '                    OBJINV.FORMULA = "{AGENCYSALEORDER.ASO_no}=" & Val(TXTSONO.Text) & " and {AGENCYSALEORDER.ASO_yearid}=" & YearId
        '                    OBJINV.Show()
        '                End If
        '            End If
    ...
    Private Sub CMBTRANS2_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbtrans2.Enter
        Try
            If cmbtrans2.Text.Trim = "" Then FILLNAME(CMBPACKING, EDIT, " And (GROUP_SECONDARY = 'SUNDRY DEBTORS' OR GROUP_SECONDARY = 'SUNDRY CREDITORS')   AND ACC_TYPE = 'ACCOUNTS'")
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    End Sub
    ...
    Private Sub CMBTRANS2_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbtrans2.Validating
        Try
            If cmbtrans2.Text.Trim <> "" Then NAMEVALIDATE(CMBPACKING, CMBCODE, e, Me, txtDeliveryadd, " AND  (GROUP_SECONDARY = 'SUNDRY DEBTORS' OR GROUP_SECONDARY = 'SUNDRY CREDITORS')", "Sundry Debtors", "ACCOUNTS", cmbtrans.Text, CMBAGENT.Text)
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    End Sub
    ...
    Private Sub OpenToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OpenToolStripButton.Click
        Try
            ...
            Dim objpodtls As New AgencySaleOrderDetails
...
line1:
    ...
    Private Sub TOOLCLOSE_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'Try
        '    Dim OBJSO As New AGENCYSALEORDERClose
    ...
    Private Sub AgencySaleOrder_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Try
            ...
            If ClientName = "MNARESH" Or ClientName = "MSANCHITKUMAR" Or ClientName = "SNCM" Or ClientName = "MASHOK" Then LBLRATE.Visible = True
...
LINESINGLE:
    ...
    Sub GETSTOCK(ByVal ITEMNAME As String, ByVal DESIGNNO As String, ByVal COLOR As String)
        Try
            If ITEMNAME <> "" Then
                ...
                'PENDINGAGENCYSALEORDER
                DT = OBJCMN.SEARCH("ISNULL(SUM(ALLAgencySaleOrder_DESC.ASO_QTY - ALLAgencySaleOrder_DESC.ASO_RECDQTY),0) AS PCS, ISNULL(SUM(ALLAgencySaleOrder_DESC.ASO_MTRS - ALLAgencySaleOrder_DESC.ASO_RECDMTRS),0) AS MTRS", "", " ALLAgencySaleOrder_DESC INNER JOIN ITEMMASTER ON ALLAgencySaleOrder_DESC.ASO_ITEMID = ITEMMASTER.item_id LEFT OUTER JOIN DESIGNMASTER ON ALLAgencySaleOrder_DESC.ASO_DESIGNID = DESIGNMASTER.DESIGN_ID LEFT OUTER JOIN COLORMASTER ON ALLAgencySaleOrder_DESC.ASO_COLORID = COLORMASTER.COLOR_id ", " AND ROUND(ALLAgencySaleOrder_DESC.ASO_MTRS - ALLAgencySaleOrder_DESC.ASO_RECDMTRS, 2) > 0 AND ALLAgencySaleOrder_DESC.ASO_CLOSED = 'FALSE' AND ITEMMASTER.ITEM_NAME = '" & ITEMNAME & "' AND ISNULL(DESIGNMASTER.DESIGN_NO,'') = '" & DESIGNNO & "' AND ISNULL(COLOR_NAME,'') = '" & COLOR & "' AND ALLAgencySaleOrder_DESC.ASO_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
    ...
    Private Sub TXTSONO_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTSONO.Validating
        Try
            ...
            If Val(TXTSONO.Text.Trim) <> 0 And EDIT = False Then
                Dim OBJCMN As New ClsCommon
                Dim dttable As DataTable = OBJCMN.SEARCH("  ISNULL(ASO_no, 0) AS SONO ", "", " AGENCYSALEORDER ", "  AND AGENCYSALEORDER.ASO_no=" & TXTSONO.Text.Trim & " AND AGENCYSALEORDER.ASO_CMPID = " & CmpId & " AND AGENCYSALEORDER.ASO_LOCATIONID = " & Locationid & " AND AGENCYSALEORDER.ASO_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
    ...
    Sub SMSCODE()
        If ALLOWSMS = True And TXTMOBILENO.Text.Trim <> "" Then
            ...
            '    DT = OBJCMN.search("DISTINCT ITEMMASTER.ITEM_NAME AS ITEMNAME ", "", "AgencySaleOrder_DESC INNER JOIN ITEMMASTER ON ASO_ITEMID = ITEM_ID ", " AND ASO_NO = " & Val(TXTSONO.Text.Trim) & " AND ASO_YEARID = " & YearId)
            '    For Each DTROW As DataRow In DT.Rows
            ...
            If SENDMSG(MSG, TXTMOBILENO.Text.Trim) = "1701" Then
                MsgBox("Message Sent")
                DT = OBJCMN.Execute_Any_String("UPDATE AGENCYSALEORDER SET ASO_SMSSEND = 1 WHERE ASO_NO = " & TEMPSONO & " AND ASO_YEARID = " & YearId, "", "")
                LBLSMS.Visible = True
            Else
    ...
    Private Sub cmbitemname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbitemname.Validated
        Try
            If cmbitemname.Text.Trim <> "" And cmbname.Text.Trim <> "" Then
                ...
                If (ClientName = "MAHAVIR" Or ClientName = "BARKHA" Or ClientName = "MAHAJAN" Or ClientName = "SHUBHI" Or ClientName = "SUBHLAXMI" Or ClientName = "SMS" Or ClientName = "RAJKRIPA" Or ClientName = "MAHAVIRPOLYCOT" Or ClientName = "SIDDHPOLYCOT" Or ClientName = "SIDDHGIRI" Or ClientName = "MASHOK") Then
                    ...
                    If DT.Rows.Count > 0 Then
                        If ClientName <> "SIDDHGIRI" And ClientName <> "MASHOK" Then TXTCUT.Text = DT.Rows(0).Item("CUT")
                        ...
                        Else
                            CMBPER.Text = "Mtrs"
                        End If
                        If ClientName = "SIDDHGIRI" Or ClientName = "MASHOK" Then cmbqtyunit.Text = DT.Rows(0).Item("UNIT")
    ...
    Private Sub TOOLWHATSAPP_Click(sender As Object, e As EventArgs) Handles TOOLWHATSAPP.Click
        Try
            ...
            DT = OBJCMN.Execute_Any_String("UPDATE AGENCYSALEORDER SET ASO_SENDWHATSAPP = 1 WHERE ASO_NO = " & TEMPSONO & " AND ASO_YEARID = " & YearId, "", "")
    ...
    Async Sub SENDWHATSAPP(SONO As Integer)
        Try
            ...
            OBJSO.FORMULA = "{AGENCYSALEORDER.ASO_no}=" & Val(SONO) & " and {AGENCYSALEORDER.ASO_yearid}=" & YearId
            OBJSO.SONO = SONO
            OBJSO.NOOFCOPIES = 1
            OBJSO.Show()
            OBJSO.Close()
    ...
    Private Sub TXTCHNO_Validating(sender As Object, e As CancelEventArgs) Handles TXTCMPSONO.Validating
        Try
            If (ClientName = "KRISHNA") And CMBCMPNAME.Text.Trim <> "" And Val(TXTCMPSONO.Text.Trim) > 0 And EDIT = False Then
                ...
                'NOW FETCH AGENCYSALEORDER DATA
                Dim ALPARAVAL As New ArrayList
