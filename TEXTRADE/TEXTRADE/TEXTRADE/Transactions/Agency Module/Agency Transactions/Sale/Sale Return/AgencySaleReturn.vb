...
Public Class AgencySaleReturn
            ...
            ElseIf EDIT = True Then
                ...
                Else
                    ...
                    If CMBPAYTYPE.Text.Trim = "Against Bill" Then
                        ...
                        Dim OBJCMN As New ClsCommon
                        Dim DT As DataTable = OBJCMN.SEARCH(" ISNULL(SUM(T.RECAMT),0) AS RECAMT", "", " (SELECT SUM(AGENCYSALERETURN_BILLDESC.ASALRET_amt)  AS RECAMT, ASALRET_BILLINITIALS AS BILLINITIALS, ASALRET_NO as RECNO, ASALRET_cmpid AS CMPID, 0 AS LOCATIONID, ASALRET_yearid AS YEARID FROM AGENCYSALERETURN_BILLDESC WHERE ASALRET_paytype = 'Against Bill' GROUP BY ASALRET_BILLINITIALS, ASALRET_no, ASALRET_CMPID , ASALRET_YEARID) AS T ", " AND T.RECNO =  " & TXTSALRETNO.Text.Trim & " AND T.BILLINITIALS ='" & CMBBILLNO.Text.Trim & "' AND T.YEARID = " & YearId)
                        If DT.Rows.Count > 0 Then
    ...
    Sub getmaxno()
        Try
            ...
            DTTABLE = getmax(" isnull(max(ASALRET_no),0) + 1 ", " AGENCYSALERETURN ", " AND ASALRET_cmpid=" & CmpId & " and ASALRET_locationid=" & Locationid & " and ASALRET_yearid=" & YearId)
        ...
        Try
            ...
            If Val(TXTINVNO.Text.Trim) > 0 And ClientName <> "CC" And ClientName <> "C3" Then
                ...
                If TXTINVTYPE.Text.Trim = "INVOICE" Then
                    DT = OBJCMN.SEARCH("AINVOICE_BALANCE AS INVBAL", "", "AGENCYINVOICEMASTER ", " AND AINVOICE_NO = " & Val(TXTINVNO.Text.Trim) & "  AND AINVOICE_YEARID = " & YearId) 'AND REGISTER_NAME = '" & TXTINVREGNAME.Text.Trim & "'
                Else
                    DT = OBJCMN.SEARCH("ABILL_BALANCE AS INVBAL", "", "AGENCYOPENINGBILL  INNER JOIN LEDGERS ON ACC_ID = ABILL_LEDGERID", " AND ACC_CMPNAME = '" & CMBNAME.Text.Trim & "' AND ABILL_NO = " & Val(TXTINVNO.Text.Trim) & "  AND ABILL_YEARID = " & YearId) 'AND REGISTER_NAME = '" & TXTINVREGNAME.Text.Trim & "'
                End If
                BALANCE = Val(DT.Rows(0).Item("INVBAL"))
                If EDIT = True Then
                    Dim DT1 As DataTable = OBJCMN.SEARCH("ASALRET_GRANDTOTAL AS RETTOTAL", "", "AGENCYSALERETURN", " AND ASALRET_NO = " & Val(TEMPSALRETNO) & " AND ASALRET_YEARID = " & YearId)
            ...
            For Each row As DataGridViewRow In GRIDSALRET.Rows
                ...
                If ALLOWMANUALCNDN = True Then
                    If TXTSALRETNO.Text <> "" And CMBNAME.Text.Trim <> "" And EDIT = False Then
                        Dim OBJCMN As New ClsCommon
                        Dim dttable As DataTable = OBJCMN.SEARCH(" ISNULL(AGENCYSALERETURN.ASALRET_no, 0) AS SALRETNO", "", "  AGENCYSALERETURN ", "  AND AGENCYSALERETURN.ASALRET_NO=" & TXTSALRETNO.Text.Trim & "  AND AGENCYSALERETURN.ASALRET_cmpid = " & CmpId & " AND AGENCYSALERETURN.ASALRET_yearid = " & YearId) 'AND REGISTER_NAME = '" & TXTINVREGNAME.Text.Trim & "'
                        If dttable.Rows.Count > 0 Then
        ...
        Try
            ...
            If EDIT = True Then
                ...
                If dttable.Rows.Count > 0 Then
                    ...
                    'CHARGES GRID
                    Dim OBJCMN As New ClsCommon
                    Dim dttable1 As DataTable = OBJCMN.SEARCH(" AGENCYSALERETURN_CHGS.ASALRET_gridsrno AS GRIDSRNO, LEDGERS.Acc_cmpname AS CHARGES, AGENCYSALERETURN_CHGS.ASALRET_PER AS PER, AGENCYSALERETURN_CHGS.ASALRET_AMT AS AMT, AGENCYSALERETURN_CHGS.ASALRET_TAXID AS TAXID ", "", " AGENCYSALERETURN_CHGS INNER JOIN LEDGERS ON AGENCYSALERETURN_CHGS.ASALRET_CHARGESID = LEDGERS.Acc_id ", " AND AGENCYSALERETURN_CHGS.ASALRET_NO = " & TEMPSALRETNO & " AND AGENCYSALERETURN_CHGS.ASALRET_YEARID = " & YearId)
                    ...
                    dttable1 = OBJCMN.SEARCH(" AGENCYSALERETURN_BILLDESC.ASALRET_GRIDSRNO AS GRIDSRNO, AGENCYSALERETURN_BILLDESC.ASALRET_PAYTYPE AS PAYTYPE, AGENCYSALERETURN_BILLDESC.ASALRET_BILLINITIALS AS BILLINITIALS, AGENCYSALERETURN_BILLDESC.ASALRET_GRIDREMARKS AS NARR, AGENCYSALERETURN_BILLDESC.ASALRET_AMT AS AMT, AGENCYSALERETURN_BILLDESC.ASALRET_AMTPAID AS AMTPAID, AGENCYSALERETURN_BILLDESC.ASALRET_EXTRAAMT AS EXTRAAMT, AGENCYSALERETURN_BILLDESC.ASALRET_RETURN AS [RETURN], AGENCYSALERETURN_BILLDESC.ASALRET_BALANCE AS BALANCE ", "", " AGENCYSALERETURN_BILLDESC ", " AND AGENCYSALERETURN_BILLDESC.ASALRET_NO = " & TEMPSALRETNO & " AND AGENCYSALERETURN_BILLDESC.ASALRET_YEARID = " & YearId)
    ...
    Private Sub OpenToolStripButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles OpenToolStripButton.Click
        Try
            ...
            Dim OBJPURCH As New AgencySaleReturnDetails
...
LINE1:
    ...
    Sub PRINTREPORT(ByVal SRNO As Integer)
        ...
        '    If TEMPMSG = vbYes Then
        '        Dim OBJPUR As New AgencySaleReturnDesign
            ...
            If Convert.ToDateTime(SALRETDATE.Text).Date < "01/07/2017" Then
                If TXTCHALLANNO.Text.Trim.Length > 0 Then
                    If (EDIT = False) Or (EDIT = True And LCase(PARTYCHALLANNO) <> LCase(TXTCHALLANNO.Text.Trim)) Then
                        ...
                        Dim dt As DataTable = objclscommon.SEARCH(" ASALRET_challanno, LEDGERS.ACC_cmpname", "", " AGENCYSALERETURN inner join LEDGERS on LEDGERS.ACC_id = ASALRET_ledgerid ", " and ASALRET_challanno = '" & TXTCHALLANNO.Text.Trim & "' and LEDGERS.ACC_cmpname = '" & CMBNAME.Text.Trim & "' AND ASALRET_YEARID =" & YearId)
                        If dt.Rows.Count > 0 Then
    ...
    Private Function CHECKBARCODE() As Boolean
        Try
            Dim BLN As Boolean = True
            Dim OBJCMN As New ClsCommon
            Dim DT As DataTable = OBJCMN.SEARCH(" ISNULL(ASALRET_BARCODE,'') AS BARCODE ", "", " AGENCYSALERETURN_DESC ", " AND AGENCYSALERETURN_DESC.ASALRET_YEARID =  " & YearId)
            If DT.Rows.Count > 0 Then
...
LINE1:
    ...
    Sub FILLGRIDINVOICE()
        Try
            ...
            Dim DT As New DataTable
            DT = objpayment.GETBILLS(CmpId, CMBNAME.Text.Trim, YearId, CMBSELLER.Text.Trim, Convert.ToDateTime(SALRETDATE.Text).Date)
    ...
    Private Sub AgencySaleReturn_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Try
    ...
    Private Sub TOOLWHATSAPP_Click(sender As Object, e As EventArgs) Handles TOOLWHATSAPP.Click
        Try
            ...
            DT = OBJCMN.Execute_Any_String("UPDATE AGENCYSALERETURN SET ASALRET_SENDWHATSAPP = 1 WHERE ASALRET_NO = " & TEMPSALRETNO & " AND ASALRET_YEARID = " & YearId, "", "")
    ...
    Async Sub SENDWHATSAPP(SRNO As Integer)
        ...
        '    Dim OBJSR As New AgencySaleReturnDesign
    ...
    Private Sub TXTSALRETNO_Validating(sender As Object, e As CancelEventArgs) Handles TXTSALRETNO.Validating
        'Try
        '    If ALLOWMANUALCNDN = True Then
        '        If (Val(TXTSALRETNO.Text.Trim) <> 0 And EDIT = False) Or (EDIT = True And TEMPSALRETNO <> Val(TXTSALRETNO.Text.Trim)) Then 'And TXTINVREGNAME.Text.Trim <> ""
        '            Dim OBJCMN As New ClsCommon
        '            Dim dttable As DataTable = OBJCMN.SEARCH("ISNULL(AGENCYSALERETURN.ASALRET_no, 0) AS SALRETNO, ISNULL(REGISTERMASTER.register_name,'') AS REGNAME", "", " REGISTERMASTER INNER JOIN AGENCYSALERETURN ON REGISTERMASTER.register_id = AGENCYSALERETURN.ASALRET_INVOICEREGID ", "  AND AGENCYSALERETURN.ASALRET_NO=" & TXTSALRETNO.Text.Trim & "  AND AGENCYSALERETURN.ASALRET_yearid = " & YearId) 'AND REGISTER_NAME = '" & TXTINVREGNAME.Text.Trim & "'
        '            If dttable.Rows.Count > 0 Then
    ...
    Async Sub GENERATEINV()
        Try
            ...
            If TEMPSTATUS = "FAILED" Then
                ...
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTSALRETNO.Text.Trim) & ",'AGENCYSALERETURN','" & TOKEN & "','','" & TEMPSTATUS & "','" & REQUESTEDTEXT & "', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                Exit Sub
            End If
            ...
            Try
                ...
                Dim DTINI As DataTable = OBJCMN.SEARCH("ASALRET_PRINTINITIALS AS PRINTINITIALS", "", "AGENCYSALERETURN", " AND ASALRET_NO = " & Val(TXTSALRETNO.Text.Trim) & " AND ASALRET_YEARID = " & YearId)
                ...
                DT = OBJCMN.Execute_Any_String(" SELECT ISNULL(AGENCYSALERETURN_DESC.ASALRET_GRIDSRNO,0) AS SRNO, ISNULL(ITEMMASTER.item_name,'') AS ITEMNAME, ISNULL(HSN_CODE,'') AS HSNCODE, ISNULL(HSN_CGST,0) AS CGST, ISNULL(HSN_SGST,0) AS SGST, ISNULL(HSN_IGST,0) AS IGST, ISNULL(AGENCYSALERETURN_DESC.ASALRET_QTY,0) AS PCS, ISNULL(AGENCYSALERETURN_DESC.ASALRET_MTRS,0) AS MTRS, ISNULL(AGENCYSALERETURN_DESC.ASALRET_PER,'Mtrs') AS PER, ISNULL(AGENCYSALERETURN_DESC.ASALRET_RATE,0) AS RATE, ISNULL(AGENCYSALERETURN_DESC.ASALRET_AMT,0) AS TOTALAMT, 0 AS LINEDISC, 0 AS LINETAXABLEAMT, 0 AS LINECGSTAMT, 0 AS LINESGSTAMT, 0 AS LINEIGSTAMT, ISNULL(AGENCYSALERETURN_DESC.ASALRET_AMT,0) AS LINEGRIDDTOTAL, ISNULL(HSN_TYPE,'Goods') HSNTYPE FROM AGENCYSALERETURN INNER JOIN AGENCYSALERETURN_DESC ON AGENCYSALERETURN.ASALRET_NO = AGENCYSALERETURN_DESC.ASALRET_NO AND AGENCYSALERETURN.ASALRET_YEARID = AGENCYSALERETURN_DESC.ASALRET_YEARID INNER JOIN ITEMMASTER ON item_id = ASALRET_ITEMID INNER JOIN HSNMASTER ON HSN_ID = ASALRET_HSNCODEID WHERE AGENCYSALERETURN.ASALRET_NO = " & Val(TXTSALRETNO.Text.Trim) & " and AGENCYSALERETURN.ASALRET_YEARID = " & YearId & " ORDER BY AGENCYSALERETURN_DESC.ASALRET_GRIDSRNO", "", "")
...
ERRORMESSAGE:
                ...
                'ADD DATA IN EINVOICEENTRY
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTSALRETNO.Text.Trim) & ",'AGENCYSALERETURN','" & TOKEN & "','','FAILED','" & REQUESTEDTEXT & "', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
            ...
            DT = OBJCMN.Execute_Any_String("UPDATE AGENCYSALERETURN SET ASALRET_IRNNO = '" & TXTIRNNO.Text.Trim & "', ASALRET_ACKNO = '" & TXTACKNO.Text.Trim & "', ASALRET_ACKDATE = '" & Format(ACKDATE.Value.Date, "MM/dd/yyyy") & "' WHERE ASALRET_NO = " & Val(TXTSALRETNO.Text.Trim) & " AND ASALRET_YEARID = " & YearId, "", "")

            'ADD DATA IN EINVOICEENTRY
            DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTSALRETNO.Text.Trim) & ",'AGENCYSALERETURN','" & TOKEN & "','" & IRNNO & "','" & TEMPSTATUS & "', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
            ...
            If TEMPSTATUS = "SUCCESS" Then
                ...
                If PBQRCODE.Image IsNot Nothing Then
                    Dim OBJINVOICE As New ClsAgencySaleReturn
                    Dim MS As New IO.MemoryStream
                    PBQRCODE.Image.Save(MS, Drawing.Imaging.ImageFormat.Png)
                ...
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTSALRETNO.Text.Trim) & ",'AGENCYSALERETURN','" & TOKEN & "','" & IRNNO & "','QRCODE SUCCESS', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTSALRETNO.Text.Trim) & ",'AGENCYSALERETURN','" & TOKEN & "','" & IRNNO & "','QRCODE SUCCESS1', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
    ...
    Private Async Sub CMDGETQRCODE_Click(sender As Object, e As EventArgs) Handles CMDGETQRCODE.Click
        Try
            If EDIT = True And TXTIRNNO.Text.Trim <> "" And IsNothing(PBQRCODE.Image) = True Then
                ...
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTSALRETNO.Text.Trim) & ",'AGENCYSALERETURN','" & TOKEN & "','" & TXTIRNNO.Text.Trim & "','QRCODE SUCCESS', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
                DT = OBJCMN.Execute_Any_String("INSERT INTO EINVOICEENTRY VALUES (" & Val(TXTSALRETNO.Text.Trim) & ",'AGENCYSALERETURN','" & TOKEN & "','" & TXTIRNNO.Text.Trim & "','QRCODE SUCCESS1', '', GETDATE(), " & CmpId & "," & Userid & "," & YearId & ")", "", "")
