...
Public Class AgencyReceiptDetails
    ...
    Private Sub AgencyReceiptDetails_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Try
    ...
    Sub fillgrid(ByVal TEMPCONDITION)
        Try
            ...
            Dim dt As DataTable = objclsCMST.search(" CAST(0 AS BIT) AS CHK, AGENCYRECEIPTMASTER.Areceipt_no AS SRNO, LEDGERS.Acc_cmpname AS Name, AGENCYRECEIPTMASTER.Areceipt_date AS Date, AGENCYRECEIPTMASTER.Areceipt_total AS Total, AGENCYRECEIPTMASTER.Areceipt_CHQNO AS [Chq. No.],AGENCYRECEIPTMASTER.Areceipt_registerid AS Registerid, AGENCYRECEIPTMASTER.Areceipt_remarks AS Remarks, BANKLEDGERS.Acc_cmpname AS SELLER, ISNULL(AGENCYRECEIPTMASTER.Areceipt_CHECKPDC, 0) AS CHECKPDC, AGENCYRECEIPTMASTER.Areceipt_BILLREMARKS AS BILLREMARKS, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENTNAME, ISNULL(GROUPMASTER.group_name, '') AS GROUPNAME, ISNULL(AGENCYRECEIPTMASTER.Areceipt_CHQDATE, GETDATE()) AS CHQDATE, AGENCYRECEIPTMASTER.Areceipt_SPECIALREMARKS AS SPECIALREMARK, ISNULL(AGENCYRECEIPTMASTER.Areceipt_initials, '') AS INITIALS, ISNULL(AGENTLEDGERS.Acc_email, '') AS AGENTEMAILID, ISNULL(LEDGERS.Acc_email, '') AS PARTYEMAILID,ISNULL(AGENTLEDGERS.ACC_WHATSAPPNO, '') AS AGENTWHATSAPP, ISNULL(LEDGERS.ACC_WHATSAPPNO, '') AS PARTYWHATSAPP ", "", " AGENCYRECEIPTMASTER LEFT OUTER JOIN LEDGERS ON AGENCYRECEIPTMASTER.Areceipt_ledgerid = LEDGERS.Acc_id INNER JOIN LEDGERS AS BANKLEDGERS ON AGENCYRECEIPTMASTER.Areceipt_sellerid = BANKLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON LEDGERS.ACC_AGENTID = AGENTLEDGERS.Acc_id INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id", TEMPCONDITION)
        ...
        Try
            ...
            If cmbregister.Text.Trim.Length > 0 Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    fillgrid(" and dbo.AGENCYRECEIPTMASTER.ARECEIPT_cmpid=" & CmpId & " and dbo.AGENCYRECEIPTMASTER.ARECEIPT_LOCATIONid=" & Locationid & " and dbo.AGENCYRECEIPTMASTER.ARECEIPT_YEARid=" & YearId & " AND AGENCYRECEIPTMASTER.ARECEIPT_registerid = " & PAYREGID & " order by dbo.AGENCYRECEIPTMASTER.ARECEIPT_no ")
                Else
    ...
    Private Sub AgencyReceiptDetails_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub ReceiptDetails_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Shown
    ...
    Private Sub TOOLWHATSAPP_Click_1(sender As Object, e As EventArgs) Handles TOOLWHATSAPP.Click
        Try
            If (Val(TXTFROM.Text.Trim) = 0 Or Val(TXTTO.Text.Trim) = 0 Or Val(TXTCOPIES.Text.Trim) = 0) AndAlso gridpayment.SelectedRowsCount = 0 Then Exit Sub
            'IF WE HAVE SELECTED FROM AND TO THEN WORK WITH THE CURRENT CODE ELSE GO FOR SELECTED ENTRIES CODE
            If Val(TXTFROM.Text.Trim) > 0 And Val(TXTTO.Text.Trim) > 0 Then
    ...
    Private Sub TXTFROM_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TXTTO.KeyPress, TXTFROM.KeyPress
