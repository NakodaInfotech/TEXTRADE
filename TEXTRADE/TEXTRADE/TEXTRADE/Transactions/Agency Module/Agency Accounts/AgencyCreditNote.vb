...
LINE1:
                ...
                'GET INVOICE DETAILS 
                DT = OBJCMN.SEARCH("*", "", " (SELECT CAST(0 AS BIT) AS CHK, CAST(AGENCYINVOICEMASTER.AINVOICE_INITIALS AS VARCHAR(100)) AS BILLINITIALS, AGENCYINVOICEMASTER.AINVOICE_DATE AS BILLDATE, AGENCYINVOICEMASTER.AINVOICE_GRANDTOTAL AS BILLAMT, AGENCYINVOICEMASTER.AINVOICE_NO AS BILLNO, LEDGERS.Acc_cmpname AS NAME, ISNULL(AGENCYINVOICEMASTER.AINVOICE_TOTALMTRS, 0) AS TOTALMTRS, ISNULL(AGENCYINVOICEMASTER.AINVOICE_TOTALPCS, 0) AS TOTALPCS, ISNULL(AGENCYINVOICEMASTER.AINVOICE_SUBTOTAL, 0) AS TAXABLEAMT, ISNULL(SELLERLEDGERS.Acc_cmpname, '') AS SELLERNAME FROM AGENCYINVOICEMASTER INNER JOIN LEDGERS ON AGENCYINVOICEMASTER.AINVOICE_CMPID = LEDGERS.Acc_cmpid AND AGENCYINVOICEMASTER.AINVOICE_LOCATIONID = LEDGERS.Acc_locationid AND  AGENCYINVOICEMASTER.AINVOICE_YEARID = LEDGERS.Acc_yearid AND AGENCYINVOICEMASTER.AINVOICE_LEDGERID = LEDGERS.Acc_id INNER JOIN LEDGERS AS SELLERLEDGERS ON AGENCYINVOICEMASTER.AINVOICE_PURLEDGERID = SELLERLEDGERS.Acc_id  WHERE (AGENCYINVOICEMASTER.AINVOICE_YEARID = " & YearId & ") AND SELLERLEDGERS.Acc_cmpname = '" & CMBDEBITLEDGER.Text.Trim & "' AND LEDGERS.Acc_cmpname = '" & CMBNAME.Text.Trim & "'   UNION ALL SELECT CAST(0 AS BIT) AS CHK, CAST(AGENCYOPENINGBILL.ABILL_INITIALS AS VARCHAR(100)) AS BILLINITIALS, AGENCYOPENINGBILL.ABILL_DATE AS BILLDATE, AGENCYOPENINGBILL.ABILL_AMT AS BILLAMT,  AGENCYOPENINGBILL.ABILL_NO AS BILLNO, LEDGERS.Acc_cmpname AS NAME, ISNULL(AGENCYOPENINGBILL.ABILL_PCS, 0) AS TOTALPCS, ISNULL(AGENCYOPENINGBILL.ABILL_MTRS, 0) AS TOTALMTRS, ISNULL(AGENCYOPENINGBILL.ABILL_TAXABLEAMT, 0) AS TAXABLEAMT, ISNULL(SELLERLEDGERS.Acc_cmpname, '') AS SELLERNAME FROM AGENCYOPENINGBILL INNER JOIN LEDGERS ON AGENCYOPENINGBILL.ABILL_CMPID = LEDGERS.Acc_cmpid AND AGENCYOPENINGBILL.ABILL_LOCATIONID = LEDGERS.Acc_locationid AND AGENCYOPENINGBILL.ABILL_YEARID = LEDGERS.Acc_yearid AND AGENCYOPENINGBILL.ABILL_LEDGERID = LEDGERS.Acc_id INNER JOIN LEDGERS AS SELLERLEDGERS ON AGENCYOPENINGBILL.ABILL_DELIVERYATID = SELLERLEDGERS.Acc_id WHERE (AGENCYOPENINGBILL.ABILL_YEARID  = " & YearId & ")  AND SELLERLEDGERS.Acc_cmpname = '" & CMBDEBITLEDGER.Text.Trim & "' AND LEDGERS.Acc_cmpname = '" & CMBNAME.Text.Trim & "' ) AS T  ", " ORDER BY BILLDATE ,BILLNO  ")
                If DT.Rows.Count > 0 Then
    ...
    Sub SETGRIDINVOICE(ByVal DT As DataTable)
        Try
            ...
            GRIDBILL.Columns(i).Visible = False
            GRIDBILL.Columns(i).Name = "CRDAYS"
            i += 1

            GRIDBILL.Columns(i).Visible = False
            GRIDBILL.Columns(i).Name = "DUEDATE"
            i += 1
...
#End Region
    ...
    Private Sub CMBNAEME_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBNAME.Validating
        Try
    ...
    Private Sub CMBDEBITLEDGER_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBDEBITLEDGER.Validating
        Try
            If CMBDEBITLEDGER.Text.Trim <> "" Then NAMEVALIDATE(CMBDEBITLEDGER, CMBCODE, e, Me, TXTADD, " and GROUPMASTER.GROUP_SECONDARY = 'SUNDRY CREDITORS'", "SUNDRY CREDITORS", "ACCOUNTS")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
