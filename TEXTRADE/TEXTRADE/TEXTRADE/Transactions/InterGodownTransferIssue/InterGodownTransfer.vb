        ...
        Try
            If ALLOWBARCODEPRINT Then
                ...
                For Each ROW As DataGridViewRow In GRIDJO.Rows
                    ...
                    BARCODEPRINTING(ROW.Cells(GBARCODE.Index).Value, ROW.Cells(GPIECETYPE.Index).Value, ROW.Cells(GMERCHANT.Index).Value, ROW.Cells(GQUALITY.Index).Value, ROW.Cells(GDESIGN.Index).Value, ROW.Cells(GCOLOR.Index).Value, ROW.Cells(GUNIT.Index).Value, ROW.Cells(GLOTNO.Index).Value, ROW.Cells(GBALENO.Index).Value, GRIDDESC, Val(ROW.Cells(GMTRS.Index).Value), Val(ROW.Cells(GPCS.Index).Value), 0, "", TEMPHEADER, SUPRIYAHEADER, WHOLESALEBARCODE, "", "")
NEXTLINE:
...
LINE1:
    ...
    Private Sub CHKUPDATEBALENO_CheckedChanged(sender As Object, e As EventArgs) Handles CHKUPDATEBALENO.CheckedChanged
        Try
            If GRIDJO.RowCount > 0 And CHKUPDATEBALENO.CheckState = CheckState.Checked And EDIT = False Then
                For Each ROW As DataGridViewRow In GRIDJO.Rows
                    ROW.Cells(GBALENO.Index).Value = "PS" & AccFrom.Year.ToString.Substring(2, 2) & AccTo.Year.ToString.Substring(2, 2) & Val(TXTGODOWNNO.Text.Trim)
                Next
            End If
        Catch ex As Exception
            Throw ex
...
LINE1:
    ...
    Private Sub InterGodownTransfer_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
            If ClientName = "VALIANT" Then CHKUPDATEBALENO.Visible = True
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
