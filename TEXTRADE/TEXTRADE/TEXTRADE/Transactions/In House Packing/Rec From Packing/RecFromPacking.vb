...
LINE1:
    ...
    Private Sub CMBBARCODE_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles CMBBARCODE.Validated
        Try
            Dim OBJCMN As New ClsCommon
            Dim DT As DataTable = OBJCMN.SEARCH("ISSUEPACKING_DESC.ISS_NO AS ISSNO, ISSUEPACKING_DESC.ISS_NO AS FROMNO, ISSUEPACKING_DESC.ISS_GRIDSRNO AS FROMSRNO, 'ISSUEPACKING' AS GRIDTYPE, ISNULL(ISSUEPACKING_DESC.ISS_LOTNO,'') AS LOTNO, ISSUEPACKING_DESC.ISS_MTRS AS ISSUEMTRS, ROUND(ISSUEPACKING_DESC.ISS_MTRS - ISSUEPACKING_DESC.ISS_OUTMTRS,2) AS PENDINGMTRS, ISNULL(LEDGERS.Acc_cmpname,'') AS NAME, ISNULL(ISSUEPACKING_DESC.ISS_RATE,0) AS RATE, ISNULL(ISSUEPACKING.ISS_CHALLANNO,'') AS CHALLANNO, ISNULL(CONTRACTMASTER.CONTRACT_NAME,'') AS CONTRACTOR, ISNULL(ISSUEPACKING_DESC.ISS_BALENO,'') AS BALENO ", "", " ISSUEPACKING_DESC INNER JOIN ISSUEPACKING ON ISSUEPACKING_DESC.ISS_NO = ISSUEPACKING.ISS_no AND ISSUEPACKING_DESC.ISS_YEARID = ISSUEPACKING.ISS_yearid LEFT OUTER JOIN LEDGERS ON ISSUEPACKING.ISS_WEAVERID = LEDGERS.Acc_id LEFT OUTER JOIN CONTRACTMASTER ON ISSUEPACKING.ISS_CONTRACTID = CONTRACTMASTER.CONTRACT_ID ", " AND ISSUEPACKING_DESC.ISS_BARCODE='" & CMBBARCODE.Text.Trim & "' AND ROUND(ISSUEPACKING_DESC.ISS_MTRS-ISSUEPACKING_DESC.ISS_OUTMTRS,2) > 0 AND ISSUEPACKING.ISS_YEARID = " & YearId)
            If DT.Rows.Count > 0 Then
...
1:
    ...
    Private Sub CMBBARCODE_KeyDown(sender As Object, e As KeyEventArgs) Handles CMBBARCODE.KeyDown
        Try
            If e.KeyCode = Keys.F1 And ALLOWBARCODEPRINT = True Then
