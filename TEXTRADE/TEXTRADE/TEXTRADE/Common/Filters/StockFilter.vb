...
Public Class StockFilter
    ...
    Private Sub cmdshow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdshow.Click
        Try
            ...
            If RBBARCODEBALESTOCK.Checked = True Or RBBARCODESTOCKDTLS.Checked = True Or RBITEMDESIGNSHADESEMISMALLSUMM.Checked = True Or RBITEMDESIGNSHADESMALLSUMM.Checked = True Or RBITEMDESIGNSHADESMALLNOUNITSUMM.Checked = True Or RBCATEGORYITEMDESIGNSHADESMALLNOUNITSUMM.Checked = True Or RBCATEGORYITEMDESIGNSHADESMALLNOSINGLEUNITSUMM.Checked = True Or RBDESIGNNOTSENT.Checked = True Or RDBBALECOUNT.Checked = True Or (ALLOWBARCODEPRINT = True And RBITEMDESIGNSHADESUMM.Checked = True) Or (ALLOWBARCODEPRINT = True And (RBGODOWNWISESUMM.Checked = True Or RDBBARCODESTOCKSUMMIMG.Checked = True)) Then
                ...
                If RBCATEGORYITEMDESIGNSHADESMALLNOSINGLEUNITSUMM.Checked = True Then
                    OBJSTOCK.WHERECLAUSE = OBJSTOCK.WHERECLAUSE & " AND EXISTS ( SELECT 1 FROM ITEMMASTER IM INNER JOIN DESIGNMASTER  ON DESIGNMASTER.DESIGN_ITEMID = IM.item_id INNER JOIN DESIGNMASTER_COLOR  ON DESIGNMASTER.DESIGN_id = DESIGNMASTER_COLOR.DESIGN_ID         INNER JOIN COLORMASTER CM2 ON DESIGNMASTER_COLOR.DESIGN_COLORID = CM2.COLOR_id WHERE IM.item_name = BARCODESTOCK.ITEMNAME AND DESIGNMASTER.DESIGN_NO = BARCODESTOCK.DESIGNNO GROUP BY IM.item_name, DESIGNMASTER.DESIGN_NO HAVING COUNT(DISTINCT CM2.COLOR_id) > 1  ) AND ( SELECT COUNT(DISTINCT BS2.COLORID) FROM BARCODESTOCK AS BS2 WHERE BS2.ITEMNAME = BARCODESTOCK.ITEMNAME AND BS2.DESIGNNO = BARCODESTOCK.DESIGNNO AND BS2.YEARID = " & YearId & " ) > 1 "
                End If
                'FOR ITEMNAME
                GRIDITEM.ClearColumnsFilter()
                For i As Integer = 0 To GRIDITEM.RowCount - 1
                ...
                ElseIf RBITEMDESIGNSHADESEMISMALLSUMM.Checked = True Then
                    OBJSTOCK.FRMSTRING = "BARCODEITEMDESIGNSHADESTOCKSMALLSUMM"
    ...
    Private Sub StockFilter_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
            ...
            If ALLOWBARCODEPRINT = False Then
                ...
                RBCATEGORYITEMDESIGNSHADESMALLNOSINGLEUNITSUMM.Visible = False
                RBGODOWNWISESUMM.Visible = False
                RBORDERVSSTOCK.Visible = False
                RBITEMDESIGNSHADESEMISMALLSUMM.Visible = False
            Else
