...
Public Class OutstandingFilter
    ...
    Sub FILLBILL()
        Try
            Dim objclsCMST As New ClsCommonMaster
            Dim DT As DataTable = objclsCMST.search("*", "", " (SELECT CAST (0 AS BIT) AS CHK, ISNULL(INVOICEMASTER.INVOICE_INITIALS,'') AS BILLINITIALS, ISNULL(INVOICEMASTER.INVOICE_GRANDTOTAL,0) AS GRANDTOTAL FROM LEDGERS INNER JOIN INVOICEMASTER ON LEDGERS.Acc_id = INVOICEMASTER.INVOICE_LEDGERID INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id WHERE LEDGERS.ACC_YEARID = " & YearId & " AND LEDGERS.ACC_CMPNAME = '" & CMBPARTYNAME.Text.Trim & "' UNION ALL SELECT CAST (0 AS BIT) AS CHK, ISNULL(PURCHASEMASTER.BILL_INITIALS,'') AS BILLINITIALS, ISNULL(PURCHASEMASTER.BILL_GRANDTOTAL,0) AS GRANDTOTAL FROM LEDGERS INNER JOIN PURCHASEMASTER ON LEDGERS.Acc_id = PURCHASEMASTER.BILL_LEDGERID INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id WHERE LEDGERS.ACC_YEARID = " & YearId & " AND LEDGERS.ACC_CMPNAME = '" & CMBPARTYNAME.Text.Trim & "' UNION ALL SELECT CAST (0 AS BIT) AS CHK, ISNULL(OPENINGBILL.BILL_INITIALS,'') AS BILLINITIALS, ISNULL(OPENINGBILL.BILL_GRANDTOTAL,0) AS GRANDTOTAL FROM LEDGERS INNER JOIN OPENINGBILL ON LEDGERS.Acc_id = OPENINGBILL.BILL_LEDGERID INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id WHERE LEDGERS.ACC_YEARID = " & YearId & " AND LEDGERS.ACC_CMPNAME = '" & CMBPARTYNAME.Text.Trim & "') AS T", "")
