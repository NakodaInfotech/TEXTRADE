...
Public Class SaleInvoiceFilter
    ...
    Sub FILLCMB()
        Try
            ...
            Dim DTSUPP As DataTable = OBJCMN.SEARCH(" CAST (0 AS BIT) AS CHK,  ISNULL(LEDGERS.ACC_CMPNAME,'') AS NAME", " ", " LEDGERS INNER JOIN GROUPMASTER ON LEDGERS.ACC_GROUPID = GROUPMASTER.GROUP_ID", " AND GROUPMASTER.GROUP_SECONDARY = 'Sundry Creditors' AND LEDGERS.ACC_TYPE = 'ACCOUNTS' AND LEDGERS.ACC_YEARID = " & YearId & " ORDER BY LEDGERS.ACC_CMPNAME")
            If DTSUPP.Rows.Count > 0 Then
                GRIDSUPP.FocusedRowHandle = GRIDSUPP.RowCount - 1
                GRIDSUPP.TopRowIndex = GRIDSUPP.RowCount - 15
            End If
            GRIDSUPPDETAILS.DataSource = DTSUPP
    ...
    Private Sub cmdshow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdshow.Click
        Try
            ...
            If RDBENTRYWISE.Checked = True Or RDBPARTYENTRYWISE.Checked = True Or RDBAGENTENTRYWISE.Checked = True Or RBREGISTERENTRY.Checked = True Or RBREGISTERENTRYITEM.Checked = True Or RDBTERM.Checked = True Then
                'FOR SUPPNAME
                GRIDSUPP.ClearColumnsFilter()
                For i As Integer = 0 To GRIDSUPP.RowCount - 1
                    Dim dtrow As DataRow = GRIDSUPP.GetDataRow(i)
                    If Convert.ToBoolean(dtrow("CHK")) = True Then
                        If SUPPCLAUSE = "" Then SUPPCLAUSE = " AND ({PURLEDGERS.ACC_CMPNAME} = '" & dtrow("NAME") & "'" Else SUPPCLAUSE = SUPPCLAUSE & " OR {PURLEDGERS.ACC_CMPNAME} = '" & dtrow("NAME") & "'"
                    End If
                Next

                If SUPPCLAUSE <> "" Then
                    SUPPCLAUSE = SUPPCLAUSE & ")"
                    OBJSALE.WHERECLAUSE = OBJSALE.WHERECLAUSE & SUPPCLAUSE
                End If
            End If
            ...
            ElseIf RDBENTRYWISE.Checked = True Then
                If CHKSUMMARY.Checked = True Then OBJSALE.FRMSTRING = "ENTRYWISESUMM" Else OBJSALE.FRMSTRING = "ENTRYWISE"
    ...
    Private Sub SaleInvoiceFilter_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
            If ClientName = "SAKARIA" Or ClientName = "NVAHAN" Or ClientName = "ABHEE" Then
