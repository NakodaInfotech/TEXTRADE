...
Public Class HomePage
    ...
    Sub FILLMONTHLYSALE()
        Try
            Dim DT As DataTable = OBJCMN.SEARCH(" UPPER(DATENAME(MONTH, INVOICE_DATE)) AS [MONTHNAME], SUM(INVOICE_SUBTOTAL) AS TAXABLEAMT, SUM(INVOICE_GRANDTOTAL) AS GRANDTOTAL, ROUND(SUM(INVOICE_GRANDTOTAL)/GTOTAL* 100,2) AS PER ", "", " INVOICEMASTER CROSS APPLY (SELECT TOP 1 SUM(INVOICE_GRANDTOTAL) AS GTOTAL FROM INVOICEMASTER AS GINV WHERE INVOICEMASTER.INVOICE_YEARID = GINV.INVOICE_YEARID ) AS GINV ", " AND INVOICE_YEARID = " & YearId & " GROUP BY DATENAME(MONTH, INVOICE_DATE), GTOTAL ORDER BY (CASE WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'APRIL' THEN 1 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'MAY' THEN 2 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'JUNE' THEN 3 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'JULY' THEN 4 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'AUGUST' THEN 5 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'SEPTEMBER' THEN 6 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'OCTOBER' THEN 7 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'NOVEMBER' THEN 8 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'DECEMBER' THEN 9 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'JANUARY' THEN 10 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'FEBRUARY' THEN 11 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'MARCH' THEN 12 ELSE 13 END)")
            GRIDMONTHLYSALEDETAILS.DataSource = DT
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Sub FILLPO()
        Try
            ...
            If ClientName = "MASHOK" Or ClientName = "ABHEE" Then
                DT = OBJCMN.SEARCH(" ISNULL(LEDGERS.Acc_cmpname,'') AS NAME, ISNULL(ITEMMASTER.item_name,'') AS [ITEMNAME], SUM(ALLPURCHASEORDER_DESC.BALANCE) AS [MTRS]", "", " ALLPURCHASEORDER INNER JOIN LEDGERS On ALLPURCHASEORDER.PO_ledgerid = LEDGERS.Acc_id INNER JOIN ALLPURCHASEORDER_DESC On ALLPURCHASEORDER.PO_no = ALLPURCHASEORDER_DESC.PO_NO And ALLPURCHASEORDER.PO_YEARID = ALLPURCHASEORDER_DESC.PO_YEARID And ALLPURCHASEORDER.TYPE = ALLPURCHASEORDER_DESC.TYPE INNER JOIN ITEMMASTER On ALLPURCHASEORDER_DESC.PO_ITEMID = ITEMMASTER.item_id ", " AND ALLPURCHASEORDER_DESC.BALANCE > 0 And ALLPURCHASEORDER_DESC.PO_CLOSED=0  And ALLPURCHASEORDER.PO_YEARID = " & YearId & " GROUP BY ISNULL(LEDGERS.ACC_CMPNAME,''), ISNULL(ITEMMASTER.ITEM_NAME,'')")
            Else
