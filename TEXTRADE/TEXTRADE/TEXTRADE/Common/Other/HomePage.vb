...
Public Class HomePage
    ...
    Sub FILLMONTHLYSALE()
        Try
            ...
            If RBPURCHASE.Checked = True Then
                DT = OBJCMN.Execute_Any_String(";WITH MONTHDATA AS (SELECT MONTH(BILL_PARTYBILLDATE) AS MONTHNO, UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) AS MONTHNAME, SUM(BILL_SUBTOTAL) AS TAXABLEAMT, SUM(BILL_GRANDTOTAL) AS GRANDTOTAL FROM PURCHASEMASTER WHERE BILL_YEARID = " & YearId & " GROUP BY MONTH(BILL_PARTYBILLDATE), DATENAME(MONTH, BILL_PARTYBILLDATE)  UNION ALL  SELECT MONTH(AINVOICE_DATE) AS MONTHNO, UPPER(DATENAME(MONTH, AINVOICE_DATE)) AS MONTHNAME, SUM(AINVOICE_SUBTOTAL) AS TAXABLEAMT, SUM(AINVOICE_GRANDTOTAL) AS GRANDTOTAL FROM AGENCYINVOICEMASTER WHERE AINVOICE_YEARID = " & YearId & " GROUP BY MONTH(AINVOICE_DATE), DATENAME(MONTH, AINVOICE_DATE) ), FINALDATA AS (SELECT MONTHNO, MONTHNAME, SUM(TAXABLEAMT) AS TAXABLEAMT, SUM(GRANDTOTAL) AS GRANDTOTAL FROM MONTHDATA GROUP BY MONTHNO, MONTHNAME) SELECT MONTHNAME, TAXABLEAMT, GRANDTOTAL, ROUND((GRANDTOTAL * 100.0) / SUM(GRANDTOTAL) OVER(), 2) AS PER FROM FINALDATA ORDER BY CASE WHEN MONTHNO >= 4 THEN MONTHNO - 3 ELSE MONTHNO + 9 END", "", "")
                'DT = OBJCMN.SEARCH(" UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) AS [MONTHNAME], SUM(BILL_SUBTOTAL) AS TAXABLEAMT, SUM(BILL_GRANDTOTAL) AS GRANDTOTAL, ROUND(SUM(BILL_GRANDTOTAL)/GTOTAL* 100,2) AS PER  ", "", " PURCHASEMASTER CROSS APPLY (SELECT TOP 1 SUM(BILL_GRANDTOTAL) AS GTOTAL FROM PURCHASEMASTER AS GINV WHERE PURCHASEMASTER.BILL_YEARID = GINV.BILL_YEARID ) AS GINV  ", " AND BILL_YEARID = " & YearId & " GROUP BY DATENAME(MONTH, BILL_PARTYBILLDATE), GTOTAL ORDER BY (CASE WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'APRIL' THEN 1 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'MAY' THEN 2 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'JUNE' THEN 3 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'JULY' THEN 4 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'AUGUST' THEN 5 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'SEPTEMBER' THEN 6 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'OCTOBER' THEN 7 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'NOVEMBER' THEN 8 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'DECEMBER' THEN 9 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'JANUARY' THEN 10 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'FEBRUARY' THEN 11 WHEN UPPER(DATENAME(MONTH, BILL_PARTYBILLDATE)) = 'MARCH' THEN 12 ELSE 13 END)")
            Else
                DT = OBJCMN.Execute_Any_String(";WITH MONTHDATA AS (SELECT MONTH(INVOICE_DATE) AS MONTHNO, UPPER(DATENAME(MONTH, INVOICE_DATE)) AS MONTHNAME, SUM(INVOICE_SUBTOTAL) AS TAXABLEAMT, SUM(INVOICE_GRANDTOTAL) AS GRANDTOTAL FROM INVOICEMASTER WHERE INVOICE_YEARID = " & YearId & " GROUP BY MONTH(INVOICE_DATE), DATENAME(MONTH, INVOICE_DATE)  UNION ALL  SELECT MONTH(AINVOICE_DATE) AS MONTHNO, UPPER(DATENAME(MONTH, AINVOICE_DATE)) AS MONTHNAME, SUM(AINVOICE_SUBTOTAL) AS TAXABLEAMT, SUM(AINVOICE_GRANDTOTAL) AS GRANDTOTAL FROM AGENCYINVOICEMASTER WHERE AINVOICE_YEARID = " & YearId & " GROUP BY MONTH(AINVOICE_DATE), DATENAME(MONTH, AINVOICE_DATE) ), FINALDATA AS (SELECT MONTHNO, MONTHNAME, SUM(TAXABLEAMT) AS TAXABLEAMT, SUM(GRANDTOTAL) AS GRANDTOTAL FROM MONTHDATA GROUP BY MONTHNO, MONTHNAME) SELECT MONTHNAME, TAXABLEAMT, GRANDTOTAL, ROUND((GRANDTOTAL * 100.0) / SUM(GRANDTOTAL) OVER(), 2) AS PER FROM FINALDATA ORDER BY CASE WHEN MONTHNO >= 4 THEN MONTHNO - 3 ELSE MONTHNO + 9 END", "", "")
                'DT = OBJCMN.SEARCH(" UPPER(DATENAME(MONTH, INVOICE_DATE)) AS [MONTHNAME], SUM(INVOICE_SUBTOTAL) AS TAXABLEAMT, SUM(INVOICE_GRANDTOTAL) AS GRANDTOTAL, ROUND(SUM(INVOICE_GRANDTOTAL)/GTOTAL* 100,2) AS PER ", "", " INVOICEMASTER CROSS APPLY (SELECT TOP 1 SUM(INVOICE_GRANDTOTAL) AS GTOTAL FROM INVOICEMASTER AS GINV WHERE INVOICEMASTER.INVOICE_YEARID = GINV.INVOICE_YEARID ) AS GINV ", " AND INVOICE_YEARID = " & YearId & " GROUP BY DATENAME(MONTH, INVOICE_DATE), GTOTAL ORDER BY (CASE WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'APRIL' THEN 1 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'MAY' THEN 2 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'JUNE' THEN 3 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'JULY' THEN 4 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'AUGUST' THEN 5 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'SEPTEMBER' THEN 6 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'OCTOBER' THEN 7 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'NOVEMBER' THEN 8 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'DECEMBER' THEN 9 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'JANUARY' THEN 10 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'FEBRUARY' THEN 11 WHEN UPPER(DATENAME(MONTH, INVOICE_DATE)) = 'MARCH' THEN 12 ELSE 13 END)")
            End If
    ...
    Sub FILLPERFORMANCE()
        Try
            ...
            If RBPARTY.Checked = True Then
                DT = OBJCMN.Execute_Any_String(" ;WITH PARTYDATA AS (SELECT  L.ACC_CMPNAME AS NAME, SUM(I.INVOICE_GRANDTOTAL) AS GRANDTOTAL FROM INVOICEMASTER I INNER JOIN LEDGERS L ON I.INVOICE_LEDGERID = L.ACC_ID WHERE I.INVOICE_YEARID = " & YearId & " GROUP BY L.ACC_CMPNAME  UNION ALL SELECT L.ACC_CMPNAME AS NAME, SUM(A.AINVOICE_GRANDTOTAL) AS GRANDTOTAL FROM AGENCYINVOICEMASTER A INNER JOIN LEDGERS L ON A.AINVOICE_LEDGERID = L.ACC_ID WHERE A.AINVOICE_YEARID = " & YearId & " GROUP BY L.ACC_CMPNAME ), FINALDATA AS (SELECT NAME, SUM(GRANDTOTAL) AS GRANDTOTAL FROM PARTYDATA GROUP BY NAME ) SELECT NAME,GRANDTOTAL, ROUND((GRANDTOTAL * 100.0) / SUM(GRANDTOTAL) OVER(), 2) AS PER FROM FINALDATA ORDER BY GRANDTOTAL DESC", "", "")
            ...
            Else
                DT = OBJCMN.Execute_Any_String(" ;WITH PARTYDATA AS ( SELECT  L.ITEM_NAME AS NAME, SUM(I.INVOICE_AMOUNT) AS GRANDTOTAL FROM INVOICEMASTER_DESC I INNER JOIN ITEMMASTER L ON I.INVOICE_ITEMID = L.ITEM_ID WHERE I.INVOICE_YEARID = " & YearId & " GROUP BY L.ITEM_NAME UNION ALL SELECT L.ITEM_NAME AS NAME, SUM(A.AINVOICE_AMOUNT) AS GRANDTOTAL FROM AGENCYINVOICEMASTER_DESC A INNER JOIN ITEMMASTER L ON A.AINVOICE_ITEMID = L.ITEM_ID WHERE A.AINVOICE_YEARID = " & YearId & " GROUP BY L.ITEM_NAME ), FINALDATA AS ( SELECT  NAME, SUM(GRANDTOTAL) AS GRANDTOTAL FROM PARTYDATA GROUP BY NAME ) SELECT NAME, GRANDTOTAL, ROUND((GRANDTOTAL * 100.0) / SUM(GRANDTOTAL) OVER(), 2) AS PER FROM FINALDATA ORDER BY GRANDTOTAL DESC ", "", "")
    ...
    Sub FILLRECGRID()
        Try
            ...
            Dim dt As DataTable = OBJCMN.SEARCH("  T.NAME, SUM(T.BALANCE) AS BALANCE ", "", " (SELECT NAME,CASE WHEN DR > 0 THEN DR ELSE -CR END AS BALANCE FROM TRIALBALANCE WHERE  SECONDARY = 'Sundry Debtors' AND YEARID = " & YearId & " AND (DR > 0 OR CR > 0) UNION ALL SELECT NAME, CASE WHEN DR > 0 THEN DR ELSE -CR END AS BALANCE FROM AGENCYTRIALBALANCE WHERE SECONDARY = 'Sundry Debtors' AND YEARID = " & YearId & " AND (DR > 0 OR CR > 0)) T  ", " GROUP BY T.NAME ORDER BY T.NAME")
    ...
    Sub FILLPAYGRID()
        Try
            ...
            Dim dt As DataTable = OBJCMN.SEARCH("  T.NAME, SUM(T.BALANCE) AS BALANCE ", "", " (SELECT NAME,CASE WHEN CR > 0 THEN CR ELSE -DR END AS BALANCE FROM TRIALBALANCE WHERE  SECONDARY = 'Sundry Creditors' AND YEARID = " & YearId & " AND (CR > 0 OR DR > 0) UNION ALL SELECT NAME, CASE WHEN CR > 0 THEN CR ELSE -DR END AS BALANCE FROM AGENCYTRIALBALANCE WHERE SECONDARY = 'Sundry Creditors' AND YEARID = " & YearId & " AND (DR > 0 OR CR > 0)) T  ", " GROUP BY T.NAME ORDER BY T.NAME")
    ...
    Private Sub RBSALE_CheckedChanged(sender As Object, e As EventArgs) Handles RBSALE.CheckedChanged
        FILLMONTHLYSALE()
    End Sub
