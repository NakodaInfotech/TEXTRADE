...
Public Class PendingLRNo
    ...
    Private Sub PendingReturnDate_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Dim DTTRANS As DataTable = OBJCMN.SEARCH("LEDGERS.ACC_CMPNAME AS TRANSNAME", "", " LEDGERS ", " AND LEDGERS.ACC_TYPE = 'TRANSPORT' AND LEDGERS.ACC_YEARID = " & YearId)
            For Each ROW As DataRow In DTTRANS.Rows
                CMBTRANSPORT.Items.Add(ROW("TRANSNAME"))
            Next
            If ClientName = "ABHEE" Then CMBTRANSPORT.ReadOnly = False Else CMBTRANSPORT.ReadOnly = True
        ...
        Try
            ...
            If RBPENDING.Checked = True Then
                If ClientName = "ABHEE" Then
                    INVWHERECLAUSE = " AND ISNULL(INVOICEMASTER_DESC.INVOICE_GRIDLRNO,'') = '' "
            ...
            If ClientName = "ABHEE" Then
                DT = OBJCMN.search("*, CAST(0 AS BIT) AS CHK", "", "(SELECT INVOICEMASTER.INVOICE_NO AS INVOICENO, INVOICEMASTER.INVOICE_REGISTERID AS REGID, LEDGERS.Acc_cmpname AS NAME, INVOICEMASTER.INVOICE_DATE AS DATE, ISNULL(TRANSLEDGERS.Acc_cmpname, '') 
                         AS TRANSNAME, INVOICEMASTER_DESC.INVOICE_GRIDLRNO AS LRNO, (CASE WHEN INVOICEMASTER_DESC.INVOICE_GRIDLRNO = '' THEN NULL ELSE INVOICEMASTER.INVOICE_LRDATE END) AS LRDATE, 
                         INVOICEMASTER_DESC.INVOICE_PCS AS PCS, INVOICEMASTER_DESC.INVOICE_MTRS AS MTRS, ISNULL(INVOICEMASTER_DESC.INVOICE_PARTYBILLNO, '') AS CHALLANNO, ISNULL(SHIPTOLEDGERS.Acc_cmpname, '') AS SHIPTO, 
                         ISNULL(LEDGERS.Acc_mobile, '') AS MOBILENO, ISNULL(CITYMASTER.city_name, '') AS CITY, ISNULL(INVOICEMASTER.INVOICE_BALENOFROM, 0) AS BALENOFROM, ISNULL(INVOICEMASTER.INVOICE_EWAYBILLNO, '') 
                         AS EWAYBILLNO, ISNULL(REGISTERMASTER.register_name, '') AS REGNAME, ISNULL(FROMCITYMASTER.city_name, '') AS FROMCITY, ISNULL(INVOICEMASTER.INVOICE_GRANDTOTAL, 0) AS GRANDTOTAL, 
                         'INVOICE' AS TYPE, ISNULL(PURLEDGERS.Acc_cmpname, '') AS AGENTNAME, INVOICEMASTER.INVOICE_PRINTINITIALS AS PRINTINITIALS, ISNULL(LEDGERS.ACC_WHATSAPPNO, '') AS PARTYWHATSAPP, 
                         ISNULL(PURLEDGERS.ACC_WHATSAPPNO, '') AS AGENTWHATSAPP , INVOICEMASTER_DESC.INVOICE_SRNO AS GRIDSRNO
            ...
            For I As Integer = 0 To gridbill.RowCount - 1
                ...
                If ROW("LRNO") <> "" And IsDBNull(ROW("LRDATE")) = False Then
                    If ROW("TYPE") = "INVOICE" Then
                        If ClientName = "ABHEE" Then
                            DT = OBJCMN.Execute_Any_String("UPDATE INVOICEMASTER_DESC SET INVOICE_GRIDTRANSID = (SELECT ACC_ID FROM LEDGERS WHERE ACC_CMPNAME ='" & ROW("TRANSNAME") & "' AND ACC_YEARID = " & YearId & "), INVOICE_GRIDLRNO = '" & ROW("LRNO") & "' WHERE INVOICE_NO = " & Val(ROW("INVOICENO")) & " AND INVOICE_SRNO = " & Val(ROW("GRIDSRNO")) & " AND INVOICE_REGISTERID = " & Val(ROW("REGID")) & " AND INVOICE_YEARID = " & YearId, "", "")
                        Else
...
NEXTLINE:
    ...
    Private Sub CMDDELETE_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMDDELETE.Click
        Try
            ...
            If ROW("TYPE") = "INVOICE" Then
                If ClientName = "ABHEE" Then
                    DT = OBJCMN.Execute_Any_String("UPDATE INVOICEMASTER_DESC SET INVOICE_GRIDLRNO = '', INVOICE_GRIDTRANSID = 0 WHERE INVOICE_NO = " & Val(ROW("INVOICENO")) & " AND INVOICE_SRNO = " & Val(ROW("GRIDSRNO")) & " AND INVOICE_REGISTERID = " & Val(ROW("REGID")) & " AND INVOICE_YEARID = " & YearId, "", "")
                Else
