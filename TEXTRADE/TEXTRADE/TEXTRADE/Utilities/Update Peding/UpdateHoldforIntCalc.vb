...
Public Class UpdateHoldforIntCalc
    ...
    Sub FILLGRID()
        Try
            ...
            Else
                dt = objclsCMST.search("*", "", " HOLDFORINTCALCVIEW ", " AND HOLDINTCALC = 'TRUE' AND HOLDFORINTCALCVIEW.YEARID=" & YearId & " ORDER BY ENTRYTYPE, DATE, ENTRYNO")
            End If
        ...
        Try
            ...
            If RBENTERED.Checked = True Then
                ...
                For I As Integer = 0 To Val(SELECTEDROWS.Length - 1)
                    ...
                    If DTROW("ENTRYTYPE") = "INVOICE" Then DT = OBJCMN.Execute_Any_String(" UPDATE INVOICEMASTER SET INVOICE_HOLDINTCALC = 0 FROM INVOICEMASTER INNER JOIN REGISTERMASTER ON INVOICE_REGISTERID = REGISTER_ID WHERE INVOICE_NO = " & Val(DTROW("ENTRYNO")) & " AND REGISTER_NAME = '" & DTROW("REGNAME") & "' AND INVOICE_YEARID = " & YearId, "", "")
