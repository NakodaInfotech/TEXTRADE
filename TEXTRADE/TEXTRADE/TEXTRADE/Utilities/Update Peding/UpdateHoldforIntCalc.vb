        ...
        Try
            ...
            If RBPENDING.Checked = True Then
                ...
                For I As Integer = 0 To Val(SELECTEDROWS.Length - 1)
                    ...
                    If DTROW("ENTRYTYPE") = "AGENCYINVOICE" Then DT = OBJCMN.Execute_Any_String(" UPDATE AGENCYINVOICEMASTER SET AINVOICE_HOLDINTCALC = 1 FROM AGENCYINVOICEMASTER WHERE AINVOICE_NO = " & Val(DTROW("ENTRYNO")) & " AND AINVOICE_YEARID = " & YearId, "", "")
            ...
            If RBENTERED.Checked = True Then
                ...
                For I As Integer = 0 To Val(SELECTEDROWS.Length - 1)
                    ...
                    If DTROW("ENTRYTYPE") = "INVOICE" Then DT = OBJCMN.Execute_Any_String(" UPDATE INVOICEMASTER SET INVOICE_HOLDINTCALC = 0, INVOICE_CHANGEDATE = '" & Format(Convert.ToDateTime(DTROW("NEWDATE")).Date, "MM/dd/yyyy") & "', INVOICE_CRDAYS = INVOICE_CRDAYS + DATEDIFF(DAY, INVOICE_DATE, '" & Format(Convert.ToDateTime(DTROW("NEWDATE")), "yyyy-MM-dd") & "'), INVOICE_DUEDATE = DATEADD(DAY,INVOICE_CRDAYS + DATEDIFF(DAY, INVOICE_DATE, '" & Format(Convert.ToDateTime(DTROW("NEWDATE")), "yyyy-MM-dd") & "'), INVOICE_DATE) FROM INVOICEMASTER INNER JOIN REGISTERMASTER ON INVOICE_REGISTERID = REGISTER_ID WHERE INVOICE_NO = " & Val(DTROW("ENTRYNO")) & " AND REGISTER_NAME = '" & DTROW("REGNAME") & "' AND INVOICE_YEARID = " & YearId, "", "")
                    If DTROW("ENTRYTYPE") = "PURCHASE" Then DT = OBJCMN.Execute_Any_String(" UPDATE PURCHASEMASTER SET BILL_HOLDINTCALC = 0, BILL_CHANGEDATE = '" & Format(Convert.ToDateTime(DTROW("NEWDATE")).Date, "MM/dd/yyyy") & "', BILL_CRDAYS = BILL_CRDAYS + DATEDIFF(DAY, BILL_PARTYBILLDATE, '" & Format(Convert.ToDateTime(DTROW("NEWDATE")), "yyyy-MM-dd") & "'), BILL_DUEDATE = DATEADD(DAY,BILL_CRDAYS + DATEDIFF(DAY, BILL_PARTYBILLDATE, '" & Format(Convert.ToDateTime(DTROW("NEWDATE")), "yyyy-MM-dd") & "'), BILL_PARTYBILLDATE) FROM PURCHASEMASTER INNER JOIN REGISTERMASTER ON BILL_REGISTERID = REGISTER_ID WHERE BILL_NO = " & Val(DTROW("ENTRYNO")) & " AND REGISTER_NAME = '" & DTROW("REGNAME") & "' AND BILL_YEARID = " & YearId, "", "")
                    ...
                    If DTROW("ENTRYTYPE") = "AGENCYINVOICE" Then DT = OBJCMN.Execute_Any_String(" UPDATE AGENCYINVOICEMASTER SET AINVOICE_HOLDINTCALC = 0, AINVOICE_CRDAYS = AINVOICE_CRDAYS + DATEDIFF(DAY, AINVOICE_DATE, '" & Format(Convert.ToDateTime(DTROW("NEWDATE")), "yyyy-MM-dd") & "'), AINVOICE_DUEDATE = DATEADD(DAY,AINVOICE_CRDAYS + DATEDIFF(DAY, AINVOICE_DATE, '" & Format(Convert.ToDateTime(DTROW("NEWDATE")), "yyyy-MM-dd") & "'), AINVOICE_DATE)  FROM AGENCYINVOICEMASTER WHERE AINVOICE_NO = " & Val(DTROW("ENTRYNO")) & " AND AINVOICE_YEARID = " & YearId, "", "")
SKIPLINE:
                Next
                MsgBox("Details Updated Successfully")
                FILLGRID()
