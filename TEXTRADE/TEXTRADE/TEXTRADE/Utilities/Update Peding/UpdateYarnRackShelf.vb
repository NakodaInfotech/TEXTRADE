...
Public Class UpdateYarnRackShelf
    ...
    Private Sub UpdateYarnRackShelf_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            If EDIT = True Then
                ...
                Dim dttable As DataTable = OBJRACK.SELECTUPDATEYARNRACKSHELF(TEMPENTRYNO, YearId)
    ...
    Sub GETMAXNO()
        ...
        DTTABLE = getmax(" isnull(max(UPDATE_no),0) + 1 ", " UPDATEYARNRACKSHELF ", " AND UPDATE_yearid=" & YearId)
        ...
        Try
            ...
            For I As Integer = 0 To gridbill.RowCount - 1
                ...
                If GRIDSRNO = "" Then
                    ...
                    If IsDBNull(ROW("BILLNO")) <> True Then BILLNO = ROW("BILLNO") Else BILLNO = ""
                    BARCODE = ROW("BARCODE")
                ...
                Else
                    ...
                    BAGS = BAGS & "|" & Val(ROW("BAGS"))
                    CONES = CONES & "|" & ROW("CONES")
                    WT = WT & "|" & Val(ROW("WT"))
                    CATEGORY = CATEGORY & "|" & ROW("CATEGORY")
                    LRNO = LRNO & "|" & ROW("LRNO")
                    MILLNAME = MILLNAME & "|" & ROW("MILLNAME")
                    If IsDBNull(ROW("BILLNO")) <> True Then BILLNO = BILLNO & "|" & ROW("BILLNO") Else BILLNO = BILLNO & "|" & ""
                    BARCODE = BARCODE & "|" & ROW("BARCODE")
                    JOBBERNAME = JOBBERNAME & "|" & ROW("JOBBERNAME")
    ...
    Sub FILLGRID()
        Try
            ...
            Dim dt As DataTable = objclsCMST.Execute_Any_String(" SELECT *, 0 as SRNO FROM YARNBARCODESTOCK WHERE YARNBARCODESTOCK.YEARID = 0 ", "", "")
...
LINE1:
            ...
            If TEMPENTRYNO > 0 Then
                EDIT = True
                UpdateYarnRackShelf_Load(sender, e)
            Else
                CLEAR()
                EDIT = False
            End If
    ...
    Private Sub TXTBARCODE_Validated(sender As Object, e As EventArgs) Handles TXTBARCODE.Validated
        Try
            If TXTBARCODE.Text.Trim.Length > 0 Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.SEARCH(" TOP 1 * ", "", "YARNBARCODESTOCK", " AND BARCODE = '" & TXTBARCODE.Text.Trim & "' AND DONE = 0 AND YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
