...
Public Class CoverNote
    ...
    Private Sub CMDSELECTINV_Click(sender As Object, e As EventArgs) Handles CMDSELECTINV.Click
        Try
            ...
            Dim TEMPAGENINVNO As String = ""
            Dim TEMPAGENINVINITIALS As String = ""
            ...
            If DTTABLE.Rows.Count > 0 Then
                ...
                Dim NEWDT As DataTable = DV.ToTable(True, "INVNO", "PRINTINITIALS", "TYPE")

                Dim hasInvoice As Boolean = NEWDT.Select("TYPE='INVOICE'").Length > 0
                Dim hasAgency As Boolean = NEWDT.Select("TYPE='AGENCYINVOICE'").Length > 0
                If hasInvoice Then
                    For Each DTR As DataRow In NEWDT.Rows
                        If DTR("TYPE").ToString() = "INVOICE" Then
                ...
                If hasAgency Then
                    For Each DTR As DataRow In NEWDT.Rows
                        If TEMPAGENINVNO.Trim = "" Then
                            TEMPAGENINVNO = DTR("INVNO").ToString
                        Else
                            TEMPAGENINVNO = TEMPAGENINVNO & "," & DTR("INVNO").ToString
                        End If
                    Next

                    For Each DTR As DataRow In NEWDT.Rows
                        If TEMPAGENINVINITIALS.Trim = "" Then
                            TEMPAGENINVINITIALS = "'" & DTR("PRINTINITIALS").ToString & "'"
                        Else
                            TEMPAGENINVINITIALS = TEMPAGENINVINITIALS & "," & "'" & DTR("PRINTINITIALS").ToString & "'"
                        End If
                    Next
                End If
                'For Each DTR As DataRow In NEWDT.Rows
                '    If TEMPTYPE.Trim = "" Then
                '        TEMPTYPE = "'" & DTR("TYPE").ToString & "'"
                '    Else
                '        TEMPTYPE = TEMPTYPE & "," & "'" & DTR("TYPE").ToString & "'"
                '    End If
                'Next
                'Dim invoiceCondition As String = ""
                'Dim agencyCondition As String = ""

                'For Each row As DataRow In NEWDT.Rows

                '    Dim invNo As String = row("INVNO").ToString()
                '    Dim printinitials As String = row("PRINTINITIALS").ToString() '.Replace("'", "''")
                '    Dim type As String = row("TYPE").ToString()

                '    If type = "INVOICE" Then
                '        If invoiceCondition <> "" Then invoiceCondition &= " OR "
                '        invoiceCondition &= "  (INVOICEMASTER.INVOICE_NO = " & invNo &
                '            " AND INVOICEMASTER.INVOICE_PRINTINITIALS = '" & printinitials & "')"

                '    ElseIf type = "AGENCYINVOICE" Then
                '        If agencyCondition <> "" Then agencyCondition &= " OR "
                '        agencyCondition &= " (AGENCYINVOICEMASTER.AINVOICE_NO = " & invNo &
                '           " AND AGENCYINVOICEMASTER.AINVOICE_PRINTINITIALS = '" & printinitials & "')"
                '    End If
                ...
                Dim OBJCMN As New ClsCommon()
                    ...
                    'DT1 = OBJCMN.SEARCH(" INVOICEMASTER.INVOICE_NO AS INVNO, INVOICEMASTER.INVOICE_INITIALS AS INVINITIALS, INVOICEMASTER.INVOICE_DATE AS INVDATE, INVOICEMASTER.INVOICE_PRINTINITIALS AS PRINTINITIALS, INVOICEMASTER.INVOICE_LRNO AS LRNO, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSPORT, INVOICEMASTER.INVOICE_LRDATE AS LRDATE, REGISTERMASTER.register_name AS REGNAME, INVOICEMASTER.INVOICE_TOTALPCS AS TOTALPCS, INVOICEMASTER.INVOICE_TOTALMTRS AS TOTALMTRS, INVOICEMASTER.INVOICE_GRANDTOTAL AS GRANDTOTAL, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENTNAME, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(SHIPTOLEDGERS.Acc_cmpname, '') AS SHIPTO  ", "", " INVOICEMASTER INNER JOIN LEDGERS AS SHIPTOLEDGERS ON INVOICEMASTER.INVOICE_PACKINGID = SHIPTOLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON INVOICEMASTER.INVOICE_AGENTID = AGENTLEDGERS.Acc_id AND INVOICEMASTER.INVOICE_YEARID = AGENTLEDGERS.Acc_yearid LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON INVOICEMASTER.INVOICE_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS ON INVOICEMASTER.INVOICE_YEARID = LEDGERS.Acc_yearid AND INVOICEMASTER.INVOICE_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN REGISTERMASTER ON INVOICEMASTER.INVOICE_YEARID = REGISTERMASTER.register_yearid AND INVOICEMASTER.INVOICE_REGISTERID = REGISTERMASTER.register_id  ", "  AND INVOICEMASTER.INVOICE_NO in ( " & TEMPINVNO & ") AND INVOICEMASTER.INVOICE_INITIALS in ( " & TEMPINVINITIALS & ") AND INVOICEMASTER.INVOICE_YEARID = " & YearId)
                    'Dim DT As DataRow = NEWDT.Rows(0)
                    'TEMPTYPE = DT("TYPE").ToString
                    'Dim hasInvoice As Boolean = NEWDT.Select("TYPE='INVOICE'").Length > 0
                    'Dim hasAgency As Boolean = NEWDT.Select("TYPE='AGENCYINVOICE'").Length > 0
                    'If TEMPTYPE = "INVOICE" Then
                    If hasInvoice Then
                        'DT1 = OBJCMN.SEARCH(" * ", "", " (SELECT INVOICEMASTER.INVOICE_NO AS INVNO, INVOICEMASTER.INVOICE_INITIALS AS INVINITIALS, INVOICEMASTER.INVOICE_DATE AS INVDATE, INVOICEMASTER.INVOICE_PRINTINITIALS AS PRINTINITIALS, INVOICEMASTER.INVOICE_LRNO AS LRNO, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSPORT, INVOICEMASTER.INVOICE_LRDATE AS LRDATE, REGISTERMASTER.register_name AS REGNAME, INVOICEMASTER.INVOICE_TOTALPCS AS TOTALPCS, INVOICEMASTER.INVOICE_TOTALMTRS AS TOTALMTRS, INVOICEMASTER.INVOICE_GRANDTOTAL AS GRANDTOTAL, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENTNAME, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(SHIPTOLEDGERS.Acc_cmpname, '') AS SHIPTO, 'INVOICE' AS TYPE FROM INVOICEMASTER INNER JOIN LEDGERS AS SHIPTOLEDGERS ON INVOICEMASTER.INVOICE_PACKINGID = SHIPTOLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON INVOICEMASTER.INVOICE_AGENTID = AGENTLEDGERS.Acc_id AND INVOICEMASTER.INVOICE_YEARID = AGENTLEDGERS.Acc_yearid LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON INVOICEMASTER.INVOICE_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS ON INVOICEMASTER.INVOICE_YEARID = LEDGERS.Acc_yearid AND INVOICEMASTER.INVOICE_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN REGISTERMASTER ON INVOICEMASTER.INVOICE_YEARID = REGISTERMASTER.register_yearid AND INVOICEMASTER.INVOICE_REGISTERID = REGISTERMASTER.register_id  WHERE INVOICEMASTER.INVOICE_NO in ( " & TEMPINVNO & ") AND INVOICEMASTER.INVOICE_INITIALS in ( " & TEMPINVINITIALS & ") AND INVOICEMASTER.INVOICE_YEARID = " & YearId & " UNION ALL SELECT  AGENCYINVOICEMASTER.AINVOICE_NO AS INVNO, AGENCYINVOICEMASTER.AINVOICE_INITIALS AS INVINITIALS, AGENCYINVOICEMASTER.AINVOICE_DATE AS INVDATE,  AGENCYINVOICEMASTER.AINVOICE_PRINTINITIALS AS PRINTINITIALS, AGENCYINVOICEMASTER.AINVOICE_LRNO AS LRNO, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSPORT,  AGENCYINVOICEMASTER.AINVOICE_LRDATE AS LRDATE, '' AS REGNAME, AGENCYINVOICEMASTER.AINVOICE_TOTALPCS AS TOTALPCS, AGENCYINVOICEMASTER.AINVOICE_TOTALMTRS AS TOTALMTRS,  AGENCYINVOICEMASTER.AINVOICE_GRANDTOTAL AS GRANDTOTAL, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENTNAME, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(SHIPTOLEDGERS.Acc_cmpname, '')  AS SHIPTO,'AGENCYINVOICE' AS TYPE FROM AGENCYINVOICEMASTER LEFT OUTER JOIN LEDGERS AS SHIPTOLEDGERS ON AGENCYINVOICEMASTER.AINVOICE_PACKINGID = SHIPTOLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON AGENCYINVOICEMASTER.AINVOICE_AGENTID = AGENTLEDGERS.Acc_id AND AGENCYINVOICEMASTER.AINVOICE_YEARID = AGENTLEDGERS.Acc_yearid LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON AGENCYINVOICEMASTER.AINVOICE_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS ON AGENCYINVOICEMASTER.AINVOICE_YEARID = LEDGERS.Acc_yearid AND AGENCYINVOICEMASTER.AINVOICE_LEDGERID = LEDGERS.Acc_id WHERE AGENCYINVOICEMASTER.AINVOICE_NO in ( " & TEMPINVNO & ") AND AGENCYINVOICEMASTER.AINVOICE_INITIALS in ( " & TEMPINVINITIALS & ") AND AGENCYINVOICEMASTER.AINVOICE_YEARID = " & YearId & ")AS T  ", " ORDER BY T.INVNO  ")

                        DT1 = OBJCMN.SEARCH(" INVOICEMASTER.INVOICE_NO AS INVNO, INVOICEMASTER.INVOICE_INITIALS AS INVINITIALS, INVOICEMASTER.INVOICE_DATE AS INVDATE, INVOICEMASTER.INVOICE_PRINTINITIALS AS PRINTINITIALS, INVOICEMASTER.INVOICE_LRNO AS LRNO, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSPORT, INVOICEMASTER.INVOICE_LRDATE AS LRDATE, REGISTERMASTER.register_name AS REGNAME, INVOICEMASTER.INVOICE_TOTALPCS AS TOTALPCS, INVOICEMASTER.INVOICE_TOTALMTRS AS TOTALMTRS, INVOICEMASTER.INVOICE_GRANDTOTAL AS GRANDTOTAL, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENTNAME, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(SHIPTOLEDGERS.Acc_cmpname, '') AS SHIPTO, 'INVOICE' AS TYPE  ", "", " INVOICEMASTER LEFT OUTER JOIN LEDGERS AS SHIPTOLEDGERS ON INVOICEMASTER.INVOICE_PACKINGID = SHIPTOLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON INVOICEMASTER.INVOICE_AGENTID = AGENTLEDGERS.Acc_id AND INVOICEMASTER.INVOICE_YEARID = AGENTLEDGERS.Acc_yearid LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON INVOICEMASTER.INVOICE_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS ON INVOICEMASTER.INVOICE_YEARID = LEDGERS.Acc_yearid AND INVOICEMASTER.INVOICE_LEDGERID = LEDGERS.Acc_id LEFT OUTER JOIN REGISTERMASTER ON INVOICEMASTER.INVOICE_YEARID = REGISTERMASTER.register_yearid AND INVOICEMASTER.INVOICE_REGISTERID = REGISTERMASTER.register_id  ", " AND INVOICEMASTER.INVOICE_NO in ( " & TEMPINVNO & ") AND INVOICEMASTER.INVOICE_PRINTINITIALS in ( " & TEMPINVINITIALS & ") AND INVOICEMASTER.INVOICE_YEARID = " & YearId)
                        'If DT1.Rows.Count > 0 Then
                        For Each dr As DataRow In DT1.Rows
                                GRIDCOVER.Rows.Add(0, dr("INVNO"), dr("REGNAME"), dr("INVINITIALS"), dr("PRINTINITIALS"), dr("NAME"), dr("AGENTNAME"), Format(Convert.ToDateTime(dr("INVDATE")), "dd/MM/yyyy"), dr("LRNO"), Format(Convert.ToDateTime(dr("LRDATE")), "dd/MM/yyyy"), dr("TRANSPORT"), dr("SHIPTO"), Val(dr("TOTALMTRS")), Val(dr("TOTALPCS")), Val(dr("GRANDTOTAL")), dr("TYPE"))
                            Next

                            'End If
                        End If
                        'If TEMPTYPE = "AGENCYINVOICE" Then
                        If hasAgency Then
                        DT1 = OBJCMN.SEARCH(" AGENCYINVOICEMASTER.AINVOICE_NO AS INVNO, AGENCYINVOICEMASTER.AINVOICE_INITIALS AS INVINITIALS, AGENCYINVOICEMASTER.AINVOICE_DATE AS INVDATE, AGENCYINVOICEMASTER.AINVOICE_PRINTINITIALS AS PRINTINITIALS, AGENCYINVOICEMASTER.AINVOICE_LRNO AS LRNO, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSPORT, AGENCYINVOICEMASTER.AINVOICE_LRDATE AS LRDATE, '' AS REGNAME,  AGENCYINVOICEMASTER.AINVOICE_TOTALPCS AS TOTALPCS, AGENCYINVOICEMASTER.AINVOICE_TOTALMTRS AS TOTALMTRS, AGENCYINVOICEMASTER.AINVOICE_GRANDTOTAL AS GRANDTOTAL, ISNULL(AGENTLEDGERS.Acc_cmpname, '') AS AGENTNAME, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(SHIPTOLEDGERS.Acc_cmpname, '') AS SHIPTO , 'AGENCYINVOICE' AS TYPE ", "", " AGENCYINVOICEMASTER LEFT OUTER JOIN LEDGERS AS SHIPTOLEDGERS ON AGENCYINVOICEMASTER.AINVOICE_PACKINGID = SHIPTOLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON AGENCYINVOICEMASTER.AINVOICE_AGENTID = AGENTLEDGERS.Acc_id AND AGENCYINVOICEMASTER.AINVOICE_YEARID = AGENTLEDGERS.Acc_yearid LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON AGENCYINVOICEMASTER.AINVOICE_TRANSID = TRANSLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS ON AGENCYINVOICEMASTER.AINVOICE_YEARID = LEDGERS.Acc_yearid AND AGENCYINVOICEMASTER.AINVOICE_LEDGERID = LEDGERS.Acc_id  ", " AND AGENCYINVOICEMASTER.AINVOICE_NO in ( " & TEMPAGENINVNO & ") AND AGENCYINVOICEMASTER.AINVOICE_PRINTINITIALS in ( " & TEMPAGENINVINITIALS & ")  AND AGENCYINVOICEMASTER.AINVOICE_YEARID = " & YearId)
                        'If DT1.Rows.Count > 0 Then
                        For Each dr As DataRow In DT1.Rows
                                GRIDCOVER.Rows.Add(0, dr("INVNO"), dr("REGNAME"), dr("INVINITIALS"), dr("PRINTINITIALS"), dr("NAME"), dr("AGENTNAME"), Format(Convert.ToDateTime(dr("INVDATE")), "dd/MM/yyyy"), dr("LRNO"), Format(Convert.ToDateTime(dr("LRDATE")), "dd/MM/yyyy"), dr("TRANSPORT"), dr("SHIPTO"), Val(dr("TOTALMTRS")), Val(dr("TOTALPCS")), Val(dr("GRANDTOTAL")), dr("TYPE"))
                            Next
    ...
    Private Sub cmdok_Click(sender As Object, e As EventArgs) Handles cmdok.Click
        Try
            ...
            If COURIERDATE.Text = "__/__/____" Then alParaval.Add("") Else alParaval.Add(COURIERDATE.Text)
            ...
            For Each row As Windows.Forms.DataGridViewRow In GRIDCOVER.Rows
                If row.Cells(0).Value <> Nothing Then
                    If SRNO = "" Then
                        ...
                        SHIPTO = row.Cells(GSHIPTO.Index).Value.ToString
                    ...
                    Else
                        ...
                        SHIPTO = SHIPTO & "|" & row.Cells(GSHIPTO.Index).Value.ToString
            ...
            If EDIT = True Then
                ...
                If DT.Rows.Count > 0 Then
                    ...
                    For Each dr As DataRow In DT.Rows
                        ...
                        GRIDCOVER.Rows.Add(dr("SRNO"), dr("INVNO"), dr("REGNAME"), dr("INVINITIALS"), dr("PRINTINITIALS"), dr("PARTYNAME"), dr("AGENTNAME"), Format(Convert.ToDateTime(dr("INVDATE")), "dd/MM/yyyy").ToString, dr("LRNO"), dr("LRDATE"), dr("TRANSPORT"), dr("SHIPTO"), Val(dr("TOTALMTRS")), Val(dr("TOTALPCS")), Val(dr("GRANDTOTAL")), dr("TYPE"))
    ...
    Async Sub SENDWHATSAPP(COVERNOTENO As Integer)
        Try
            ...
            If TEMPMSG2 = vbYes Then
                ...
                OBJSR.PARTYNAME = CMBNAME.Text.Trim
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.SEARCH("ISNULL(STATE_REMARK,'') AS STATECODE, ISNULL(LEDGERS.ACC_WHATSAPPNO,'') AS WHATSAPPNO", "", " COVERNOTE INNER JOIN LEDGERS ON COVER_LEDGERID = LEDGERS.ACC_ID LEFT OUTER JOIN STATEMASTER ON LEDGERS.ACC_STATEID = STATE_ID  ", " AND COVERNOTE.COVER_NO = " & Val(COVERNOTENO) & " AND COVERNOTE.COVER_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then WHATSAPPNO = DT.Rows(0).Item("WHATSAPPNO")
                OBJSR.PRINTSETTING = PRINTDIALOG
                ...
                OBJWHATSAPP.PATH.Add(Application.StartupPath & "\" & CMBNAME.Text.Trim & "_COVERNOTE_" & Val(COVERNOTENO) & ".pdf")
                OBJWHATSAPP.FILENAME.Add(CMBNAME.Text.Trim & "_COVERNOTE_" & Val(COVERNOTENO) & ".pdf")
            ...
            If TEMPMSG3 = vbYes Then
                ...
                OBJSR.DIRECTMAIL = True
                OBJSR.WHERECLAUSE = "{COVERNOTE.COVER_NO}=" & Val(COVERNOTENO) & " and {COVERNOTE.COVER_YEARID}=" & YearId
                OBJSR.AGENTNAME = CMBAGENT.Text.Trim
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.SEARCH("ISNULL(STATE_REMARK,'') AS STATECODE, ISNULL(LEDGERS.ACC_WHATSAPPNO,'') AS WHATSAPPNO", "", " COVERNOTE INNER JOIN LEDGERS ON COVER_LEDGERID = LEDGERS.ACC_ID LEFT OUTER JOIN STATEMASTER ON LEDGERS.ACC_STATEID = STATE_ID  ", " AND COVERNOTE.COVER_NO = " & Val(COVERNOTENO) & " AND COVERNOTE.COVER_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then WHATSAPPNO = DT.Rows(0).Item("WHATSAPPNO")
                OBJSR.PRINTSETTING = PRINTDIALOG
                ...
                OBJWHATSAPP.PARTYNAME = CMBNAME.Text.Trim
                OBJWHATSAPP.AGENTNAME = CMBAGENT.Text.Trim
                OBJWHATSAPP.PATH.Add(Application.StartupPath & "\" & CMBAGENT.Text.Trim & "_AGENTCOVERNOTE_" & Val(COVERNOTENO) & ".pdf")
                OBJWHATSAPP.FILENAME.Add(CMBAGENT.Text.Trim & "_AGENTCOVERNOTE_" & Val(COVERNOTENO) & ".pdf")
                OBJWHATSAPP.ShowDialog()
