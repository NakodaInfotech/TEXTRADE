...
Public Class DesignCardMaster
    ...
    Sub FILLWARPGRID()
        ...
        ElseIf GRIDDOUBLECLICK = True Then
            ...
            GRIDWARP.Item(WSHADE.Index, TEMPROW).Value = cmbgridshade.Text.Trim
   ...
   Not String.IsNullOrWhiteSpace(row.Cells(SPSYM.Index).Value.ToString()) Then
    ...
    Sub FILLWEFTGRID()
        ...
        ElseIf GRIDWEFTDOUBLECLICK = True Then
            ...
            GRIDWEFT.Item(FSHADE.Index, TEMPWEFTROW).Value = cmbweftgridshade.Text.Trim
...
LINE1:
    ...
    Private Sub cmbgridshade_Enter(sender As Object, e As EventArgs) Handles cmbgridshade.Enter
        Try
            If cmbgridshade.Text.Trim = "" Then FILLCOLOR(cmbgridshade, "", "")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub cmbgridshade_Validating(sender As Object, e As CancelEventArgs) Handles cmbgridshade.Validating
        Try
            If cmbgridshade.Text.Trim <> "" Then COLORVALIDATE(cmbgridshade, e, Me, "", "")
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    End Sub
...
line1:
    ...
    Sub SAVEBEAMDESIGN(EDIT As Boolean)
        Try
            ...
            Dim DT3 As DataTable = OBJCMN.SEARCH("BEAM_ID", "", "BEAMMASTER",
            " and BEAM_NAME = '" & CMBITEMNAME.Text & "' and BEAM_YEARid = " & YearId)

            If DT3.Rows.Count > 0 Then
                TEMPBEAMID = DT3.Rows(0).Item("BEAM_ID")
            End If

            Dim DT As DataTable = OBJCMN.SEARCH(
            "ISNULL(HSNMASTER.HSN_CODE, 0) AS HSNCODE", "",
            "ITEMMASTER LEFT OUTER JOIN HSNMASTER ON ITEMMASTER.ITEM_HSNCODEID = HSNMASTER.HSN_ID",
            " and ITEMMASTER.ITEM_NAME = '" & CMBITEMNAME.Text & "' and ITEMMASTER.ITEM_YEARid = " & YearId)

            alParaval.Add(CMBITEMNAME.Text.Trim)
            If DT.Rows.Count > 0 Then
                alParaval.Add(DT.Rows(0).Item("HSNCODE"))
            Else
                alParaval.Add("0")
            End If
            ...
            Dim DT2 As DataTable = OBJCMN.SEARCH("ITEM_ID", "", "ITEMMASTER",
            " and ITEM_NAME = '" & CMBITEMNAME.Text & "' and ITEM_YEARid = " & YearId)
            If DT2.Rows.Count > 0 Then TEMPITEMID = DT2.Rows(0).Item("ITEM_ID")

            Dim DT1 As DataTable = OBJCMN.SEARCH("BEAM_ID", "", "BEAMMASTER",
            " and BEAM_NAME = '" & CMBITEMNAME.Text & "' and BEAM_YEARid = " & YearId)
