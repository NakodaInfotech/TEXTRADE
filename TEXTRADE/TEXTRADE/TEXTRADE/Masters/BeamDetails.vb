...
Public Class BeamDetails
    ...
    Sub fillgrid()
        Try
            ...
            Dim dttable As DataTable = OBJCMN.search(" BEAMMASTER.BEAM_NAME AS BEAMNAME, HSNMASTER.HSN_CODE AS HSNCODE, YARNQUALITYMASTER.YARN_NAME AS QUALITY, COLORMASTER.COLOR_name AS SHADE, BEAMMASTER_DESC.BEAM_GRIDWT AS GRIDWTMTRS,  BEAMMASTER.BEAM_ID AS BEAMID, BEAMMASTER.BEAM_YEARID , ISNULL(BEAMMASTER.BEAM_TAPLINE,0)  AS TAPLINE  , ISNULL(BEAMMASTER_DESC.BEAM_GRIDENDS,0)  AS ENDS  ", "", " BEAMMASTER INNER JOIN BEAMMASTER_DESC ON BEAMMASTER.BEAM_ID = BEAMMASTER_DESC.BEAM_ID AND BEAMMASTER.BEAM_YEARID = BEAMMASTER_DESC.BEAM_YEARID INNER JOIN HSNMASTER ON BEAMMASTER.BEAM_HSNCODEID = HSNMASTER.HSN_ID INNER JOIN YARNQUALITYMASTER ON BEAMMASTER_DESC.BEAM_GRIDQUALITYID = YARNQUALITYMASTER.YARN_ID LEFT OUTER JOIN COLORMASTER ON BEAMMASTER_DESC.BEAM_SHADEID = COLORMASTER.COLOR_id   ", "  and BEAMMASTER.BEAM_yearid = " & YearId)
