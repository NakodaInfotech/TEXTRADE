...
Public Class StoresReOrderLevelReport
    ...
    Sub FILLGRID()
        Try
            ...
            'Dim DT As DataTable = OBJCMN.SEARCH(" T.ITEMNAME, T.QTY, ROUND(ISNULL(SUM(QTY)-SUM(ISSQTY),0),2) AS STOCKQTY ", "", " (SELECT STOREITEMMASTER.STOREITEM_NAME AS ITEMNAME, STORESREORDERLEVEL.REORDER_QTY AS QTY, STORESREORDER_YEARID AS YEARID FROM STORESREORDERLEVEL INNER JOIN STOREITEMMASTER ON STORESREORDERLEVEL.REORDER_ITEMID = STOREITEMMASTER.item_id) AS T LEFT OUTER JOIN STORESSTOCKREGISTER ON T.ITEMNAME = STORESSTOCKREGISTER.ITEMNAME AND T.YEARID= STORESSTOCKREGISTER.YEARID ", " T.YEARID = " & YearId & " GROUP BY T.ITEMNAME")
            Dim DT As DataTable = OBJCMN.SEARCH(" T.ITEMNAME, T.QTY, ROUND(ISNULL(SUM(T.QTY)-SUM(ISSQTY),0),2) AS STOCKQTY ", "", " (SELECT STOREITEMMASTER.STOREITEM_NAME AS ITEMNAME, STORESREORDERLEVEL.REORDER_QTY AS QTY, STORESREORDERLEVEL.REORDER_YEARID AS YEARID FROM STORESREORDERLEVEL INNER JOIN STOREITEMMASTER ON STORESREORDERLEVEL.REORDER_ITEMID = STOREITEMMASTER.STOREITEM_ID) AS T LEFT OUTER JOIN STORESTOCKREGISTER ON T.ITEMNAME = STORESTOCKREGISTER.ITEMNAME AND T.YEARID= STORESTOCKREGISTER.YEARID  ", " and T.YEARID = " & YearId & " GROUP BY T.ITEMNAME,T.QTY")
