...
Public Class BaleWiseStockDetails
    ...
    Sub fillgrid()
        ...
        If ClientName = "SAKARIA" Then
            ...
            DT = OBJCMN.SEARCH(" BARCODESTOCK.JOBBERNAME AS NAME, BARCODESTOCK.ITEMNAME, BARCODESTOCK.BALENO, SUM(PCS) AS PCS, SUM(MTRS) AS MTRS, ROUND(ISNULL((BALEWISEPURRATE.TAXABLEAMT/ BALEWISEPURRATE.QTY), BARCODESTOCK.PURRATE),2) AS RATE, ROUND(ISNULL((ROUND((BALEWISEPURRATE.TAXABLEAMT/ BALEWISEPURRATE.QTY),2) * (BALEWISEPURRATE.QTY)), BARCODESTOCK.PURRATE*BARCODESTOCK.MTRS) ,2) AS AMOUNT, BARCODESTOCK.DESIGNNO, BARCODESTOCK.BARCODE ", "", " BARCODESTOCK LEFT OUTER JOIN BALEWISEPURRATE ON BARCODESTOCK.ITEMNAME = BALEWISEPURRATE.ITEMNAME AND BARCODESTOCK.BALENO = BALEWISEPURRATE.BALENO AND BARCODESTOCK.JOBBERNAME = BALEWISEPURRATE.NAME  ", " AND BARCODESTOCK.YEARID = " & YearId & " GROUP BY BARCODESTOCK.JOBBERNAME, BARCODESTOCK.ITEMNAME, BARCODESTOCK.BALENO, BALEWISEPURRATE.TAXABLEAMT, BALEWISEPURRATE.QTY, BARCODESTOCK.PURRATE, BARCODESTOCK.MTRS, BARCODESTOCK.DESIGNNO, BARCODESTOCK.BARCODE  ORDER BY BARCODESTOCK.JOBBERNAME, BARCODESTOCK.ITEMNAME, BARCODESTOCK.BALENO ")
        Else
