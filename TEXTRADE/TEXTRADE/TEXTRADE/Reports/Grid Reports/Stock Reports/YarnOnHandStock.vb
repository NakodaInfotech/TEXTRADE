    ...
    Sub fillgrid(ByVal TEMPCONDITION)
        Try
            ...
            If ALLOWYARNBARCODEPRINT = True Then
                Dim dt As DataTable = OBJCMN.SEARCH(" GODOWN, YARNQUALITY, CATEGORY, MILLNAME, DESIGNNO, COLOR, LOTNO,  SUM(ISNULL(CONES,0)) AS CONES, SUM(BAGS) AS BAGS, SUM(WT) AS WT, BARCODE , BILLNO , JOBBERNAME , RACK, LRNO ", "", "  YARNBARCODESTOCK ", TEMPCONDITION & " GROUP BY GODOWN, YARNQUALITY, CATEGORY, MILLNAME, DESIGNNO, COLOR, LOTNO , BARCODE , BILLNO , JOBBERNAME , LRNO , RACK HAVING SUM(WT) > 0")
                ...
                End If
            Else
                Dim dt As DataTable = OBJCMN.SEARCH(" GODOWN, YARNQUALITY, CATEGORY, MILLNAME, DESIGNNO, COLOR, LOTNO,  SUM(ISNULL(CONES,0)) AS CONES, SUM(BAGS) AS BAGS, SUM(WT) AS WT, LRNO ", "", "  YARNSTOCKVIEW ", TEMPCONDITION & " GROUP BY GODOWN, YARNQUALITY, CATEGORY, MILLNAME, DESIGNNO, COLOR, LRNO, LOTNO HAVING SUM(WT) > 0")
                gridbilldetails.DataSource = dt
                If dt.Rows.Count > 0 Then
                    gridbill.FocusedRowHandle = gridbill.RowCount - 1
                    gridbill.TopRowIndex = gridbill.RowCount - 15
