...
Public Class DesignwiseStock
    ...
    Sub fillgrid(ByVal TEMPCONDITION)
        Try
            ...
            Dim GROUPBY As String = " GROUP BY ISNULL(BARCODESTOCK.PURRATE, 0), QUALITY,DESIGNNO"
            'If TEMPQUALITY <> "" Then GROUPBY = GROUPBY & ", QUALITY"

            Dim objclsCMST As New ClsCommonMaster
            Dim dt As DataTable
            'dt = objclsCMST.search("CAST(0 AS BIT) AS SAMPLEMATCH, ITEMNAME, QUALITY, DESIGNNO, SUM(PCS) AS TOTALPCS, SUM(MTRS) AS TOTALMTRS, CATEGORY, isnull(SAMPLEBARCODE.SB_BARCODE,'') as BARCODE", "", "  BARCODESTOCK", WHERECLAUSE & TEMPCONDITION & "GROUP BY ITEMNAME, QUALITY, DESIGNNO, CATEGORY ORDER BY ITEMNAME, DESIGNNO")
            If TEMPQUALITY = "" Then
                dt = objclsCMST.search("CAST(0 AS BIT) AS SAMPLEMATCH, BARCODESTOCK.ITEMNAME, BARCODESTOCK.QUALITY, BARCODESTOCK.DESIGNNO, SUM(BARCODESTOCK.PCS) AS TOTALPCS, SUM(BARCODESTOCK.MTRS) AS TOTALMTRS, BARCODESTOCK.CATEGORY, isnull(SAMPLEBARCODE.SB_BARCODE,'') as BARCODE , ISNULL(BARCODESTOCK.PURRATE, 0) AS PURRATE ", "", "  BARCODESTOCK LEFT OUTER JOIN SAMPLEBARCODE ON BARCODESTOCK.YEARID = SAMPLEBARCODE.SB_YEARID AND BARCODESTOCK.ITEMID = SAMPLEBARCODE.SB_ITEMID AND BARCODESTOCK.DESIGNID = SAMPLEBARCODE.SB_DESIGNID AND ISNULL(SAMPLEBARCODE.SB_COLORID,0)=0", TEMPCONDITION & WHERECLAUSE & "GROUP BY BARCODESTOCK.ITEMNAME, BARCODESTOCK.QUALITY, BARCODESTOCK.DESIGNNO, BARCODESTOCK.CATEGORY, SAMPLEBARCODE.SB_BARCODE , ISNULL(BARCODESTOCK.PURRATE, 0) ORDER BY BARCODESTOCK.ITEMNAME, BARCODESTOCK.DESIGNNO")
            Else
                dt = objclsCMST.search("CAST(0 AS BIT) AS SAMPLEMATCH, SUM(PCS) AS TOTALPCS, SUM(MTRS) AS TOTALMTRS ,ISNULL(BARCODESTOCK.PURRATE, 0) AS PURRATE,'" & TEMPQUALITY & "' AS QUALITY, DESIGNNO", "", "  BARCODESTOCK", TEMPCONDITION & GROUPBY)
            End If
    ...
    Private Sub TXTBARCODE_Validated(sender As Object, e As EventArgs) Handles TXTBARCODE.Validated
        ' OLD CODE BY FETCHING SAMPLEBARCODE WHILE SCANNING EACH BARCODE
        '        Try
        ...
        Try
            If TXTBARCODE.Text.Trim.Length > 0 Then
                ...
                'IF ENTRY IS NOT FOUND THEN GIVE MESSAGE
                If MATCHFOUND = False Then MsgBox("Invalid Barcode : " & TXTBARCODE.Text.Trim, MsgBoxStyle.Critical)
