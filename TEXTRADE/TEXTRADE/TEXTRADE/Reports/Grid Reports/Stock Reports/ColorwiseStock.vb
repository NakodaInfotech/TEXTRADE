        ...
        Try
            ...
            If FRMSTRING = "NILSTOCKREPORT" Then
                ...
                If ALLOWBARCODEPRINT = True Then
                    ...
                    DT = OBJCMN.Execute_Any_String(" IF OBJECT_ID('tempdb..#BSR_TEMP') IS NOT NULL DROP TABLE #BSR_TEMP; SELECT ITEMNAME, DESIGNNO, COLOR, MTRS, CAST([DATE] AS DATE) AS StockDate INTO #BSR_TEMP FROM BARCODESTOCKREGISTER WHERE YEARID = " & YearId & WHERECLAUSE & " AND UNIT IN ('Bales','DAMAGE','FENT','Kgs','LUMP','Mtrs','Pcs','SAMPLE','THAN','UNCHECKED'); CREATE INDEX IX_TEMP_ITEM ON #BSR_TEMP(ITEMNAME, DESIGNNO, COLOR); CREATE INDEX IX_TEMP_DATE ON #BSR_TEMP(StockDate); WITH A AS ( SELECT ITEMNAME, DESIGNNO, COLOR FROM #BSR_TEMP WHERE StockDate <= '" & Format(FROMDATE, "MM/dd/yyyy") & "' GROUP BY ITEMNAME, DESIGNNO, COLOR HAVING ROUND(SUM(MTRS),2) > " & Val(TEMPMTRS) & " ), B AS ( SELECT ITEMNAME, DESIGNNO, COLOR FROM #BSR_TEMP WHERE StockDate <= '" & Format(TODATE, "MM/dd/yyyy") & "' GROUP BY ITEMNAME, DESIGNNO, COLOR HAVING ROUND(SUM(MTRS),2) > " & Val(TEMPMTRS) & " ) SELECT ITEMNAME, DESIGNNO, COLOR FROM A EXCEPT SELECT ITEMNAME, DESIGNNO, COLOR FROM B ORDER BY ITEMNAME, DESIGNNO, COLOR;  DROP TABLE #BSR_TEMP;", "", "")
