...
Public Class ItemMonthlyStockStatement
    ...
    Sub FILLGRID()
        Try
            ...
            Dim EXTRA_WHERE_FOR_OPENING As String = ""

            If ClientName.ToUpper() = "SURYODAYA" Then
                EXTRA_WHERE_FOR_OPENING = " AND STOCKMASTER.SM_TYPE = 'INHOUSE'"
            End If
		...
		SELECT ''
    ) SELECT  MONTHSNAME, ISNULL([OPENING],0) AS [OPENING], ISNULL([FINISH INWARD],0) AS [FINISH INWARD], ISNULL([PUR RETURN],0) AS [PUR RETURN], ISNULL([DYEING REC],0) AS [DYEING REC], CASE WHEN '" & ClientName.ToUpper() & "' = 'SURYODAYA' THEN 0 ELSE ISNULL([ISSUE PACKING],0) END AS [ISSUE PACKING],
    CASE WHEN '" & ClientName.ToUpper() & "' = 'SURYODAYA' THEN 0 ELSE ISNULL([REC PACKING],0) END AS [REC PACKING], ISNULL([JOBOUT],0) AS [JOBOUT], ISNULL([JOBIN],0) AS [JOBIN], ISNULL([STOCK OUT],0) AS [STOCK OUT], ISNULL([STOCK IN],0) AS [STOCK IN], ISNULL([SALE INVOICE],0) AS [SALE INVOICE], ISNULL([SALE RETURN],0) AS [SALE RETURN]
FROM
(
...
SELECT '' AS MONTHNAME, SUM(STOCKMASTER.SM_MTRS) AS MTRS, 'OPENING' AS TYPE FROM STOCKMASTER INNER JOIN ITEMMASTER ON STOCKMASTER.SM_ITEMID = ITEMMASTER.ITEM_ID " & WHERECLAUSE & EXTRA_WHERE_FOR_OPENING & " AND STOCKMASTER.SM_YEARID = " & YearId & "
...
UNION ALL
SELECT UPPER(DATENAME(MONTH,INVOICE_DATE)) AS MONTHNAME, SUM(INVOICEMASTER_DESC.INVOICE_MTRS)*-1 AS MTRS, 'SALE INVOICE' AS TYPE FROM INVOICEMASTER INNER JOIN INVOICEMASTER_DESC ON INVOICEMASTER.INVOICE_NO = INVOICEMASTER_DESC.INVOICE_NO AND INVOICEMASTER.INVOICE_YEARID = INVOICEMASTER_DESC.INVOICE_YEARID AND INVOICEMASTER.INVOICE_REGISTERID = INVOICEMASTER_DESC.INVOICE_REGISTERID INNER JOIN ITEMMASTER ON INVOICEMASTER_DESC.INVOICE_ITEMID = ITEMMASTER.ITEM_ID
" & WHERECLAUSE & " AND INVOICEMASTER.INVOICE_YEARID = " & YearId & "
GROUP BY DATENAME(MONTH,INVOICE_DATE)
