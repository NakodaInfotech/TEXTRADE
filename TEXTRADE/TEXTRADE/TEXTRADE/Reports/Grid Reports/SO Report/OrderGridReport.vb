...
Public Class OrderGridReport
    ...
    Sub FILLGRID()
        Try
            ...
            If CMBCATEGORY.Text = "YARN" Then ORDERTYPE = "YARN" & FRMSTRING

            If FRMSTRING = "SO" Then
                WHERECLAUSE = " AND ALLSALEORDER.SO_yearid=" & YearId
            ElseIf FRMSTRING = "PO" Then
                WHERECLAUSE = " AND ALLPURCHASEORDER.PO_yearid=" & YearId
            ElseIf FRMSTRING = "YARNSO" Then
                WHERECLAUSE = " AND ALLYARNSALEORDER.PO_yearid=" & YearId
            ElseIf FRMSTRING = "YARNPO" Then
                WHERECLAUSE = " AND ALLYARNPURCHASEORDER.PO_yearid=" & YearId
            End If
            ' Apply date filter
            If chkdate.Checked = True Then
                ...
                ElseIf FRMSTRING = "PO" Then
                    WHERECLAUSE &= " AND ALLPURCHASEORDER.Po_date BETWEEN " & fromD & " AND " & toD & ""
                ElseIf FRMSTRING = "YARNSO" Then
                    WHERECLAUSE &= " AND ALLYARNSALEORDER.Yso_date BETWEEN " & fromD & " AND " & toD & ""
                ElseIf FRMSTRING = "YARNPO" Then
                    WHERECLAUSE &= " AND ALLYARNPURCHASEORDER.YPo_date BETWEEN " & fromD & " AND " & toD & ""
                End If
            Else
                If FRMSTRING = "SO" Then PERIOD = " AND ALLSALEORDER.so_date BETWEEN '" & Format(AccFrom, "yyyy-MM-dd") & "' AND '" & Format(AccTo, "yyyy-MM-dd") & "'" Else PERIOD = " AND ALLPURCHASEORDER.Po_date BETWEEN '" & Format(AccFrom, "yyyy-MM-dd") & "' AND '" & Format(AccTo, "yyyy-MM-dd") & "'"
            End If
            If FRMSTRING = "SO" Then
            ...
            ElseIf ORDERTYPE = "PO" Then
                DT = OBJCMN.SEARCH(" ITEMMASTER.item_name AS ITEMNAME, '' AS MILLNAME, ALLPURCHASEORDER.PO_no AS SONO, ALLPURCHASEORDER.PO_date AS SODATE, LEDGERS.Acc_cmpname AS NAME, ISNULL(AGENTLEDGERS.Acc_cmpname,'') AS AGENTNAME, '' AS NOTE, ALLPURCHASEORDER_DESC.PO_QTY AS PCS, ALLPURCHASEORDER_DESC.PO_RECDQTY AS OUTPCS, ALLPURCHASEORDER_DESC.BALANCE AS BALPCS, ALLPURCHASEORDER_DESC.PO_RATE AS RATE,0 AS [DAYS], ISNULL(ITEMMASTER.ITEM_REORDER,0) AS PERDAYPROD ", "", " ALLPURCHASEORDER INNER JOIN ALLPURCHASEORDER_DESC ON ALLPURCHASEORDER.PO_no = ALLPURCHASEORDER_DESC.PO_NO AND ALLPURCHASEORDER.TYPE = ALLPURCHASEORDER_DESC.TYPE AND ALLPURCHASEORDER.PO_YEARID = ALLPURCHASEORDER_DESC.PO_YEARID INNER JOIN ITEMMASTER ON ALLPURCHASEORDER_DESC.PO_ITEMID = ITEMMASTER.item_id INNER JOIN LEDGERS ON ALLPURCHASEORDER.PO_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON ALLPURCHASEORDER.PO_BROKERid = AGENTLEDGERS.Acc_id LEFT OUTER JOIN DESIGNMASTER ON ALLPURCHASEORDER_DESC.PO_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN COLORMASTER ON ALLPURCHASEORDER_DESC.PO_COLORID = COLORMASTER.COLOR_ID", SOCLAUSE & " ORDER BY ITEMMASTER.item_name, ALLPURCHASEORDER.PO_DATE, ALLPURCHASEORDER.PO_NO")
            ElseIf ORDERTYPE = "YARNSO" Then
                DT = OBJCMN.SEARCH(" ITEMMASTER.item_name AS ITEMNAME, '' AS MILLNAME, ALLYARNSALEORDER.YSO_NO AS SONO, ALLYARNSALEORDER.YSO_date AS SODATE, LEDGERS.Acc_cmpname AS NAME, ISNULL(AGENTLEDGERS.Acc_cmpname,'') AS AGENTNAME, ALLYARNSALEORDER.YSO_NOTE AS NOTE, ALLYARNSALEORDER_DESC.YSO_BAGS AS PCS, ALLYARNSALEORDER_DESC.YSO_RECDBAGS AS OUTPCS, ALLYARNSALEORDER_DESC.BALANCE AS BALPCS, ALLYARNSALEORDER_DESC.YSO_RATE AS RATE, 0 AS [DAYS], ISNULL(ITEMMASTER.ITEM_REORDER,0) AS PERDAYPROD  ", "", "  ALLYARNSALEORDER INNER JOIN ALLYARNSALEORDER_DESC ON ALLYARNSALEORDER.YSO_no = ALLYARNSALEORDER_DESC.YSO_NO AND ALLYARNSALEORDER.TYPE = ALLYARNSALEORDER_DESC.TYPE AND ALLYARNSALEORDER.YSO_YEARID = ALLYARNSALEORDER_DESC.YSO_YEARID INNER JOIN ITEMMASTER ON ALLYARNSALEORDER_DESC.YSO_YARNQUALITYID = ITEMMASTER.item_id INNER JOIN LEDGERS ON ALLYARNSALEORDER.YSO_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS AGENTLEDGERS ON ALLYARNSALEORDER.YSO_BROKERID = AGENTLEDGERS.Acc_id LEFT OUTER JOIN DESIGNMASTER ON ALLYARNSALEORDER_DESC.YSO_DESIGNID = DESIGNMASTER.DESIGN_id LEFT OUTER JOIN COLORMASTER ON ALLYARNSALEORDER_DESC.YSO_SHADEID = COLORMASTER.COLOR_ID ", SOCLAUSE & " ORDER BY ITEMMASTER.item_name, ALLYARNSALEORDER.YSO_DATE, ALLYARNSALEORDER.YSO_NO")
            End If
