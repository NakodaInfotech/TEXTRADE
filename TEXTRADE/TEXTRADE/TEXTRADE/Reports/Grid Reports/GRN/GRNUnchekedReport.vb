...
Public Class GRNUnchekedReport
    ...
    Sub fillgrid(ByVal WHERECLAUSE)
        Try
            ...
            Dim dt As DataTable = objclsCMST.search("  GRN.grn_no AS GRNNO, GRN.grn_CHALLANNO AS CHALLANNO, GRN.grn_date AS DATE, LEDGERS.Acc_cmpname AS NAME, ISNULL(JOBBERLEDGERS.Acc_cmpname, '') AS JOBBERNAME, ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN, ISNULL(BROKERLEDGERS.Acc_cmpname, '') AS BROKER, GRN.GRN_TOTALQTY AS QTY, GRN.GRN_TOTALMTRS AS MTRS, GRN.GRN_PLOTNO AS LOTNO,  ISNULL(ITEMMASTER.item_name, '') AS ITEMNAME", "", "  GRN INNER JOIN LEDGERS ON GRN.grn_ledgerid = LEDGERS.Acc_id INNER JOIN GRN_DESC ON GRN.grn_no = GRN_DESC.GRN_NO AND GRN.GRN_TYPE = GRN_DESC.GRN_GRIDTYPE AND GRN.grn_yearid = GRN_DESC.GRN_YEARID LEFT OUTER JOIN ITEMMASTER ON GRN_DESC.GRN_ITEMID = ITEMMASTER.item_id LEFT OUTER JOIN LEDGERS AS BROKERLEDGERS ON GRN.GRN_BROKERID = BROKERLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS JOBBERLEDGERS ON GRN.GRN_TOLEDGERID = JOBBERLEDGERS.Acc_id LEFT OUTER JOIN GODOWNMASTER ON GRN.GRN_GODOWNID = GODOWNMASTER.GODOWN_id", WHERECLAUSE & " GROUP BY  GRN.grn_no , GRN.grn_CHALLANNO , GRN.grn_date , LEDGERS.Acc_cmpname , ISNULL(JOBBERLEDGERS.Acc_cmpname, '') , ISNULL(GODOWNMASTER.GODOWN_name, '') , ISNULL(BROKERLEDGERS.Acc_cmpname, '') , GRN.GRN_TOTALQTY , GRN.GRN_TOTALMTRS , GRN.GRN_PLOTNO , GRN.GRN_TYPE,  ISNULL(ITEMMASTER.item_name, '')  ORDER BY GRN.grn_no ")
