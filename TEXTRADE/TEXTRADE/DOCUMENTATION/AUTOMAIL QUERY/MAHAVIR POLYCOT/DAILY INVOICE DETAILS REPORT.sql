...
SELECT @TOTALPCS = SUM(INVOICEMASTER.INVOICE_TOTALPCS)  FROM  INVOICEMASTER WHERE INVOICE_DATE = CAST(GETDATE() AS DATE)
SELECT @TOTALAMT = SUM(INVOICEMASTER.INVOICE_GRANDTOTAL)  FROM  INVOICEMASTER WHERE INVOICE_DATE = CAST(GETDATE()as DATE)
SET @SUBJECT = 'DAILY INVOICE DETAILS REPORT FOR ' + CAST(DAY( GETDATE()) AS VARCHAR(2)) + '-' + CAST(MONTH( GETDATE()-1) AS VARCHAR(2)) + '-' + CAST(YEAR( GETDATE()-1) AS VARCHAR(4))
...
where INVOICE_DATE = CAST(GETDATE() AS DATE)
) AS T
