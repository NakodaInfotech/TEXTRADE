DECLARE @SUBJECT VARCHAR(100), @ENTRYBODY VARCHAR(MAX), @PENDINGPO DECIMAL(18,2), @PENDINGPOVALUE  DECIMAL(18,2), @NOOFPO INT, @TOTALAMT FLOAT, @PENDINGSO DECIMAL(18,2), @PENDINGSOVALUE  DECIMAL(18,2), @NOOFSO INT, @TOTALINVAMT DECIMAL(18,2), @TOTALINVMTRS DECIMAL(18,2), @NOOFINV INT, @RECDAMT DECIMAL(18,2), @PAIDAMT DECIMAL(18,2), @BANKBALANCE DECIMAL(18,2), @DYEINGREC DECIMAL(18,2), @JOBIN DECIMAL(18,2), @DYEINGBALANCE DECIMAL(18,2)
...
SELECT @TOTALINVAMT =  SUM(INVOICE_SUBTOTAL), @TOTALINVMTRS = SUM(INVOICE_TOTALMTRS), @NOOFINV = COUNT(INVOICE_NO) FROM INVOICEMASTER WHERE INVOICE_DATE = CAST(GETDATE() -1 AS DATE)
SET @SUBJECT = 'MIS SUMMARY REPORT FOR ' + CAST(DAY( GETDATE()) AS VARCHAR(2)) + '-' + CAST(MONTH( GETDATE()-1) AS VARCHAR(2)) + '-' + CAST(YEAR( GETDATE()-1) AS VARCHAR(4))
...
SET @BODY = N'<H1 style="font-family:Tahoma; font-size:11px;">DAILY RECEIPT DETAILS</H1>' +
				...
				<td style="font-family:Tahoma; font-size:11px; font-weight:bold; color:black;" align="right" bgcolor="yellow">' + CAST(@TOTALINVAMT AS VARCHAR) + N'</td>
				</tr></tfoot></Table></html>'
