DECLARE @SUBJECT VARCHAR(100), @ENTRYBODY VARCHAR(MAX), @TOTALMTRS DECIMAL(18,2), @CMPNAME VARCHAR(100), @YEARID INT
SET @CMPNAME = 'MAHAVIR POLYCOT'
...
WHERE CAST(SALEORDER_DESC.SO_CLOSEDDATE AS DATE) = CAST(GETDATE() AS DATE) AND SALEORDER_DESC.SO_CLOSED = 'TRUE' AND SALEORDER.SO_YEARID = @YEARID
