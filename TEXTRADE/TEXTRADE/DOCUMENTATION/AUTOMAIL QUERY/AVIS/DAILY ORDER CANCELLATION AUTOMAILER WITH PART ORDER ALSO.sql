...
SELECT @TOTALMTRS = ROUND(SUM(SALEORDER_DESC.SO_MTRS)-SUM(SALEORDER_DESC.SO_RECDMTRS),2) FROM  SALEORDER INNER JOIN SALEORDER_DESC ON SALEORDER.SO_NO = SALEORDER_DESC.SO_NO AND SALEORDER.SO_YEARID = SALEORDER_DESC.SO_YEARID  WHERE CAST(SALEORDER_DESC.SO_CLOSEDDATE AS DATE) = CAST(GETDATE() AS DATE) AND SALEORDER_DESC.SO_CLOSED = 'TRUE'
...
[TD/@style]='font-family:Tahoma; font-size:11px;', [TD/@align]='right',[TD/@width]='80px', TD=ROUND(SUM(CAST(SALEORDER_DESC.SO_MTRS AS DECIMAL(18,2))) - SUM(CAST(SALEORDER_DESC.SO_RECDMTRS AS DECIMAL(18,2))),2) ,''
...
--FOR DETAILS***********************************************************
begin
DECLARE @PREVIOUSNAME VARCHAR(100), @DETAILSBODY VARCHAR(MAX), @PARTYMTRS DECIMAL(18,2), @NAME VARCHAR(100), @SONO INT, @SODATE DATE, @DESIGNNO VARCHAR(100), @SHADE VARCHAR(100), @MTRS DECIMAL(18,2), @REASON VARCHAR(200), @ITEMNAME VARCHAR(100)
SET @PREVIOUSNAME = ''
SET @PARTYMTRS = 0
SET @DETAILSBODY = ''
declare CUR_MAIN cursor for SELECT        ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, SALEORDER.so_no AS SONO, SALEORDER.so_date AS SODATE, ISNULL(ITEMMASTER.ITEM_NAME,'') AS ITEMNAME, ISNULL(DESIGNMASTER.DESIGN_NO, '') AS DESIGNNO, ISNULL(COLORMASTER.COLOR_name, 
                         '') AS SHADE, ROUND(SUM(SALEORDER_DESC.SO_MTRS)-SUM(SALEORDER_DESC.SO_RECDMTRS),2) AS MTRS, ISNULL(SALEORDER_DESC.SO_CLOSEDREASON,'') AS REASON
FROM            SALEORDER INNER JOIN
                         ...
                         ITEMMASTER ON SALEORDER_DESC.SO_ITEMID = ITEMMASTER.ITEM_id LEFT OUTER JOIN
                         DESIGNMASTER ON SALEORDER_DESC.SO_DESIGNID = DESIGNMASTER.DESIGN_id
WHERE CAST(SALEORDER_DESC.SO_CLOSEDDATE AS DATE) = CAST(GETDATE() AS DATE) AND SALEORDER_DESC.SO_CLOSED = 'TRUE'
GROUP BY ISNULL(LEDGERS.Acc_cmpname, ''), SALEORDER.so_no, SALEORDER.so_date, ISNULL(ITEMMASTER.ITEM_NAME,''), ISNULL(DESIGNMASTER.DESIGN_NO, ''), ISNULL(COLORMASTER.COLOR_name, ''), ISNULL(SALEORDER_DESC.SO_CLOSEDREASON,'')
ORDER BY NAME
open CUR_MAIN
	fetch next from CUR_MAIN into @NAME, @SONO, @SODATE, @ITEMNAME, @DESIGNNO, @SHADE, @MTRS, @REASON
	while @@Fetch_STATUS = 0
		begin
			...
			BEGIN
				...
				SET @DETAILSBODY = @DETAILSBODY + N'<TD colspan="6"; style="font-family:Tahoma; font-size:11px; font-weight:bold; color:black;" bgcolor="yellow">TOTAL</TD>'
				...
				SET @DETAILSBODY = @DETAILSBODY + N'<TD colspan="1"; style="font-family:Tahoma; font-size:11px; font-weight:bold; color:black;" bgcolor="yellow"></TD>'
				set @PARTYMTRS = 0
				SET @DETAILSBODY = @DETAILSBODY + N'</tr>'
				SET @DETAILSBODY = @DETAILSBODY + N'<tr>'
			END
			...
			SET @DETAILSBODY = @DETAILSBODY + N'<TD style="font-family:Tahoma; font-size:11px;" width="150px">' + @ITEMNAME + N'</TD>'
			SET @DETAILSBODY = @DETAILSBODY + N'<TD style="font-family:Tahoma; font-size:11px;" width="100px">' + @DESIGNNO + N'</TD>'
...
SET @DBODY = N'<H1 style="font-family:Tahoma; font-size:11px;">CANCELLED DETAILS</H1>' +
				N'<Table Border = "1">' + 
				N'<Tr style="font-family:Tahoma; font-size:11px;"><Th>Name</Th><Th>SO No</Th><Th>SO Dt</Th><Th>Item Name</Th><Th>Design No</Th><Th>Shade</Th><Th>Mtrs</Th><Th>Reason</Th></Tr>'+
				+ISNULL(@DETAILSBODY,'')+ N'<tfoot><tr><td colspan="6"; style="font-family:Tahoma; font-size:11px; font-weight:bold; color:black;" bgcolor="yellow">GRAND TOTAL:</td>
				<td style="font-family:Tahoma; font-size:11px; font-weight:bold; color:black;" align="right" bgcolor="yellow">' + CAST(@TOTALMTRS AS VARCHAR) + N'</td>
				<td colspan="1"; style="font-family:Tahoma; font-size:11px; font-weight:bold; color:black;" bgcolor="yellow"></td>
				</tr></tfoot></Table></html>'
...
SET @FINALBODY = @BODY + @DBODY


EXEC msdb.dbo.sp_send_dbmail
@profile_name='TEXTRADE',
@recipients= 'arihantexim@yahoo.co.in',
@subject=@SUBJECT,
@body=@FINALBODY,
@body_format = 'HTML'