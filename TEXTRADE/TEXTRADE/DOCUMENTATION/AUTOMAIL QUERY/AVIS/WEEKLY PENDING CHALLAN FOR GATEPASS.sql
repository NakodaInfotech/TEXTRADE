DECLARE @SUBJECT VARCHAR(100), @ENTRYBODY VARCHAR(MAX), @TOTALMTRS DECIMAL(18,2), @TOTALPCS DECIMAL(18,2)
SELECT @TOTALMTRS = SUM(GDN_MTRS), @TOTALPCS = SUM(GDN_PCS) FROM GDN INNER JOIN GDN_DESC ON GDN.GDN_NO = GDN_DESC.GDN_NO AND GDN.GDN_YEARID = GDN_DESC.GDN_YEARID INNER JOIN LEDGERS ON GDN.GDN_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN DESIGNMASTER ON GDN_DESIGNID = DESIGN_ID LEFT OUTER JOIN COLORMASTER ON GDN_COLORID = COLOR_ID WHERE GDN_DESC.GDN_SALEDONE = 0 and ROUND(ISNULL(GDN.GDN_OUTMTRS,0),0) = 0 AND GDN.GDN_YEARID = (SELECT TOP 1 YEAR_ID FROM YEARMASTER inner join CMPMASTER ON YEAR_CMPID = CMP_ID WHERE CMP_DISPLAYEDNAME = 'AVIS INDUSTRIES PVT. LTD.' ORDER BY YEAR_STARTDATE DESC) 
SET @SUBJECT = 'PENDING CHALLAN FOR GATEPASS ' + CAST(DAY(GETDATE()) AS VARCHAR(2)) + '-' + CAST(MONTH( GETDATE()) AS VARCHAR(2)) + '-' + CAST(YEAR( GETDATE()) AS VARCHAR(4))
DECLARE @BODY VARCHAR(MAX)
SET @BODY = ''


DECLARE @LEDGERNAME AS VARCHAR(100), @PARTYPCS INT, @PARTYMTRS FLOAT
declare CUR_MAIN cursor for select LEDGERS.Acc_cmpname, SUM(GDN_DESC.GDN_PCS) AS PARTYPCS, SUM(GDN_DESC.GDN_MTRS) AS PARTYMTRS from GDN INNER JOIN LEDGERS ON GDN.GDN_LEDGERID = LEDGERS.ACC_ID INNER JOIN GDN_DESC ON GDN.GDN_NO = GDN_DESC.GDN_NO AND GDN.GDN_YEARID = GDN_DESC.GDN_YEARID WHERE GDN_DESC.GDN_SALEDONE = 0 AND ROUND(ISNULL(GDN.GDN_OUTMTRS,0),0) = 0  AND GDN.GDN_YEARID = (SELECT TOP 1 YEAR_ID FROM YEARMASTER inner join CMPMASTER ON YEAR_CMPID = CMP_ID WHERE CMP_DISPLAYEDNAME = 'AVIS INDUSTRIES PVT. LTD.' ORDER BY YEAR_STARTDATE DESC)  GROUP BY LEDGERS.Acc_cmpname
open CUR_MAIN
	...
	while @@Fetch_STATUS = 0
		begin
			...
			WHERE LEDGERS.ACC_CMPNAME = @LEDGERNAME AND GDN_DESC.GDN_SALEDONE = 0 AND ROUND(ISNULL(GDN.GDN_OUTMTRS,0),0) = 0  AND GDN.GDN_YEARID = (SELECT TOP 1 YEAR_ID FROM YEARMASTER inner join CMPMASTER ON YEAR_CMPID = CMP_ID WHERE CMP_DISPLAYEDNAME = 'AVIS INDUSTRIES PVT. LTD.' ORDER BY YEAR_STARTDATE DESC)
