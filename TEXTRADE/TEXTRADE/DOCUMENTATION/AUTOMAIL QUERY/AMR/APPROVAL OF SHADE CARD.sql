...
declare CUR_MAINNAME cursor for  SELECT  distinct LEDGERS.Acc_id AS ACCID , LEDGERS.Acc_website AS PARTYEMAIL
FROM            LEDGERS inner join ITEMMASTER  on ITEMMASTER.ITEM_LEDGERID = LEDGERS.Acc_id
WHERE ISNULL(LEDGERS.Acc_website,'') <> '' AND  ITEM_SHADECARD = 1 and  ITEM_SHADEAPPROVE = 0 AND LEDGERS.Acc_yearid = @YEARID 
...
open CUR_MAINNAME
	...
	while @@Fetch_STATUS = 0
		begin
			...
			WHERE ISNULL(LEDGERS.Acc_website,'') <> '' AND  ITEM_SHADECARD = 1 and  ITEM_SHADEAPPROVE = 0 AND  LEDGERS.Acc_id = @ACCID
